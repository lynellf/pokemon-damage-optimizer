...
def Solver(monster: Dict[str, Union[List[str], Dict[str, str]]], opponent: dict, level: int):
    ...
    def get_stat_totals(x: list):
        ...
        nature = natures[int(nature_index)]
    ...
    # m = cp.Variable()
    # n = cp.Variable()
    def objective(x: List[int]):
        stat_totals = get_stat_totals(x)
        damage = get_damage_calculation(x, stat_totals)
        return damage

    # x = cp.Variable(14, integer=True)
    x = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 1, 2, 3, 3]
    constraints = []
    obj = cp.Maximize(objective(x))
