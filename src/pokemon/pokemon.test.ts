...
describe("a pokemon object", () => {
  it("returns a charizard with correct type and stat totals", () => {
    ...
    const hasStatTotals =
      hasHp && hasatk && hasDef && hasSpa && hasSpd && hasSpeed;
    const isFlyingType =
      charizard.types.find(t => t.toLowerCase() === "flying") !== undefined;
    const isFireType =
      charizard.types.find(t => t.toLowerCase() === "fire") !== undefined;
    const isValid = hasStatTotals && isFlyingType && isFireType;
    expect(isValid).toBe(true);
  });

  it("returns a Alolan Raichu with correct type and stat totals", () => {
    const alolanRaichu = new Monster({ name: "Raichu-Alola" });
    ...
    const isElectricType =
      alolanRaichu.types.find(t => t.toLowerCase() === "electric") !==
      undefined;
    const isPsychicType =
      alolanRaichu.types.find(t => t.toLowerCase() === "psychic") !== undefined;
    const isValid = hasStatTotals && isElectricType && isPsychicType;
    expect(isValid).toBe(true);
  });

  it("returns a normal Raichu without the move magic coat", () => {
    const raichu = new Monster({ name: "Raichu" });
