...
import abilities from "./vgcAbilities";

type TKey = keyof typeof dexData;
type TMonKeys = keyof typeof dataSet["pokemon"];
type TMon = typeof dataSet["pokemon"][TMonKeys];
type TMoves = { [key: string]: string[] };
type TRotom = { [key: string]: string };
type TBaseStats = {
  hp: number;
  at: number;
  df: number;
...
const dexDataKeys = Object.keys(dexData);
const scrapedKeys = Object.keys(dataSet.pokemon);

function validateStr(str: string) {
  const isValid = typeof str === "string";
  const hasCharacters = str.length > 0;

  if (!isValid || !hasCharacters) {
    throw new Error("Invalid string parameter provided");
  }
}

function strCheck(query: string, str: string) {
  return str.includes(query);
}

function nameCheck(query: string, str: string) {
  return query.toLowerCase().includes(str.toLowerCase());
}

function gen8NameFilter(output: string[], key: string) {
  const matchedKey: string | undefined = scrapedKeys.find(k =>
    strCheck(k, key)
  );
  const result = matchedKey ? [...output, key] : output;
  return result;
}

function getGen8Monsters(output: TOutput, key: string) {
  const value = dexData[key as TKey];
  output[key as TKey] = value;
  return output;
}

function getScrapedMon(query: string) {
  const queryResult = scrapedKeys.find(k => nameCheck(query, k));
  const output: TMon | undefined = queryResult
    ? dataSet.pokemon[queryResult as TMonKeys]
    : undefined;
  return output;
}

function convertAbility(query: string) {
  const queryResult = abilities.find(ab => nameCheck(query, ab));
  return queryResult ? queryResult : "";
}

function getAbilities(query: string) {
  const queryResult = getScrapedMon(query);
  const rawAbilities = queryResult ? queryResult.abilities : [];
  const convertedAbilities = rawAbilities.map(convertAbility);
  return convertedAbilities;
}

function rotomCheck(query: string) {
  validateStr(query);
  const cases: TRotom = {
    "Rotom-W": "Wash Rotom",
    "Rotom-H": "Heat Rotom",
  ...
  return cases[query] ?? "Rotom";
}

function necrozmaCheck(query: string) {
  validateStr(query);
  const cases: TRotom = {
  ...
  return cases[query] ?? "Normal";
}

function getForm(query: string) {
  let form = "Normal";
  nameCheck(query, "Alola") ? (form = "Alolan Form") : undefined;
  nameCheck(query, "Galar") ? (form = "Galarian Form") : undefined;
  nameCheck(query, "Rotom") ? (form = rotomCheck(query)) : undefined;
  nameCheck(query, "Necrozma") ? (form = necrozmaCheck(query)) : undefined;
  return form;
}
...
function filterForms({ name, forms, output, form }: TFilterForms) {
  const hasForm = forms.includes(form);
...
function getMoves(query: string) {
  const queryResult = getScrapedMon(query);
  const rawMoves = queryResult ? queryResult.moves : ({} as TMoves);
  const form = getForm(query);
  const moves = Object.entries(rawMoves).reduce(
    (output, [name, forms]) => filterForms({ output, name, forms, form }),
    [] as string[]
  );
  return moves
}

function getAbilitiesAndMoves(output: any, name: string, stats: TPokemon) {
  const abilities = getAbilities(name);
  const moves = getMoves(name)
  const newStats = { ...stats, ab: abilities, moves };
  output[name] = newStats;
  return output;
}

export const filteredKeys = dexDataKeys.reduce(gen8NameFilter, [] as string[]);
const gen8Mons = filteredKeys.reduce(getGen8Monsters, {} as TOutput);
export const pokedex = Object.entries(gen8Mons).reduce(
  (output, [name, stats]) => getAbilitiesAndMoves(output, name, stats),
  {} as any
);
