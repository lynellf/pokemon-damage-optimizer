...
def Solver(monster: Dict[str, Union[List[str], Dict[str, str]]], opponent: dict, level: int):
    ...
    bnds = (
        ...
        (0, 31),
        (0, len(natures) - 1),
        (0, len(get_attack_moves()) - 1),
        (0, 1000)
    )

    def objective(x, sign=1):
        stat_totals = get_stat_totals(x)
        output = sign * get_damage_calculation(x, stat_totals)
        x[14] = output
        return output

    def objective_constraint(x):
        obj_output = x[14]
        return obj_output - opponent.get('hp')

    def ev_constraint(x):
        return -1

    constraints = ({'type': 'ineq', 'fun': objective_constraint}, {
                   'type': 'ineq', 'fun': ev_constraint})

    solution = minimize(objective, x, args=(-1.0,),
                        method='SLSQP', bounds=bnds, constraints=(constraints))
    results = solution.x
    print('Final SSE Objective: ' + str(objective(x)))
    print('Solution')
    print('hpEV = ' + str(results[0]))
    print('atkEV = ' + str(results[1]))
    print('defEV = ' + str(results[2]))
    print('spaEV = ' + str(results[3]))
    print('speedEV = ' + str(results[4]))
    print('hpIV = ' + str(results[5]))
    print('atkIV = ' + str(results[6]))
    print('defIV = ' + str(results[7]))
    print('spaIV = ' + str(results[8]))
    print('spdIV = ' + str(results[9]))
    print('speedIV = ' + str(results[10]))
    # print('nature = ' + str(natures[results[10]]))
    # print('move = ' + str(get_attack_moves()[13]))
    print('damage = ' + str(results[14]))
