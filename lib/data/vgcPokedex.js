"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const output_json_1 = __importDefault(require("./output.json"));
const deepmerge_1 = __importDefault(require("deepmerge"));
const vgcAbilities_1 = __importDefault(require("./vgcAbilities"));
const GEN_1 = {
    Abra: {
        t1: "Psychic",
        bs: {
            hp: 25,
            at: 20,
            df: 15,
            sa: 105,
            sd: 55,
            sp: 90,
            sl: 105
        },
        w: 19.5
    },
    Aerodactyl: {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 80,
            at: 105,
            df: 65,
            sa: 60,
            sd: 75,
            sp: 130,
            sl: 60
        },
        w: 59.0
    },
    Alakazam: {
        t1: "Psychic",
        bs: {
            hp: 55,
            at: 50,
            df: 45,
            sa: 135,
            sd: 85,
            sp: 120,
            sl: 135
        },
        w: 48.0
    },
    Arbok: {
        t1: "Poison",
        bs: {
            hp: 60,
            at: 85,
            df: 69,
            sa: 65,
            sd: 79,
            sp: 80,
            sl: 65
        },
        w: 65.0
    },
    Arcanine: {
        t1: "Fire",
        bs: {
            hp: 90,
            at: 110,
            df: 80,
            sa: 100,
            sd: 80,
            sp: 95,
            sl: 80
        },
        w: 155.0
    },
    Articuno: {
        t1: "Ice",
        t2: "Flying",
        bs: {
            hp: 90,
            at: 85,
            df: 100,
            sa: 95,
            sd: 125,
            sp: 85,
            sl: 125
        },
        w: 55.4
    },
    Beedrill: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 65,
            at: 80,
            df: 40,
            sa: 45,
            sd: 80,
            sp: 75,
            sl: 45
        },
        w: 29.5,
        ab: "Swarm"
    },
    Bellsprout: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 50,
            at: 75,
            df: 35,
            sa: 70,
            sd: 30,
            sp: 40,
            sl: 70
        },
        w: 4.0,
        ab: "Chlorophyll"
    },
    Blastoise: {
        t1: "Water",
        bs: {
            hp: 79,
            at: 83,
            df: 100,
            sa: 85,
            sd: 105,
            sp: 78,
            sl: 85
        },
        w: 85.5,
        ab: "Torrent"
    },
    Bulbasaur: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 45,
            at: 49,
            df: 49,
            sa: 65,
            sd: 65,
            sp: 45,
            sl: 65
        },
        w: 6.9,
        ab: "Overgrow"
    },
    Butterfree: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 45,
            df: 50,
            sa: 80,
            sd: 80,
            sp: 70,
            sl: 80
        },
        w: 32.0
    },
    Caterpie: {
        t1: "Bug",
        bs: {
            hp: 45,
            at: 30,
            df: 35,
            sa: 20,
            sd: 20,
            sp: 45,
            sl: 20
        },
        w: 2.9
    },
    Chansey: {
        t1: "Normal",
        bs: {
            hp: 250,
            at: 5,
            df: 5,
            sa: 35,
            sd: 105,
            sp: 50,
            sl: 105
        },
        w: 34.6
    },
    Charizard: {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 78,
            at: 84,
            df: 78,
            sa: 109,
            sd: 85,
            sp: 100,
            sl: 85
        },
        w: 90.5,
        ab: "Blaze"
    },
    Charmander: {
        t1: "Fire",
        bs: {
            hp: 39,
            at: 52,
            df: 43,
            sa: 60,
            sd: 50,
            sp: 65,
            sl: 50
        },
        w: 8.5,
        ab: "Blaze"
    },
    Charmeleon: {
        t1: "Fire",
        bs: {
            hp: 58,
            at: 64,
            df: 58,
            sa: 80,
            sd: 65,
            sp: 80,
            sl: 65
        },
        w: 19.0,
        ab: "Blaze"
    },
    Clefable: {
        t1: "Normal",
        bs: {
            hp: 95,
            at: 70,
            df: 73,
            sa: 85,
            sd: 90,
            sp: 60,
            sl: 85
        },
        w: 40.0
    },
    Clefairy: {
        t1: "Normal",
        bs: {
            hp: 70,
            at: 45,
            df: 48,
            sa: 60,
            sd: 65,
            sp: 35,
            sl: 60
        },
        w: 7.5
    },
    Cloyster: {
        t1: "Water",
        t2: "Ice",
        bs: {
            hp: 50,
            at: 95,
            df: 180,
            sa: 85,
            sd: 45,
            sp: 70,
            sl: 85
        },
        w: 132.5,
        ab: "Shell Armor"
    },
    Cubone: {
        t1: "Ground",
        bs: {
            hp: 50,
            at: 50,
            df: 95,
            sa: 40,
            sd: 50,
            sp: 35,
            sl: 40
        },
        w: 6.5
    },
    Dewgong: {
        t1: "Water",
        t2: "Ice",
        bs: {
            hp: 90,
            at: 70,
            df: 80,
            sa: 70,
            sd: 95,
            sp: 70,
            sl: 95
        },
        w: 120.0,
        ab: "Thick Fat"
    },
    Diglett: {
        t1: "Ground",
        bs: {
            hp: 10,
            at: 55,
            df: 25,
            sa: 35,
            sd: 45,
            sp: 95,
            sl: 45
        },
        w: 0.8
    },
    Ditto: {
        t1: "Normal",
        bs: {
            hp: 48,
            at: 48,
            df: 48,
            sa: 48,
            sd: 48,
            sp: 48,
            sl: 48
        },
        w: 4.0
    },
    Dodrio: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 110,
            df: 70,
            sa: 60,
            sd: 60,
            sp: 100,
            sl: 60
        },
        w: 85.2
    },
    Doduo: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 35,
            at: 85,
            df: 45,
            sa: 35,
            sd: 35,
            sp: 75,
            sl: 35
        },
        w: 39.2
    },
    Dragonair: {
        t1: "Dragon",
        bs: {
            hp: 61,
            at: 84,
            df: 65,
            sa: 70,
            sd: 70,
            sp: 70,
            sl: 70
        },
        w: 16.5
    },
    Dragonite: {
        t1: "Dragon",
        t2: "Flying",
        bs: {
            hp: 91,
            at: 134,
            df: 95,
            sa: 100,
            sd: 100,
            sp: 80,
            sl: 100
        },
        w: 210.0
    },
    Dratini: {
        t1: "Dragon",
        bs: {
            hp: 41,
            at: 64,
            df: 45,
            sa: 50,
            sd: 50,
            sp: 50,
            sl: 50
        },
        w: 3.3
    },
    Drowzee: {
        t1: "Psychic",
        bs: {
            hp: 60,
            at: 48,
            df: 45,
            sa: 43,
            sd: 90,
            sp: 42,
            sl: 90
        },
        w: 32.4
    },
    Dugtrio: {
        t1: "Ground",
        bs: {
            hp: 35,
            at: 80,
            df: 50,
            sa: 50,
            sd: 70,
            sp: 120,
            sl: 70
        },
        w: 33.3
    },
    Eevee: {
        t1: "Normal",
        bs: {
            hp: 55,
            at: 55,
            df: 50,
            sa: 45,
            sd: 65,
            sp: 55,
            sl: 65
        },
        w: 6.5
    },
    Ekans: {
        t1: "Poison",
        bs: {
            hp: 35,
            at: 60,
            df: 44,
            sa: 40,
            sd: 54,
            sp: 55,
            sl: 40
        },
        w: 6.9
    },
    Electabuzz: {
        t1: "Electric",
        bs: {
            hp: 65,
            at: 83,
            df: 57,
            sa: 95,
            sd: 85,
            sp: 105,
            sl: 85
        },
        w: 30.0
    },
    Electrode: {
        t1: "Electric",
        bs: {
            hp: 60,
            at: 50,
            df: 70,
            sa: 80,
            sd: 80,
            sp: 140,
            sl: 80
        },
        w: 66.6
    },
    Exeggcute: {
        t1: "Grass",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 40,
            df: 80,
            sa: 60,
            sd: 45,
            sp: 40,
            sl: 60
        },
        w: 2.5,
        ab: "Chlorophyll"
    },
    Exeggutor: {
        t1: "Grass",
        t2: "Psychic",
        bs: {
            hp: 95,
            at: 95,
            df: 85,
            sa: 125,
            sd: 65,
            sp: 55,
            sl: 125
        },
        w: 120.0,
        ab: "Chlorophyll"
    },
    "Farfetch'd": {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 52,
            at: 65,
            df: 55,
            sa: 58,
            sd: 62,
            sp: 60,
            sl: 58
        },
        w: 15.0
    },
    Fearow: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 90,
            df: 65,
            sa: 61,
            sd: 61,
            sp: 100,
            sl: 61
        },
        w: 38.0
    },
    Flareon: {
        t1: "Fire",
        bs: {
            hp: 65,
            at: 130,
            df: 60,
            sa: 95,
            sd: 110,
            sp: 65,
            sl: 110
        },
        w: 25.0,
        ab: "Flash Fire"
    },
    Gastly: {
        t1: "Ghost",
        t2: "Poison",
        bs: {
            hp: 30,
            at: 35,
            df: 30,
            sa: 100,
            sd: 35,
            sp: 80,
            sl: 100
        },
        w: 0.1,
        ab: "Levitate"
    },
    Gengar: {
        t1: "Ghost",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 65,
            df: 60,
            sa: 130,
            sd: 75,
            sp: 110,
            sl: 130
        },
        w: 40.5,
        ab: "Levitate"
    },
    Geodude: {
        t1: "Rock",
        t2: "Ground",
        bs: {
            hp: 40,
            at: 80,
            df: 100,
            sa: 30,
            sd: 30,
            sp: 20,
            sl: 30
        },
        w: 20.0
    },
    Gloom: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 65,
            df: 70,
            sa: 85,
            sd: 75,
            sp: 40,
            sl: 85
        },
        w: 8.6,
        ab: "Chlorophyll"
    },
    Golbat: {
        t1: "Poison",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 80,
            df: 70,
            sa: 65,
            sd: 75,
            sp: 90,
            sl: 75
        },
        w: 55.0
    },
    Goldeen: {
        t1: "Water",
        bs: {
            hp: 45,
            at: 67,
            df: 60,
            sa: 35,
            sd: 50,
            sp: 63,
            sl: 50
        },
        w: 15.0
    },
    Golduck: {
        t1: "Water",
        bs: {
            hp: 80,
            at: 82,
            df: 78,
            sa: 95,
            sd: 80,
            sp: 85,
            sl: 80
        },
        w: 76.6
    },
    Golem: {
        t1: "Rock",
        t2: "Ground",
        bs: {
            hp: 80,
            at: 110,
            df: 130,
            sa: 55,
            sd: 65,
            sp: 45,
            sl: 55
        },
        w: 300.0
    },
    Graveler: {
        t1: "Rock",
        t2: "Ground",
        bs: {
            hp: 55,
            at: 95,
            df: 115,
            sa: 45,
            sd: 45,
            sp: 35,
            sl: 45
        },
        w: 105.0
    },
    Grimer: {
        t1: "Poison",
        bs: {
            hp: 80,
            at: 80,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 25,
            sl: 40
        },
        w: 30.0
    },
    Growlithe: {
        t1: "Fire",
        bs: {
            hp: 55,
            at: 70,
            df: 45,
            sa: 70,
            sd: 50,
            sp: 60,
            sl: 50
        },
        w: 19.0
    },
    Gyarados: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 95,
            at: 125,
            df: 79,
            sa: 60,
            sd: 100,
            sp: 81,
            sl: 100
        },
        w: 235.0,
        ab: "Intimidate"
    },
    Haunter: {
        t1: "Ghost",
        t2: "Poison",
        bs: {
            hp: 45,
            at: 50,
            df: 45,
            sa: 115,
            sd: 55,
            sp: 95,
            sl: 115
        },
        w: 0.1,
        ab: "Levitate"
    },
    Hitmonchan: {
        t1: "Fighting",
        bs: {
            hp: 50,
            at: 105,
            df: 79,
            sa: 35,
            sd: 110,
            sp: 76,
            sl: 35
        },
        w: 50.2
    },
    Hitmonlee: {
        t1: "Fighting",
        bs: {
            hp: 50,
            at: 120,
            df: 53,
            sa: 35,
            sd: 110,
            sp: 87,
            sl: 35
        },
        w: 49.8
    },
    Horsea: {
        t1: "Water",
        bs: {
            hp: 30,
            at: 40,
            df: 70,
            sa: 70,
            sd: 25,
            sp: 60,
            sl: 70
        },
        w: 8.0,
        ab: "Swift Swim"
    },
    Hypno: {
        t1: "Psychic",
        bs: {
            hp: 85,
            at: 73,
            df: 70,
            sa: 73,
            sd: 115,
            sp: 67,
            sl: 115
        },
        w: 75.6
    },
    Ivysaur: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 62,
            df: 63,
            sa: 80,
            sd: 80,
            sp: 60,
            sl: 80
        },
        w: 13.0,
        ab: "Overgrow"
    },
    Jigglypuff: {
        t1: "Normal",
        bs: {
            hp: 115,
            at: 45,
            df: 20,
            sa: 45,
            sd: 25,
            sp: 20,
            sl: 25
        },
        w: 5.5
    },
    Jolteon: {
        t1: "Electric",
        bs: {
            hp: 65,
            at: 65,
            df: 60,
            sa: 110,
            sd: 95,
            sp: 130,
            sl: 110
        },
        w: 24.5,
        ab: "Volt Abdsorb"
    },
    Jynx: {
        t1: "Ice",
        t2: "Psychic",
        bs: {
            hp: 65,
            at: 50,
            df: 35,
            sa: 115,
            sd: 95,
            sp: 95,
            sl: 95
        },
        w: 40.6
    },
    Kabuto: {
        t1: "Rock",
        t2: "Water",
        bs: {
            hp: 30,
            at: 80,
            df: 90,
            sa: 55,
            sd: 45,
            sp: 55,
            sl: 45
        },
        w: 11.5
    },
    Kabutops: {
        t1: "Rock",
        t2: "Water",
        bs: {
            hp: 60,
            at: 115,
            df: 105,
            sa: 65,
            sd: 70,
            sp: 80,
            sl: 70
        },
        w: 40.5
    },
    Kadabra: {
        t1: "Psychic",
        bs: {
            hp: 40,
            at: 35,
            df: 30,
            sa: 120,
            sd: 70,
            sp: 105,
            sl: 120
        },
        w: 56.5
    },
    Kakuna: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 45,
            at: 25,
            df: 50,
            sa: 25,
            sd: 25,
            sp: 35,
            sl: 25
        },
        w: 10.0,
        ab: "Shed Skin"
    },
    Kangaskhan: {
        t1: "Normal",
        bs: {
            hp: 105,
            at: 95,
            df: 80,
            sa: 40,
            sd: 80,
            sp: 90,
            sl: 40
        },
        w: 80.0
    },
    Kingler: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 130,
            df: 115,
            sa: 50,
            sd: 50,
            sp: 75,
            sl: 50
        },
        w: 60.0
    },
    Koffing: {
        t1: "Poison",
        bs: {
            hp: 40,
            at: 65,
            df: 95,
            sa: 60,
            sd: 45,
            sp: 35,
            sl: 60
        },
        w: 1.0,
        ab: "Levitate"
    },
    Krabby: {
        t1: "Water",
        bs: {
            hp: 30,
            at: 105,
            df: 90,
            sa: 25,
            sd: 25,
            sp: 50,
            sl: 25
        },
        w: 6.5
    },
    Lapras: {
        t1: "Water",
        t2: "Ice",
        bs: {
            hp: 130,
            at: 85,
            df: 80,
            sa: 85,
            sd: 95,
            sp: 60,
            sl: 95
        },
        w: 220.0
    },
    Lickitung: {
        t1: "Normal",
        bs: {
            hp: 90,
            at: 55,
            df: 75,
            sa: 60,
            sd: 75,
            sp: 30,
            sl: 60
        },
        w: 65.5
    },
    Machamp: {
        t1: "Fighting",
        bs: {
            hp: 90,
            at: 130,
            df: 80,
            sa: 65,
            sd: 85,
            sp: 55,
            sl: 65
        },
        w: 130.0,
        ab: "Guts"
    },
    Machoke: {
        t1: "Fighting",
        bs: {
            hp: 80,
            at: 100,
            df: 70,
            sa: 50,
            sd: 60,
            sp: 45,
            sl: 50
        },
        w: 70.5,
        ab: "Guts"
    },
    Machop: {
        t1: "Fighting",
        bs: {
            hp: 70,
            at: 80,
            df: 50,
            sa: 35,
            sd: 35,
            sp: 35,
            sl: 35
        },
        w: 19.5,
        ab: "Guts"
    },
    Magikarp: {
        t1: "Water",
        bs: {
            hp: 20,
            at: 10,
            df: 55,
            sa: 15,
            sd: 20,
            sp: 80,
            sl: 20
        },
        w: 10.0,
        ab: "Swift Swim"
    },
    Magmar: {
        t1: "Fire",
        bs: {
            hp: 65,
            at: 95,
            df: 57,
            sa: 100,
            sd: 85,
            sp: 93,
            sl: 85
        },
        w: 44.5
    },
    Magnemite: {
        t1: "Electric",
        bs: {
            hp: 25,
            at: 35,
            df: 70,
            sa: 95,
            sd: 55,
            sp: 45,
            sl: 95
        },
        w: 6.0
    },
    Magneton: {
        t1: "Electric",
        bs: {
            hp: 50,
            at: 60,
            df: 95,
            sa: 120,
            sd: 70,
            sp: 70,
            sl: 120
        },
        w: 60.0
    },
    Mankey: {
        t1: "Fighting",
        bs: {
            hp: 40,
            at: 80,
            df: 35,
            sa: 35,
            sd: 45,
            sp: 70,
            sl: 35
        },
        w: 28.0
    },
    Marowak: {
        t1: "Ground",
        bs: {
            hp: 60,
            at: 80,
            df: 110,
            sa: 50,
            sd: 80,
            sp: 45,
            sl: 50
        },
        w: 45.0
    },
    Meowth: {
        t1: "Normal",
        bs: {
            hp: 40,
            at: 45,
            df: 35,
            sa: 40,
            sd: 40,
            sp: 90,
            sl: 40
        },
        w: 4.2
    },
    Metapod: {
        t1: "Bug",
        bs: {
            hp: 50,
            at: 20,
            df: 55,
            sa: 25,
            sd: 25,
            sp: 30,
            sl: 25
        },
        w: 9.9,
        ab: "Shed Skin"
    },
    Mew: {
        t1: "Psychic",
        bs: {
            hp: 100,
            at: 100,
            df: 100,
            sa: 100,
            sd: 100,
            sp: 100,
            sl: 100
        },
        w: 4.0,
        ab: "Synchronize"
    },
    Mewtwo: {
        t1: "Psychic",
        bs: {
            hp: 106,
            at: 110,
            df: 90,
            sa: 154,
            sd: 90,
            sp: 130,
            sl: 154
        },
        w: 122.0
    },
    Moltres: {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 90,
            at: 100,
            df: 90,
            sa: 125,
            sd: 85,
            sp: 90,
            sl: 125
        },
        w: 60.0
    },
    "Mr. Mime": {
        t1: "Psychic",
        bs: {
            hp: 40,
            at: 45,
            df: 65,
            sa: 100,
            sd: 120,
            sp: 90,
            sl: 100
        },
        w: 54.5,
        ab: "Soundproof"
    },
    Muk: {
        t1: "Poison",
        bs: {
            hp: 105,
            at: 105,
            df: 75,
            sa: 65,
            sd: 100,
            sp: 50,
            sl: 65
        },
        w: 30.0
    },
    Nidoking: {
        t1: "Poison",
        t2: "Ground",
        bs: {
            hp: 81,
            at: 92,
            df: 77,
            sa: 85,
            sd: 75,
            sp: 85,
            sl: 75
        },
        w: 62.0
    },
    Nidoqueen: {
        t1: "Poison",
        t2: "Ground",
        bs: {
            hp: 90,
            at: 82,
            df: 87,
            sa: 75,
            sd: 85,
            sp: 76,
            sl: 75
        },
        w: 60.0
    },
    "Nidoran-F": {
        t1: "Poison",
        bs: {
            hp: 55,
            at: 47,
            df: 52,
            sa: 40,
            sd: 40,
            sp: 41,
            sl: 40
        },
        w: 7.0
    },
    "Nidoran-M": {
        t1: "Poison",
        bs: {
            hp: 46,
            at: 57,
            df: 40,
            sa: 40,
            sd: 40,
            sp: 50,
            sl: 40
        },
        w: 9.0
    },
    Nidorina: {
        t1: "Poison",
        bs: {
            hp: 70,
            at: 62,
            df: 67,
            sa: 55,
            sd: 55,
            sp: 56,
            sl: 55
        },
        w: 20.0
    },
    Nidorino: {
        t1: "Poison",
        bs: {
            hp: 61,
            at: 72,
            df: 57,
            sa: 55,
            sd: 55,
            sp: 65,
            sl: 55
        },
        w: 19.5
    },
    Ninetales: {
        t1: "Fire",
        bs: {
            hp: 73,
            at: 76,
            df: 75,
            sa: 81,
            sd: 100,
            sp: 100,
            sl: 100
        },
        w: 19.9,
        ab: "Flash Fire"
    },
    Oddish: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 45,
            at: 50,
            df: 55,
            sa: 75,
            sd: 65,
            sp: 30,
            sl: 75
        },
        w: 5.4,
        ab: "Chlorophyll"
    },
    Omanyte: {
        t1: "Rock",
        t2: "Water",
        bs: {
            hp: 35,
            at: 40,
            df: 100,
            sa: 90,
            sd: 55,
            sp: 35,
            sl: 90
        },
        w: 7.5
    },
    Omastar: {
        t1: "Rock",
        t2: "Water",
        bs: {
            hp: 70,
            at: 60,
            df: 125,
            sa: 115,
            sd: 70,
            sp: 55,
            sl: 115
        },
        w: 35.0
    },
    Onix: {
        t1: "Rock",
        t2: "Ground",
        bs: {
            hp: 35,
            at: 45,
            df: 160,
            sa: 30,
            sd: 45,
            sp: 70,
            sl: 30
        },
        w: 210.0
    },
    Paras: {
        t1: "Bug",
        t2: "Grass",
        bs: {
            hp: 35,
            at: 70,
            df: 55,
            sa: 45,
            sd: 55,
            sp: 25,
            sl: 55
        },
        w: 5.4
    },
    Parasect: {
        t1: "Bug",
        t2: "Grass",
        bs: {
            hp: 60,
            at: 95,
            df: 80,
            sa: 60,
            sd: 80,
            sp: 30,
            sl: 80
        },
        w: 29.5
    },
    Persian: {
        t1: "Normal",
        bs: {
            hp: 65,
            at: 70,
            df: 60,
            sa: 65,
            sd: 65,
            sp: 115,
            sl: 65
        },
        w: 32.0
    },
    Pidgeot: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 83,
            at: 80,
            df: 75,
            sa: 70,
            sd: 70,
            sp: 91,
            sl: 70
        },
        w: 39.5
    },
    Pidgeotto: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 63,
            at: 60,
            df: 55,
            sa: 50,
            sd: 50,
            sp: 71,
            sl: 50
        },
        w: 30.0
    },
    Pidgey: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 45,
            df: 40,
            sa: 35,
            sd: 35,
            sp: 56,
            sl: 35
        },
        w: 1.8
    },
    Pikachu: {
        t1: "Electric",
        bs: {
            hp: 35,
            at: 55,
            df: 30,
            sa: 50,
            sd: 40,
            sp: 90,
            sl: 50
        },
        w: 6.0
    },
    Pinsir: {
        t1: "Bug",
        bs: {
            hp: 65,
            at: 125,
            df: 100,
            sa: 55,
            sd: 70,
            sp: 85,
            sl: 55
        },
        w: 55.0,
        ab: "Hyper Cutter"
    },
    Poliwag: {
        t1: "Water",
        bs: {
            hp: 40,
            at: 50,
            df: 40,
            sa: 40,
            sd: 40,
            sp: 90,
            sl: 40
        },
        w: 12.4
    },
    Poliwhirl: {
        t1: "Water",
        bs: {
            hp: 65,
            at: 65,
            df: 65,
            sa: 50,
            sd: 50,
            sp: 90,
            sl: 50
        },
        w: 20.0
    },
    Poliwrath: {
        t1: "Water",
        t2: "Fighting",
        bs: {
            hp: 90,
            at: 85,
            df: 95,
            sa: 70,
            sd: 90,
            sp: 70,
            sl: 70
        },
        w: 54.0
    },
    Ponyta: {
        t1: "Fire",
        bs: {
            hp: 50,
            at: 85,
            df: 55,
            sa: 65,
            sd: 65,
            sp: 90,
            sl: 65
        },
        w: 30.0
    },
    Porygon: {
        t1: "Normal",
        bs: {
            hp: 65,
            at: 60,
            df: 70,
            sa: 85,
            sd: 75,
            sp: 40,
            sl: 75
        },
        w: 36.5
    },
    Primeape: {
        t1: "Fighting",
        bs: {
            hp: 65,
            at: 105,
            df: 60,
            sa: 60,
            sd: 70,
            sp: 95,
            sl: 60
        },
        w: 32.0
    },
    Psyduck: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 52,
            df: 48,
            sa: 65,
            sd: 50,
            sp: 55,
            sl: 50
        },
        w: 19.6
    },
    Raichu: {
        t1: "Electric",
        bs: {
            hp: 60,
            at: 90,
            df: 55,
            sa: 90,
            sd: 80,
            sp: 100,
            sl: 90
        },
        w: 30.0
    },
    Rapidash: {
        t1: "Fire",
        bs: {
            hp: 65,
            at: 100,
            df: 70,
            sa: 80,
            sd: 80,
            sp: 105,
            sl: 80
        },
        w: 95.0
    },
    Raticate: {
        t1: "Normal",
        bs: {
            hp: 55,
            at: 81,
            df: 60,
            sa: 50,
            sd: 70,
            sp: 97,
            sl: 50
        },
        w: 18.5
    },
    Rattata: {
        t1: "Normal",
        bs: {
            hp: 30,
            at: 56,
            df: 35,
            sa: 25,
            sd: 35,
            sp: 72,
            sl: 25
        },
        w: 3.5
    },
    Rhydon: {
        t1: "Ground",
        t2: "Rock",
        bs: {
            hp: 105,
            at: 130,
            df: 120,
            sa: 45,
            sd: 45,
            sp: 40,
            sl: 45
        },
        w: 120.0
    },
    Rhyhorn: {
        t1: "Ground",
        t2: "Rock",
        bs: {
            hp: 80,
            at: 85,
            df: 95,
            sa: 30,
            sd: 30,
            sp: 25,
            sl: 30
        },
        w: 115.0
    },
    Sandshrew: {
        t1: "Ground",
        bs: {
            hp: 50,
            at: 75,
            df: 85,
            sa: 20,
            sd: 30,
            sp: 40,
            sl: 30
        },
        w: 12.0,
        ab: "Sand Veil"
    },
    Sandslash: {
        t1: "Ground",
        bs: {
            hp: 75,
            at: 100,
            df: 110,
            sa: 45,
            sd: 55,
            sp: 65,
            sl: 55
        },
        w: 29.5,
        ab: "Sand Veil"
    },
    Scyther: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 70,
            at: 110,
            df: 80,
            sa: 55,
            sd: 80,
            sp: 105,
            sl: 55
        },
        w: 56.0,
        ab: "Swarm"
    },
    Seadra: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 65,
            df: 95,
            sa: 95,
            sd: 45,
            sp: 85,
            sl: 95
        },
        w: 25.0
    },
    Seaking: {
        t1: "Water",
        bs: {
            hp: 80,
            at: 92,
            df: 65,
            sa: 65,
            sd: 80,
            sp: 68,
            sl: 80
        },
        w: 39.0
    },
    Seel: {
        t1: "Water",
        bs: {
            hp: 65,
            at: 45,
            df: 55,
            sa: 45,
            sd: 70,
            sp: 45,
            sl: 70
        },
        w: 90.0,
        ab: "Thick Fat"
    },
    Shellder: {
        t1: "Water",
        bs: {
            hp: 30,
            at: 65,
            df: 100,
            sa: 45,
            sd: 25,
            sp: 40,
            sl: 45
        },
        w: 4.0,
        ab: "Shell Armor"
    },
    Slowbro: {
        t1: "Water",
        t2: "Psychic",
        bs: {
            hp: 95,
            at: 75,
            df: 110,
            sa: 100,
            sd: 80,
            sp: 30,
            sl: 80
        },
        w: 78.5
    },
    Slowpoke: {
        t1: "Water",
        t2: "Psychic",
        bs: {
            hp: 90,
            at: 65,
            df: 65,
            sa: 40,
            sd: 40,
            sp: 15,
            sl: 40
        },
        w: 36.0
    },
    Snorlax: {
        t1: "Normal",
        bs: {
            hp: 160,
            at: 110,
            df: 65,
            sa: 65,
            sd: 110,
            sp: 30,
            sl: 65
        },
        w: 460.0
    },
    Spearow: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 60,
            df: 30,
            sa: 31,
            sd: 31,
            sp: 70,
            sl: 31
        },
        w: 2.0
    },
    Squirtle: {
        t1: "Water",
        bs: {
            hp: 44,
            at: 48,
            df: 65,
            sa: 50,
            sd: 64,
            sp: 43,
            sl: 50
        },
        w: 9.0,
        ab: "Torrent"
    },
    Starmie: {
        t1: "Water",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 75,
            df: 85,
            sa: 100,
            sd: 85,
            sp: 115,
            sl: 100
        },
        w: 80.0
    },
    Staryu: {
        t1: "Water",
        bs: {
            hp: 30,
            at: 45,
            df: 55,
            sa: 70,
            sd: 55,
            sp: 85,
            sl: 70
        },
        w: 34.5
    },
    Tangela: {
        t1: "Grass",
        bs: {
            hp: 65,
            at: 55,
            df: 115,
            sa: 100,
            sd: 40,
            sp: 60,
            sl: 100
        },
        w: 35.0,
        ab: "Chlorophyll"
    },
    Tauros: {
        t1: "Normal",
        bs: {
            hp: 75,
            at: 100,
            df: 95,
            sa: 40,
            sd: 70,
            sp: 110,
            sl: 70
        },
        w: 88.4,
        ab: "Intimidate"
    },
    Tentacool: {
        t1: "Water",
        t2: "Poison",
        bs: {
            hp: 40,
            at: 40,
            df: 35,
            sa: 50,
            sd: 100,
            sp: 70,
            sl: 100
        },
        w: 45.5
    },
    Tentacruel: {
        t1: "Water",
        t2: "Poison",
        bs: {
            hp: 80,
            at: 70,
            df: 65,
            sa: 80,
            sd: 120,
            sp: 100,
            sl: 120
        },
        w: 55.0
    },
    Vaporeon: {
        t1: "Water",
        bs: {
            hp: 130,
            at: 65,
            df: 60,
            sa: 110,
            sd: 95,
            sp: 65,
            sl: 110
        },
        w: 29.0,
        ab: "Water Absorb"
    },
    Venomoth: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 70,
            at: 65,
            df: 60,
            sa: 90,
            sd: 75,
            sp: 90,
            sl: 90
        },
        w: 12.5
    },
    Venonat: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 55,
            df: 50,
            sa: 40,
            sd: 55,
            sp: 45,
            sl: 40
        },
        w: 30.0
    },
    Venusaur: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 80,
            at: 82,
            df: 83,
            sa: 100,
            sd: 100,
            sp: 80,
            sl: 100
        },
        w: 100.0,
        ab: "Overgrow"
    },
    Victreebel: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 80,
            at: 105,
            df: 65,
            sa: 100,
            sd: 60,
            sp: 70,
            sl: 100
        },
        w: 15.5,
        ab: "Chlorophyll"
    },
    Vileplume: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 75,
            at: 80,
            df: 85,
            sa: 100,
            sd: 90,
            sp: 50,
            sl: 100
        },
        w: 18.6,
        ab: "Chlorophyll"
    },
    Voltorb: {
        t1: "Electric",
        bs: {
            hp: 40,
            at: 30,
            df: 50,
            sa: 55,
            sd: 55,
            sp: 100,
            sl: 55
        },
        w: 10.4
    },
    Vulpix: {
        t1: "Fire",
        bs: {
            hp: 38,
            at: 41,
            df: 40,
            sa: 50,
            sd: 65,
            sp: 65,
            sl: 65
        },
        w: 9.9,
        ab: "Flash Fire"
    },
    Wartortle: {
        t1: "Water",
        bs: {
            hp: 59,
            at: 63,
            df: 80,
            sa: 65,
            sd: 80,
            sp: 58,
            sl: 65
        },
        w: 22.5,
        ab: "Torrent"
    },
    Weedle: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 40,
            at: 35,
            df: 30,
            sa: 20,
            sd: 20,
            sp: 50,
            sl: 20
        },
        w: 3.2
    },
    Weepinbell: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 65,
            at: 90,
            df: 50,
            sa: 85,
            sd: 45,
            sp: 55,
            sl: 85
        },
        w: 6.4,
        ab: "Chlorophyll"
    },
    Weezing: {
        t1: "Poison",
        bs: {
            hp: 65,
            at: 90,
            df: 120,
            sa: 85,
            sd: 70,
            sp: 60,
            sl: 85
        },
        w: 9.5,
        ab: "Levitate"
    },
    Wigglytuff: {
        t1: "Normal",
        bs: {
            hp: 140,
            at: 70,
            df: 45,
            sa: 75,
            sd: 50,
            sp: 45,
            sl: 50
        },
        w: 12.0
    },
    Zapdos: {
        t1: "Electric",
        t2: "Flying",
        bs: {
            hp: 90,
            at: 90,
            df: 85,
            sa: 125,
            sd: 90,
            sp: 100,
            sl: 125
        },
        w: 52.6
    },
    Zubat: {
        t1: "Poison",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 45,
            df: 35,
            sa: 30,
            sd: 40,
            sp: 55,
            sl: 40
        },
        w: 7.5
    }
};
const GEN_2 = {
    Magnemite: { t2: "Steel" },
    Magneton: { t2: "Steel" },
    Aipom: {
        t1: "Normal",
        bs: {
            hp: 55,
            at: 70,
            df: 55,
            sa: 40,
            sd: 55,
            sp: 85
        },
        w: 11.5
    },
    Ampharos: {
        t1: "Electric",
        bs: {
            hp: 90,
            at: 75,
            df: 75,
            sa: 115,
            sd: 90,
            sp: 55
        },
        w: 61.5
    },
    Ariados: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 70,
            at: 90,
            df: 70,
            sa: 60,
            sd: 60,
            sp: 40
        },
        w: 33.5
    },
    Azumarill: {
        t1: "Water",
        bs: {
            hp: 100,
            at: 50,
            df: 80,
            sa: 50,
            sd: 80,
            sp: 50
        },
        w: 28.5
    },
    Bayleef: {
        t1: "Grass",
        bs: {
            hp: 60,
            at: 62,
            df: 80,
            sa: 63,
            sd: 80,
            sp: 60
        },
        w: 15.8,
        ab: "Overgrow"
    },
    Bellossom: {
        t1: "Grass",
        bs: {
            hp: 75,
            at: 80,
            df: 85,
            sa: 90,
            sd: 100,
            sp: 50
        },
        w: 5.8,
        ab: "Chlorophyll"
    },
    Blissey: {
        t1: "Normal",
        bs: {
            hp: 255,
            at: 10,
            df: 10,
            sa: 75,
            sd: 135,
            sp: 55
        },
        w: 46.8
    },
    Celebi: {
        t1: "Psychic",
        t2: "Grass",
        bs: {
            hp: 100,
            at: 100,
            df: 100,
            sa: 100,
            sd: 100,
            sp: 100
        },
        w: 5.0,
        ab: "Natural Cure"
    },
    Chikorita: {
        t1: "Grass",
        bs: {
            hp: 45,
            at: 49,
            df: 65,
            sa: 49,
            sd: 65,
            sp: 45
        },
        w: 6.4,
        ab: "Overgrow"
    },
    Chinchou: {
        t1: "Water",
        t2: "Electric",
        bs: {
            hp: 75,
            at: 38,
            df: 38,
            sa: 56,
            sd: 56,
            sp: 67
        },
        w: 12.0
    },
    Cleffa: {
        t1: "Normal",
        bs: {
            hp: 50,
            at: 25,
            df: 28,
            sa: 45,
            sd: 55,
            sp: 15
        },
        w: 3.0
    },
    Corsola: {
        t1: "Water",
        t2: "Rock",
        bs: {
            hp: 55,
            at: 55,
            df: 85,
            sa: 65,
            sd: 85,
            sp: 35
        },
        w: 5.0
    },
    Crobat: {
        t1: "Poison",
        t2: "Flying",
        bs: {
            hp: 85,
            at: 90,
            df: 80,
            sa: 70,
            sd: 80,
            sp: 130
        },
        w: 75.0
    },
    Croconaw: {
        t1: "Water",
        bs: {
            hp: 65,
            at: 80,
            df: 80,
            sa: 59,
            sd: 63,
            sp: 58
        },
        w: 25.0,
        ab: "Torrent"
    },
    Cyndaquil: {
        t1: "Fire",
        bs: {
            hp: 39,
            at: 52,
            df: 43,
            sa: 60,
            sd: 50,
            sp: 65
        },
        w: 7.9,
        ab: "Blaze"
    },
    Delibird: {
        t1: "Ice",
        t2: "Flying",
        bs: {
            hp: 45,
            at: 55,
            df: 45,
            sa: 65,
            sd: 45,
            sp: 75
        },
        w: 16.0
    },
    Donphan: {
        t1: "Ground",
        bs: {
            hp: 90,
            at: 120,
            df: 120,
            sa: 60,
            sd: 60,
            sp: 50
        },
        w: 120.0
    },
    Dunsparce: {
        t1: "Normal",
        bs: {
            hp: 100,
            at: 70,
            df: 70,
            sa: 65,
            sd: 65,
            sp: 45
        },
        w: 14.0
    },
    Elekid: {
        t1: "Electric",
        bs: {
            hp: 45,
            at: 63,
            df: 37,
            sa: 65,
            sd: 55,
            sp: 95
        },
        w: 23.5
    },
    Entei: {
        t1: "Fire",
        bs: {
            hp: 115,
            at: 115,
            df: 85,
            sa: 90,
            sd: 75,
            sp: 100
        },
        w: 198.0
    },
    Espeon: {
        t1: "Psychic",
        bs: {
            hp: 65,
            at: 65,
            df: 60,
            sa: 130,
            sd: 95,
            sp: 110
        },
        w: 26.5
    },
    Feraligatr: {
        t1: "Water",
        bs: {
            hp: 85,
            at: 105,
            df: 100,
            sa: 79,
            sd: 83,
            sp: 78
        },
        w: 88.8,
        ab: "Torrent"
    },
    Flaaffy: {
        t1: "Electric",
        bs: {
            hp: 70,
            at: 55,
            df: 55,
            sa: 80,
            sd: 60,
            sp: 45
        },
        w: 13.3
    },
    Forretress: {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 75,
            at: 90,
            df: 140,
            sa: 60,
            sd: 60,
            sp: 40
        },
        w: 125.8
    },
    Furret: {
        t1: "Normal",
        bs: {
            hp: 85,
            at: 76,
            df: 64,
            sa: 45,
            sd: 55,
            sp: 90
        },
        w: 32.5
    },
    Girafarig: {
        t1: "Normal",
        t2: "Psychic",
        bs: {
            hp: 70,
            at: 80,
            df: 65,
            sa: 90,
            sd: 65,
            sp: 85
        },
        w: 41.5
    },
    Gligar: {
        t1: "Ground",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 75,
            df: 105,
            sa: 35,
            sd: 65,
            sp: 85
        },
        w: 64.8
    },
    Granbull: {
        t1: "Normal",
        bs: {
            hp: 90,
            at: 120,
            df: 75,
            sa: 60,
            sd: 60,
            sp: 45
        },
        w: 48.7,
        ab: "Intimidate"
    },
    Heracross: {
        t1: "Bug",
        t2: "Fighting",
        bs: {
            hp: 80,
            at: 125,
            df: 75,
            sa: 40,
            sd: 95,
            sp: 85
        },
        w: 54.0
    },
    Hitmontop: {
        t1: "Fighting",
        bs: {
            hp: 50,
            at: 95,
            df: 95,
            sa: 35,
            sd: 110,
            sp: 70
        },
        w: 48.0,
        ab: "Intimidate"
    },
    "Ho-Oh": {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 106,
            at: 130,
            df: 90,
            sa: 110,
            sd: 154,
            sp: 90
        },
        w: 199.0
    },
    Hoothoot: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 30,
            df: 30,
            sa: 36,
            sd: 56,
            sp: 50
        },
        w: 21.2
    },
    Hoppip: {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 35,
            at: 35,
            df: 40,
            sa: 35,
            sd: 55,
            sp: 50
        },
        w: 0.5,
        ab: "Chlorophyll"
    },
    Houndoom: {
        t1: "Dark",
        t2: "Fire",
        bs: {
            hp: 75,
            at: 90,
            df: 50,
            sa: 110,
            sd: 80,
            sp: 95
        },
        w: 35.0
    },
    Houndour: {
        t1: "Dark",
        t2: "Fire",
        bs: {
            hp: 45,
            at: 60,
            df: 30,
            sa: 80,
            sd: 50,
            sp: 65
        },
        w: 10.8
    },
    Igglybuff: {
        t1: "Normal",
        bs: {
            hp: 90,
            at: 30,
            df: 15,
            sa: 40,
            sd: 20,
            sp: 15
        },
        w: 1.0
    },
    Jumpluff: {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 55,
            df: 70,
            sa: 55,
            sd: 85,
            sp: 110
        },
        w: 3.0,
        ab: "Chlorophyll"
    },
    Kingdra: {
        t1: "Water",
        t2: "Dragon",
        bs: {
            hp: 75,
            at: 95,
            df: 95,
            sa: 95,
            sd: 95,
            sp: 85
        },
        w: 152.0,
        ab: "Swift Swim"
    },
    Lanturn: {
        t1: "Water",
        t2: "Electric",
        bs: {
            hp: 125,
            at: 58,
            df: 58,
            sa: 76,
            sd: 76,
            sp: 67
        },
        w: 22.5
    },
    Larvitar: {
        t1: "Rock",
        t2: "Ground",
        bs: {
            hp: 50,
            at: 64,
            df: 50,
            sa: 45,
            sd: 50,
            sp: 41
        },
        w: 72.0,
        ab: "Guts"
    },
    Ledian: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 35,
            df: 50,
            sa: 55,
            sd: 110,
            sp: 85
        },
        w: 35.6
    },
    Ledyba: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 20,
            df: 30,
            sa: 40,
            sd: 80,
            sp: 55
        },
        w: 10.8
    },
    Lugia: {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 106,
            at: 90,
            df: 130,
            sa: 90,
            sd: 154,
            sp: 110
        },
        w: 216.0
    },
    Magby: {
        t1: "Fire",
        bs: {
            hp: 45,
            at: 75,
            df: 37,
            sa: 70,
            sd: 55,
            sp: 83
        },
        w: 21.4
    },
    Magcargo: {
        t1: "Fire",
        t2: "Rock",
        bs: {
            hp: 50,
            at: 50,
            df: 120,
            sa: 80,
            sd: 80,
            sp: 30
        },
        w: 55.0
    },
    Mantine: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 40,
            df: 70,
            sa: 80,
            sd: 140,
            sp: 70
        },
        w: 220.0
    },
    Mareep: {
        t1: "Electric",
        bs: {
            hp: 55,
            at: 40,
            df: 40,
            sa: 65,
            sd: 45,
            sp: 35
        },
        w: 7.8
    },
    Marill: {
        t1: "Water",
        bs: {
            hp: 70,
            at: 20,
            df: 50,
            sa: 20,
            sd: 50,
            sp: 40
        },
        w: 8.5
    },
    Meganium: {
        t1: "Grass",
        bs: {
            hp: 80,
            at: 82,
            df: 100,
            sa: 83,
            sd: 100,
            sp: 80
        },
        w: 100.5,
        ab: "Overgrow"
    },
    Miltank: {
        t1: "Normal",
        bs: {
            hp: 95,
            at: 80,
            df: 105,
            sa: 40,
            sd: 70,
            sp: 100
        },
        w: 75.5,
        ab: "Thick Fat"
    },
    Misdreavus: {
        t1: "Ghost",
        bs: {
            hp: 60,
            at: 60,
            df: 60,
            sa: 85,
            sd: 85,
            sp: 85
        },
        w: 1.0,
        ab: "Levitate"
    },
    Murkrow: {
        t1: "Dark",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 85,
            df: 42,
            sa: 85,
            sd: 42,
            sp: 91
        },
        w: 2.1
    },
    Natu: {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 50,
            df: 45,
            sa: 70,
            sd: 45,
            sp: 70
        },
        w: 2.0
    },
    Noctowl: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 100,
            at: 50,
            df: 50,
            sa: 76,
            sd: 96,
            sp: 70
        },
        w: 40.8
    },
    Octillery: {
        t1: "Water",
        bs: {
            hp: 75,
            at: 105,
            df: 75,
            sa: 105,
            sd: 75,
            sp: 45
        },
        w: 28.5
    },
    Phanpy: {
        t1: "Ground",
        bs: {
            hp: 90,
            at: 60,
            df: 60,
            sa: 40,
            sd: 40,
            sp: 40
        },
        w: 33.5
    },
    Pichu: {
        t1: "Electric",
        bs: {
            hp: 20,
            at: 40,
            df: 15,
            sa: 35,
            sd: 35,
            sp: 60
        },
        w: 2.0
    },
    Piloswine: {
        t1: "Ice",
        t2: "Ground",
        bs: {
            hp: 100,
            at: 100,
            df: 80,
            sa: 60,
            sd: 60,
            sp: 50
        },
        w: 55.8
    },
    Pineco: {
        t1: "Bug",
        bs: {
            hp: 50,
            at: 65,
            df: 90,
            sa: 35,
            sd: 35,
            sp: 15
        },
        w: 7.2
    },
    Politoed: {
        t1: "Water",
        bs: {
            hp: 90,
            at: 75,
            df: 75,
            sa: 90,
            sd: 100,
            sp: 70
        },
        w: 33.9
    },
    Porygon2: {
        t1: "Normal",
        bs: {
            hp: 85,
            at: 80,
            df: 90,
            sa: 105,
            sd: 95,
            sp: 60
        },
        w: 32.5
    },
    Pupitar: {
        t1: "Rock",
        t2: "Ground",
        bs: {
            hp: 70,
            at: 84,
            df: 70,
            sa: 65,
            sd: 70,
            sp: 51
        },
        w: 152.0,
        ab: "Shed Skin"
    },
    Quagsire: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 95,
            at: 85,
            df: 85,
            sa: 65,
            sd: 65,
            sp: 35
        },
        w: 75.0
    },
    Quilava: {
        t1: "Fire",
        bs: {
            hp: 58,
            at: 64,
            df: 58,
            sa: 80,
            sd: 65,
            sp: 80
        },
        w: 19.0,
        ab: "Blaze"
    },
    Qwilfish: {
        t1: "Water",
        t2: "Poison",
        bs: {
            hp: 65,
            at: 95,
            df: 75,
            sa: 55,
            sd: 55,
            sp: 85
        },
        w: 3.9
    },
    Raikou: {
        t1: "Electric",
        bs: {
            hp: 90,
            at: 85,
            df: 75,
            sa: 115,
            sd: 100,
            sp: 115
        },
        w: 178.0
    },
    Remoraid: {
        t1: "Water",
        bs: {
            hp: 35,
            at: 65,
            df: 35,
            sa: 65,
            sd: 35,
            sp: 65
        },
        w: 12.0
    },
    Scizor: {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 70,
            at: 130,
            df: 100,
            sa: 55,
            sd: 80,
            sp: 65
        },
        w: 118.0,
        ab: "Swarm"
    },
    Sentret: {
        t1: "Normal",
        bs: {
            hp: 35,
            at: 46,
            df: 34,
            sa: 35,
            sd: 45,
            sp: 20
        },
        w: 6.0
    },
    Shuckle: {
        t1: "Bug",
        t2: "Rock",
        bs: {
            hp: 20,
            at: 10,
            df: 230,
            sa: 10,
            sd: 230,
            sp: 5
        },
        w: 20.5
    },
    Skarmory: {
        t1: "Steel",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 80,
            df: 140,
            sa: 40,
            sd: 70,
            sp: 70
        },
        w: 50.5
    },
    Skiploom: {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 45,
            df: 50,
            sa: 45,
            sd: 65,
            sp: 80
        },
        w: 1.0,
        ab: "Chlorophyll"
    },
    Slowking: {
        t1: "Water",
        t2: "Psychic",
        bs: {
            hp: 95,
            at: 75,
            df: 80,
            sa: 100,
            sd: 110,
            sp: 30
        },
        w: 79.5
    },
    Slugma: {
        t1: "Fire",
        bs: {
            hp: 40,
            at: 40,
            df: 40,
            sa: 70,
            sd: 40,
            sp: 20
        },
        w: 35.0
    },
    Smeargle: {
        t1: "Normal",
        bs: {
            hp: 55,
            at: 20,
            df: 35,
            sa: 20,
            sd: 45,
            sp: 75
        },
        w: 58.0
    },
    Smoochum: {
        t1: "Ice",
        t2: "Psychic",
        bs: {
            hp: 45,
            at: 30,
            df: 15,
            sa: 85,
            sd: 65,
            sp: 65
        },
        w: 6.0
    },
    Sneasel: {
        t1: "Dark",
        t2: "Ice",
        bs: {
            hp: 55,
            at: 95,
            df: 55,
            sa: 35,
            sd: 75,
            sp: 115
        },
        w: 28.0
    },
    Snubbull: {
        t1: "Normal",
        bs: {
            hp: 60,
            at: 80,
            df: 50,
            sa: 40,
            sd: 40,
            sp: 30
        },
        w: 7.8
    },
    Spinarak: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 40,
            at: 60,
            df: 40,
            sa: 40,
            sd: 40,
            sp: 30
        },
        w: 8.5
    },
    Stantler: {
        t1: "Normal",
        bs: {
            hp: 73,
            at: 95,
            df: 62,
            sa: 85,
            sd: 65,
            sp: 85
        },
        w: 71.2,
        ab: "Intimidate"
    },
    Steelix: {
        t1: "Steel",
        t2: "Ground",
        bs: {
            hp: 75,
            at: 85,
            df: 200,
            sa: 55,
            sd: 65,
            sp: 30
        },
        w: 400.0
    },
    Sudowoodo: {
        t1: "Rock",
        bs: {
            hp: 70,
            at: 100,
            df: 115,
            sa: 30,
            sd: 65,
            sp: 30
        },
        w: 38.0
    },
    Suicune: {
        t1: "Water",
        bs: {
            hp: 100,
            at: 75,
            df: 115,
            sa: 90,
            sd: 115,
            sp: 85
        },
        w: 187.0
    },
    Sunflora: {
        t1: "Grass",
        bs: {
            hp: 75,
            at: 75,
            df: 55,
            sa: 105,
            sd: 85,
            sp: 30
        },
        w: 8.5,
        ab: "Chlorophyll"
    },
    Sunkern: {
        t1: "Grass",
        bs: {
            hp: 30,
            at: 30,
            df: 30,
            sa: 30,
            sd: 30,
            sp: 30
        },
        w: 1.8,
        ab: "Chlorophyll"
    },
    Swinub: {
        t1: "Ice",
        t2: "Ground",
        bs: {
            hp: 50,
            at: 50,
            df: 40,
            sa: 30,
            sd: 30,
            sp: 50
        },
        w: 6.5
    },
    Teddiursa: {
        t1: "Normal",
        bs: {
            hp: 60,
            at: 80,
            df: 50,
            sa: 50,
            sd: 50,
            sp: 40
        },
        w: 8.8
    },
    Togepi: {
        t1: "Normal",
        bs: {
            hp: 35,
            at: 20,
            df: 65,
            sa: 40,
            sd: 65,
            sp: 20
        },
        w: 1.5
    },
    Togetic: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 40,
            df: 85,
            sa: 80,
            sd: 105,
            sp: 40
        },
        w: 3.2
    },
    Totodile: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 65,
            df: 64,
            sa: 44,
            sd: 48,
            sp: 43
        },
        w: 9.5,
        ab: "Torrent"
    },
    Typhlosion: {
        t1: "Fire",
        bs: {
            hp: 78,
            at: 84,
            df: 78,
            sa: 109,
            sd: 85,
            sp: 100
        },
        w: 79.5,
        ab: "Blaze"
    },
    Tyranitar: {
        t1: "Rock",
        t2: "Dark",
        bs: {
            hp: 100,
            at: 134,
            df: 110,
            sa: 95,
            sd: 100,
            sp: 61
        },
        w: 202.0,
        ab: "Sand Stream"
    },
    Tyrogue: {
        t1: "Fighting",
        bs: {
            hp: 35,
            at: 35,
            df: 35,
            sa: 35,
            sd: 35,
            sp: 35
        },
        w: 21.0,
        ab: "Guts"
    },
    Umbreon: {
        t1: "Dark",
        bs: {
            hp: 95,
            at: 65,
            df: 110,
            sa: 60,
            sd: 130,
            sp: 65
        },
        w: 27.0
    },
    Unown: {
        t1: "Psychic",
        bs: {
            hp: 48,
            at: 72,
            df: 48,
            sa: 72,
            sd: 48,
            sp: 48
        },
        w: 5.0,
        ab: "Levitate"
    },
    Ursaring: {
        t1: "Normal",
        bs: {
            hp: 90,
            at: 130,
            df: 75,
            sa: 75,
            sd: 75,
            sp: 55
        },
        w: 125.8,
        ab: "Guts"
    },
    Wobbuffet: {
        t1: "Psychic",
        bs: {
            hp: 190,
            at: 33,
            df: 58,
            sa: 33,
            sd: 58,
            sp: 33
        },
        w: 28.5
    },
    Wooper: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 55,
            at: 45,
            df: 45,
            sa: 25,
            sd: 25,
            sp: 15
        },
        w: 8.5
    },
    Xatu: {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 75,
            df: 70,
            sa: 95,
            sd: 70,
            sp: 95
        },
        w: 15.0
    },
    Yanma: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 65,
            df: 45,
            sa: 75,
            sd: 45,
            sp: 95
        },
        w: 38.0
    }
};
const GEN_3 = {
    Absol: {
        t1: "Dark",
        bs: {
            hp: 65,
            at: 130,
            df: 60,
            sa: 75,
            sd: 60,
            sp: 75
        },
        w: 47.0
    },
    Aggron: {
        t1: "Steel",
        t2: "Rock",
        bs: {
            hp: 70,
            at: 110,
            df: 180,
            sa: 60,
            sd: 60,
            sp: 50
        },
        w: 360.0
    },
    Altaria: {
        t1: "Dragon",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 70,
            df: 90,
            sa: 70,
            sd: 105,
            sp: 80
        },
        w: 20.6
    },
    Anorith: {
        t1: "Rock",
        t2: "Bug",
        bs: {
            hp: 45,
            at: 95,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 75
        },
        w: 12.5,
        ab: "Battle Armor"
    },
    Armaldo: {
        t1: "Rock",
        t2: "Bug",
        bs: {
            hp: 75,
            at: 125,
            df: 100,
            sa: 70,
            sd: 80,
            sp: 45
        },
        w: 68.2,
        ab: "Battle Armor"
    },
    Aron: {
        t1: "Steel",
        t2: "Rock",
        bs: {
            hp: 50,
            at: 70,
            df: 100,
            sa: 40,
            sd: 40,
            sp: 30
        },
        w: 60.0
    },
    Azurill: {
        t1: "Normal",
        bs: {
            hp: 50,
            at: 20,
            df: 40,
            sa: 20,
            sd: 40,
            sp: 20
        },
        w: 2.0
    },
    Bagon: {
        t1: "Dragon",
        bs: {
            hp: 45,
            at: 75,
            df: 60,
            sa: 40,
            sd: 30,
            sp: 50
        },
        w: 42.1
    },
    Baltoy: {
        t1: "Ground",
        t2: "Psychic",
        bs: {
            hp: 40,
            at: 40,
            df: 55,
            sa: 40,
            sd: 70,
            sp: 55
        },
        w: 21.5,
        ab: "Levitate"
    },
    Banette: {
        t1: "Ghost",
        bs: {
            hp: 64,
            at: 115,
            df: 65,
            sa: 83,
            sd: 63,
            sp: 65
        },
        w: 12.5
    },
    Barboach: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 50,
            at: 48,
            df: 43,
            sa: 46,
            sd: 41,
            sp: 60
        },
        w: 1.9
    },
    Beautifly: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 70,
            df: 50,
            sa: 90,
            sd: 50,
            sp: 65
        },
        w: 28.4,
        ab: "Swarm"
    },
    Beldum: {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 40,
            at: 55,
            df: 80,
            sa: 35,
            sd: 60,
            sp: 30
        },
        w: 95.2,
        ab: "Clear Body"
    },
    Blaziken: {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 80,
            at: 120,
            df: 70,
            sa: 110,
            sd: 70,
            sp: 80
        },
        w: 52.0,
        ab: "Blaze"
    },
    Breloom: {
        t1: "Grass",
        t2: "Fighting",
        bs: {
            hp: 60,
            at: 130,
            df: 80,
            sa: 60,
            sd: 60,
            sp: 70
        },
        w: 39.2
    },
    Cacnea: {
        t1: "Grass",
        bs: {
            hp: 50,
            at: 85,
            df: 40,
            sa: 85,
            sd: 40,
            sp: 35
        },
        w: 51.3,
        ab: "Sand Veil"
    },
    Cacturne: {
        t1: "Grass",
        t2: "Dark",
        bs: {
            hp: 70,
            at: 115,
            df: 60,
            sa: 115,
            sd: 60,
            sp: 55
        },
        w: 77.4,
        ab: "Sand Veil"
    },
    Camerupt: {
        t1: "Fire",
        t2: "Ground",
        bs: {
            hp: 70,
            at: 100,
            df: 70,
            sa: 105,
            sd: 75,
            sp: 40
        },
        w: 220.0
    },
    Carvanha: {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 45,
            at: 90,
            df: 20,
            sa: 65,
            sd: 20,
            sp: 65
        },
        w: 20.8
    },
    Cascoon: {
        t1: "Bug",
        bs: {
            hp: 50,
            at: 35,
            df: 55,
            sa: 25,
            sd: 25,
            sp: 15
        },
        w: 11.5,
        ab: "Shed Skin"
    },
    Castform: {
        t1: "Normal",
        bs: {
            hp: 70,
            at: 70,
            df: 70,
            sa: 70,
            sd: 70,
            sp: 70
        },
        w: 0.8,
        ab: "Forecast"
    },
    Chimecho: {
        t1: "Psychic",
        bs: {
            hp: 65,
            at: 50,
            df: 70,
            sa: 95,
            sd: 80,
            sp: 65
        },
        w: 1.0,
        ab: "Levitate"
    },
    Clamperl: {
        t1: "Water",
        bs: {
            hp: 35,
            at: 64,
            df: 85,
            sa: 74,
            sd: 55,
            sp: 32
        },
        w: 52.5,
        ab: "Shell Armor"
    },
    Claydol: {
        t1: "Ground",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 70,
            df: 105,
            sa: 70,
            sd: 120,
            sp: 75
        },
        w: 108.0,
        ab: "Levitate"
    },
    Combusken: {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 60,
            at: 85,
            df: 60,
            sa: 85,
            sd: 60,
            sp: 55
        },
        w: 19.5,
        ab: "Blaze"
    },
    Corphish: {
        t1: "Water",
        bs: {
            hp: 43,
            at: 80,
            df: 65,
            sa: 50,
            sd: 35,
            sp: 35
        },
        w: 11.5
    },
    Cradily: {
        t1: "Rock",
        t2: "Grass",
        bs: {
            hp: 86,
            at: 81,
            df: 97,
            sa: 81,
            sd: 107,
            sp: 43
        },
        w: 60.4
    },
    Crawdaunt: {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 63,
            at: 120,
            df: 85,
            sa: 90,
            sd: 55,
            sp: 55
        },
        w: 32.8
    },
    Delcatty: {
        t1: "Normal",
        bs: {
            hp: 70,
            at: 65,
            df: 65,
            sa: 55,
            sd: 55,
            sp: 70
        },
        w: 32.6
    },
    Deoxys: {
        t1: "Psychic",
        bs: {
            hp: 50,
            at: 150,
            df: 50,
            sa: 150,
            sd: 50,
            sp: 150
        },
        w: 60.8,
        ab: "Pressure"
    },
    "Deoxys-A": {
        t1: "Psychic",
        bs: {
            hp: 50,
            at: 180,
            df: 20,
            sa: 180,
            sd: 20,
            sp: 150
        },
        w: 60.8,
        ab: "Pressure"
    },
    "Deoxys-D": {
        t1: "Psychic",
        bs: {
            hp: 50,
            at: 70,
            df: 160,
            sa: 70,
            sd: 160,
            sp: 90
        },
        w: 60.8,
        ab: "Pressure"
    },
    "Deoxys-S": {
        t1: "Psychic",
        bs: {
            hp: 50,
            at: 95,
            df: 90,
            sa: 95,
            sd: 90,
            sp: 180
        },
        w: 60.8,
        ab: "Pressure"
    },
    Dusclops: {
        t1: "Ghost",
        bs: {
            hp: 40,
            at: 70,
            df: 130,
            sa: 60,
            sd: 130,
            sp: 25
        },
        w: 30.6
    },
    Duskull: {
        t1: "Ghost",
        bs: {
            hp: 20,
            at: 40,
            df: 90,
            sa: 30,
            sd: 90,
            sp: 25
        },
        w: 15.0,
        ab: "Levitate"
    },
    Dustox: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 50,
            df: 70,
            sa: 50,
            sd: 90,
            sp: 65
        },
        w: 31.6
    },
    Electrike: {
        t1: "Electric",
        bs: {
            hp: 40,
            at: 45,
            df: 40,
            sa: 65,
            sd: 40,
            sp: 65
        },
        w: 15.2
    },
    Exploud: {
        t1: "Normal",
        bs: {
            hp: 104,
            at: 91,
            df: 63,
            sa: 91,
            sd: 63,
            sp: 68
        },
        w: 84.0,
        ab: "Soundproof"
    },
    Feebas: {
        t1: "Water",
        bs: {
            hp: 20,
            at: 15,
            df: 20,
            sa: 10,
            sd: 55,
            sp: 80
        },
        w: 7.4,
        ab: "Swift Swim"
    },
    Flygon: {
        t1: "Ground",
        t2: "Dragon",
        bs: {
            hp: 80,
            at: 100,
            df: 80,
            sa: 80,
            sd: 80,
            sp: 100
        },
        w: 82.0,
        ab: "Levitate"
    },
    Gardevoir: {
        t1: "Psychic",
        bs: {
            hp: 68,
            at: 65,
            df: 65,
            sa: 125,
            sd: 115,
            sp: 80
        },
        w: 48.4
    },
    Glalie: {
        t1: "Ice",
        bs: {
            hp: 80,
            at: 80,
            df: 80,
            sa: 80,
            sd: 80,
            sp: 80
        },
        w: 256.5
    },
    Gorebyss: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 84,
            df: 105,
            sa: 114,
            sd: 75,
            sp: 52
        },
        w: 22.6,
        ab: "Swift Swim"
    },
    Groudon: {
        t1: "Ground",
        bs: {
            hp: 100,
            at: 150,
            df: 140,
            sa: 100,
            sd: 90,
            sp: 90
        },
        w: 950.0,
        ab: "Drought"
    },
    Grovyle: {
        t1: "Grass",
        bs: {
            hp: 50,
            at: 65,
            df: 45,
            sa: 85,
            sd: 65,
            sp: 95
        },
        w: 21.6,
        ab: "Overgrow"
    },
    Grumpig: {
        t1: "Psychic",
        bs: {
            hp: 80,
            at: 45,
            df: 65,
            sa: 90,
            sd: 110,
            sp: 80
        },
        w: 71.5
    },
    Gulpin: {
        t1: "Poison",
        bs: {
            hp: 70,
            at: 43,
            df: 53,
            sa: 43,
            sd: 53,
            sp: 40
        },
        w: 10.3
    },
    Hariyama: {
        t1: "Fighting",
        bs: {
            hp: 144,
            at: 120,
            df: 60,
            sa: 40,
            sd: 60,
            sp: 50
        },
        w: 253.8
    },
    Huntail: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 104,
            df: 105,
            sa: 94,
            sd: 75,
            sp: 52
        },
        w: 27.0,
        ab: "Swift Swim"
    },
    Illumise: {
        t1: "Bug",
        bs: {
            hp: 65,
            at: 47,
            df: 55,
            sa: 73,
            sd: 75,
            sp: 85
        },
        w: 17.7
    },
    Jirachi: {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 100,
            at: 100,
            df: 100,
            sa: 100,
            sd: 100,
            sp: 100
        },
        w: 1.1,
        ab: "Serene Grace"
    },
    Kecleon: {
        t1: "Normal",
        bs: {
            hp: 60,
            at: 90,
            df: 70,
            sa: 60,
            sd: 120,
            sp: 40
        },
        w: 22.0
    },
    Kirlia: {
        t1: "Psychic",
        bs: {
            hp: 38,
            at: 35,
            df: 35,
            sa: 65,
            sd: 55,
            sp: 50
        },
        w: 20.2
    },
    Kyogre: {
        t1: "Water",
        bs: {
            hp: 100,
            at: 100,
            df: 90,
            sa: 150,
            sd: 140,
            sp: 90
        },
        w: 352.0,
        ab: "Drizzle"
    },
    Lairon: {
        t1: "Steel",
        t2: "Rock",
        bs: {
            hp: 60,
            at: 90,
            df: 140,
            sa: 50,
            sd: 50,
            sp: 40
        },
        w: 120.0
    },
    Latias: {
        t1: "Dragon",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 80,
            df: 90,
            sa: 110,
            sd: 130,
            sp: 110
        },
        w: 40.0,
        ab: "Levitate"
    },
    Latios: {
        t1: "Dragon",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 90,
            df: 80,
            sa: 130,
            sd: 110,
            sp: 110
        },
        w: 60.0,
        ab: "Levitate"
    },
    Lileep: {
        t1: "Rock",
        t2: "Grass",
        bs: {
            hp: 66,
            at: 41,
            df: 77,
            sa: 61,
            sd: 87,
            sp: 23
        },
        w: 23.8
    },
    Linoone: {
        t1: "Normal",
        bs: {
            hp: 78,
            at: 70,
            df: 61,
            sa: 50,
            sd: 61,
            sp: 100
        },
        w: 32.5
    },
    Lombre: {
        t1: "Water",
        t2: "Grass",
        bs: {
            hp: 60,
            at: 50,
            df: 50,
            sa: 60,
            sd: 70,
            sp: 50
        },
        w: 32.5
    },
    Lotad: {
        t1: "Water",
        t2: "Grass",
        bs: {
            hp: 40,
            at: 30,
            df: 30,
            sa: 40,
            sd: 50,
            sp: 30
        },
        w: 2.6
    },
    Loudred: {
        t1: "Normal",
        bs: {
            hp: 84,
            at: 71,
            df: 43,
            sa: 71,
            sd: 43,
            sp: 48
        },
        w: 40.5,
        ab: "Soundproof"
    },
    Ludicolo: {
        t1: "Water",
        t2: "Grass",
        bs: {
            hp: 80,
            at: 70,
            df: 70,
            sa: 90,
            sd: 100,
            sp: 70
        },
        w: 55.0
    },
    Lunatone: {
        t1: "Rock",
        t2: "Psychic",
        bs: {
            hp: 70,
            at: 55,
            df: 65,
            sa: 95,
            sd: 85,
            sp: 70
        },
        w: 168.0,
        ab: "Levitate"
    },
    Luvdisc: {
        t1: "Water",
        bs: {
            hp: 43,
            at: 30,
            df: 55,
            sa: 40,
            sd: 65,
            sp: 97
        },
        w: 8.7,
        ab: "Swift Swim"
    },
    Makuhita: {
        t1: "Fighting",
        bs: {
            hp: 72,
            at: 60,
            df: 30,
            sa: 20,
            sd: 30,
            sp: 25
        },
        w: 86.4
    },
    Manectric: {
        t1: "Electric",
        bs: {
            hp: 70,
            at: 75,
            df: 60,
            sa: 105,
            sd: 60,
            sp: 105
        },
        w: 40.2
    },
    Marshtomp: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 70,
            at: 85,
            df: 70,
            sa: 60,
            sd: 70,
            sp: 50
        },
        w: 28.0,
        ab: "Torrent"
    },
    Masquerain: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 70,
            at: 60,
            df: 62,
            sa: 80,
            sd: 82,
            sp: 60
        },
        w: 3.6,
        ab: "Intimidate"
    },
    Mawile: {
        t1: "Steel",
        bs: {
            hp: 50,
            at: 85,
            df: 85,
            sa: 55,
            sd: 55,
            sp: 50
        },
        w: 11.5
    },
    Medicham: {
        t1: "Fighting",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 60,
            df: 75,
            sa: 60,
            sd: 75,
            sp: 80
        },
        w: 31.5,
        ab: "Pure Power"
    },
    Meditite: {
        t1: "Fighting",
        t2: "Psychic",
        bs: {
            hp: 30,
            at: 40,
            df: 55,
            sa: 40,
            sd: 55,
            sp: 60
        },
        w: 11.2,
        ab: "Pure Power"
    },
    Metagross: {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 135,
            df: 130,
            sa: 95,
            sd: 90,
            sp: 70
        },
        w: 550.0,
        ab: "Clear Body"
    },
    Metang: {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 75,
            df: 100,
            sa: 55,
            sd: 80,
            sp: 50
        },
        w: 202.5,
        ab: "Clear Body"
    },
    Mightyena: {
        t1: "Dark",
        bs: {
            hp: 70,
            at: 90,
            df: 70,
            sa: 60,
            sd: 60,
            sp: 70
        },
        w: 37.0,
        ab: "Intimidate"
    },
    Milotic: {
        t1: "Water",
        bs: {
            hp: 95,
            at: 60,
            df: 79,
            sa: 100,
            sd: 125,
            sp: 81
        },
        w: 162.0
    },
    Minun: {
        t1: "Electric",
        bs: {
            hp: 60,
            at: 40,
            df: 50,
            sa: 75,
            sd: 85,
            sp: 95
        },
        w: 4.2
    },
    Mudkip: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 70,
            df: 50,
            sa: 50,
            sd: 50,
            sp: 40
        },
        w: 7.6,
        ab: "Torrent"
    },
    Nincada: {
        t1: "Bug",
        t2: "Ground",
        bs: {
            hp: 31,
            at: 45,
            df: 90,
            sa: 30,
            sd: 30,
            sp: 40
        },
        w: 5.5
    },
    Ninjask: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 61,
            at: 90,
            df: 45,
            sa: 50,
            sd: 50,
            sp: 160
        },
        w: 12.0
    },
    Nosepass: {
        t1: "Rock",
        bs: {
            hp: 30,
            at: 45,
            df: 135,
            sa: 45,
            sd: 90,
            sp: 30
        },
        w: 97.0
    },
    Numel: {
        t1: "Fire",
        t2: "Ground",
        bs: {
            hp: 60,
            at: 60,
            df: 40,
            sa: 65,
            sd: 45,
            sp: 35
        },
        w: 24.0
    },
    Nuzleaf: {
        t1: "Grass",
        t2: "Dark",
        bs: {
            hp: 70,
            at: 70,
            df: 40,
            sa: 60,
            sd: 40,
            sp: 60
        },
        w: 28.0
    },
    Pelipper: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 50,
            df: 100,
            sa: 85,
            sd: 70,
            sp: 65
        },
        w: 28.0
    },
    Plusle: {
        t1: "Electric",
        bs: {
            hp: 60,
            at: 50,
            df: 40,
            sa: 85,
            sd: 75,
            sp: 95
        },
        w: 4.2
    },
    Poochyena: {
        t1: "Dark",
        bs: {
            hp: 35,
            at: 55,
            df: 35,
            sa: 30,
            sd: 30,
            sp: 35
        },
        w: 13.6
    },
    Ralts: {
        t1: "Psychic",
        bs: {
            hp: 28,
            at: 25,
            df: 25,
            sa: 45,
            sd: 35,
            sp: 40
        },
        w: 6.6
    },
    Rayquaza: {
        t1: "Dragon",
        t2: "Flying",
        bs: {
            hp: 105,
            at: 150,
            df: 90,
            sa: 150,
            sd: 90,
            sp: 95
        },
        w: 206.5,
        ab: "Air Lock"
    },
    Regice: {
        t1: "Ice",
        bs: {
            hp: 80,
            at: 50,
            df: 100,
            sa: 100,
            sd: 200,
            sp: 50
        },
        w: 175.0,
        ab: "Clear Body"
    },
    Regirock: {
        t1: "Rock",
        bs: {
            hp: 80,
            at: 100,
            df: 200,
            sa: 50,
            sd: 100,
            sp: 50
        },
        w: 230.0,
        ab: "Clear Body"
    },
    Registeel: {
        t1: "Steel",
        bs: {
            hp: 80,
            at: 75,
            df: 150,
            sa: 75,
            sd: 150,
            sp: 50
        },
        w: 205.0,
        ab: "Clear Body"
    },
    Relicanth: {
        t1: "Water",
        t2: "Rock",
        bs: {
            hp: 100,
            at: 90,
            df: 130,
            sa: 45,
            sd: 65,
            sp: 55
        },
        w: 23.4
    },
    Roselia: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 50,
            at: 60,
            df: 45,
            sa: 100,
            sd: 80,
            sp: 65
        },
        w: 2.0
    },
    Sableye: {
        t1: "Dark",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 75,
            df: 75,
            sa: 65,
            sd: 65,
            sp: 50
        },
        w: 11.0
    },
    Salamence: {
        t1: "Dragon",
        t2: "Flying",
        bs: {
            hp: 95,
            at: 135,
            df: 80,
            sa: 110,
            sd: 80,
            sp: 100
        },
        w: 102.6,
        ab: "Intimidate"
    },
    Sceptile: {
        t1: "Grass",
        bs: {
            hp: 70,
            at: 85,
            df: 65,
            sa: 105,
            sd: 85,
            sp: 120
        },
        w: 52.2,
        ab: "Overgrow"
    },
    Sealeo: {
        t1: "Ice",
        t2: "Water",
        bs: {
            hp: 90,
            at: 60,
            df: 70,
            sa: 75,
            sd: 70,
            sp: 45
        },
        w: 87.6,
        ab: "Thick Fat"
    },
    Seedot: {
        t1: "Grass",
        bs: {
            hp: 40,
            at: 40,
            df: 50,
            sa: 30,
            sd: 30,
            sp: 30
        },
        w: 4.0
    },
    Seviper: {
        t1: "Poison",
        bs: {
            hp: 73,
            at: 100,
            df: 60,
            sa: 100,
            sd: 60,
            sp: 65
        },
        w: 52.5
    },
    Sharpedo: {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 70,
            at: 120,
            df: 40,
            sa: 95,
            sd: 40,
            sp: 95
        },
        w: 88.8
    },
    Shedinja: {
        t1: "Bug",
        t2: "Ghost",
        bs: {
            hp: 1,
            at: 90,
            df: 45,
            sa: 30,
            sd: 30,
            sp: 40
        },
        w: 1.2,
        ab: "Wonder Guard"
    },
    Shelgon: {
        t1: "Dragon",
        bs: {
            hp: 65,
            at: 95,
            df: 100,
            sa: 60,
            sd: 50,
            sp: 50
        },
        w: 110.5
    },
    Shiftry: {
        t1: "Grass",
        t2: "Dark",
        bs: {
            hp: 90,
            at: 100,
            df: 60,
            sa: 90,
            sd: 60,
            sp: 80
        },
        w: 59.6
    },
    Shroomish: {
        t1: "Grass",
        bs: {
            hp: 60,
            at: 40,
            df: 60,
            sa: 40,
            sd: 60,
            sp: 35
        },
        w: 4.5
    },
    Shuppet: {
        t1: "Ghost",
        bs: {
            hp: 44,
            at: 75,
            df: 35,
            sa: 63,
            sd: 33,
            sp: 45
        },
        w: 2.3
    },
    Silcoon: {
        t1: "Bug",
        bs: {
            hp: 50,
            at: 35,
            df: 55,
            sa: 25,
            sd: 25,
            sp: 15
        },
        w: 10.0,
        ab: "Shed Skin"
    },
    Skitty: {
        t1: "Normal",
        bs: {
            hp: 50,
            at: 45,
            df: 45,
            sa: 35,
            sd: 35,
            sp: 50
        },
        w: 11.0
    },
    Slaking: {
        t1: "Normal",
        bs: {
            hp: 150,
            at: 160,
            df: 100,
            sa: 95,
            sd: 65,
            sp: 100
        },
        w: 130.5,
        ab: "Truant"
    },
    Slakoth: {
        t1: "Normal",
        bs: {
            hp: 60,
            at: 60,
            df: 60,
            sa: 35,
            sd: 35,
            sp: 30
        },
        w: 24.0,
        ab: "Truant"
    },
    Snorunt: {
        t1: "Ice",
        bs: {
            hp: 50,
            at: 50,
            df: 50,
            sa: 50,
            sd: 50,
            sp: 50
        },
        w: 16.8
    },
    Solrock: {
        t1: "Rock",
        t2: "Psychic",
        bs: {
            hp: 70,
            at: 95,
            df: 85,
            sa: 55,
            sd: 65,
            sp: 70
        },
        w: 154.0,
        ab: "Levitate"
    },
    Spheal: {
        t1: "Ice",
        t2: "Water",
        bs: {
            hp: 70,
            at: 40,
            df: 50,
            sa: 55,
            sd: 50,
            sp: 25
        },
        w: 39.5,
        ab: "Thick Fat"
    },
    Spinda: {
        t1: "Normal",
        bs: {
            hp: 60,
            at: 60,
            df: 60,
            sa: 60,
            sd: 60,
            sp: 60
        },
        w: 5.0
    },
    Spoink: {
        t1: "Psychic",
        bs: {
            hp: 60,
            at: 25,
            df: 35,
            sa: 70,
            sd: 80,
            sp: 60
        },
        w: 30.6
    },
    Surskit: {
        t1: "Bug",
        t2: "Water",
        bs: {
            hp: 40,
            at: 30,
            df: 32,
            sa: 50,
            sd: 52,
            sp: 65
        },
        w: 1.7,
        ab: "Swift Swim"
    },
    Swablu: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 45,
            at: 40,
            df: 60,
            sa: 40,
            sd: 75,
            sp: 50
        },
        w: 1.2
    },
    Swalot: {
        t1: "Poison",
        bs: {
            hp: 100,
            at: 73,
            df: 83,
            sa: 73,
            sd: 83,
            sp: 55
        },
        w: 80.0
    },
    Swampert: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 100,
            at: 110,
            df: 90,
            sa: 85,
            sd: 90,
            sp: 60
        },
        w: 81.9,
        ab: "Torrent"
    },
    Swellow: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 85,
            df: 60,
            sa: 50,
            sd: 50,
            sp: 125
        },
        w: 19.8,
        ab: "Guts"
    },
    Taillow: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 55,
            df: 30,
            sa: 30,
            sd: 30,
            sp: 85
        },
        w: 2.3,
        ab: "Guts"
    },
    Torchic: {
        t1: "Fire",
        bs: {
            hp: 45,
            at: 60,
            df: 40,
            sa: 70,
            sd: 50,
            sp: 45
        },
        w: 2.5,
        ab: "Blaze"
    },
    Torkoal: {
        t1: "Fire",
        bs: {
            hp: 70,
            at: 85,
            df: 140,
            sa: 85,
            sd: 70,
            sp: 20
        },
        w: 80.4,
        ab: "White Smoke"
    },
    Trapinch: {
        t1: "Ground",
        bs: {
            hp: 45,
            at: 100,
            df: 45,
            sa: 45,
            sd: 45,
            sp: 10
        },
        w: 15.0
    },
    Treecko: {
        t1: "Grass",
        bs: {
            hp: 40,
            at: 45,
            df: 35,
            sa: 65,
            sd: 55,
            sp: 70
        },
        w: 5.0,
        ab: "Overgrow"
    },
    Tropius: {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 99,
            at: 68,
            df: 83,
            sa: 72,
            sd: 87,
            sp: 51
        },
        w: 100.0,
        ab: "Chlorophyll"
    },
    Vibrava: {
        t1: "Ground",
        t2: "Dragon",
        bs: {
            hp: 50,
            at: 70,
            df: 50,
            sa: 50,
            sd: 50,
            sp: 70
        },
        w: 15.3,
        ab: "Levitate"
    },
    Vigoroth: {
        t1: "Normal",
        bs: {
            hp: 80,
            at: 80,
            df: 80,
            sa: 55,
            sd: 55,
            sp: 90
        },
        w: 46.5,
        ab: "Vital Spirit"
    },
    Volbeat: {
        t1: "Bug",
        bs: {
            hp: 65,
            at: 73,
            df: 55,
            sa: 47,
            sd: 75,
            sp: 85
        },
        w: 17.7
    },
    Wailmer: {
        t1: "Water",
        bs: {
            hp: 130,
            at: 70,
            df: 35,
            sa: 70,
            sd: 35,
            sp: 60
        },
        w: 130.0
    },
    Wailord: {
        t1: "Water",
        bs: {
            hp: 170,
            at: 90,
            df: 45,
            sa: 90,
            sd: 45,
            sp: 60
        },
        w: 398.0
    },
    Walrein: {
        t1: "Ice",
        t2: "Water",
        bs: {
            hp: 110,
            at: 80,
            df: 90,
            sa: 95,
            sd: 90,
            sp: 65
        },
        w: 150.6,
        ab: "Thick Fat"
    },
    Whiscash: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 110,
            at: 78,
            df: 73,
            sa: 76,
            sd: 71,
            sp: 60
        },
        w: 23.6
    },
    Whismur: {
        t1: "Normal",
        bs: {
            hp: 64,
            at: 51,
            df: 23,
            sa: 51,
            sd: 23,
            sp: 28
        },
        w: 16.3,
        ab: "Soundproof"
    },
    Wingull: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 30,
            df: 30,
            sa: 55,
            sd: 30,
            sp: 85
        },
        w: 9.5
    },
    Wurmple: {
        t1: "Bug",
        bs: {
            hp: 45,
            at: 45,
            df: 35,
            sa: 20,
            sd: 30,
            sp: 20
        },
        w: 3.6
    },
    Wynaut: {
        t1: "Psychic",
        bs: {
            hp: 95,
            at: 23,
            df: 48,
            sa: 23,
            sd: 48,
            sp: 23
        },
        w: 14.0
    },
    Zangoose: {
        t1: "Normal",
        bs: {
            hp: 73,
            at: 115,
            df: 60,
            sa: 60,
            sd: 60,
            sp: 90
        },
        w: 40.3
    },
    Zigzagoon: {
        t1: "Normal",
        bs: {
            hp: 38,
            at: 30,
            df: 41,
            sa: 30,
            sd: 41,
            sp: 60
        },
        w: 17.5
    }
};
const GEN_4 = {
    Abomasnow: {
        t1: "Grass",
        t2: "Ice",
        bs: {
            hp: 90,
            at: 92,
            df: 75,
            sa: 92,
            sd: 85,
            sp: 60
        },
        w: 135.5,
        ab: "Snow Warning"
    },
    Ambipom: {
        t1: "Normal",
        bs: {
            hp: 75,
            at: 100,
            df: 66,
            sa: 60,
            sd: 66,
            sp: 115
        },
        w: 20.3
    },
    Arceus: {
        t1: "Normal",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Bug": {
        t1: "Bug",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Dark": {
        t1: "Dark",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Dragon": {
        t1: "Dragon",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Electric": {
        t1: "Electric",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Fighting": {
        t1: "Fighting",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Fire": {
        t1: "Fire",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Flying": {
        t1: "Flying",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Ghost": {
        t1: "Ghost",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Grass": {
        t1: "Grass",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Ground": {
        t1: "Ground",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Ice": {
        t1: "Ice",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Poison": {
        t1: "Poison",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Psychic": {
        t1: "Psychic",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Rock": {
        t1: "Rock",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Steel": {
        t1: "Steel",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    "Arceus-Water": {
        t1: "Water",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    Azelf: {
        t1: "Psychic",
        bs: {
            hp: 75,
            at: 125,
            df: 70,
            sa: 125,
            sd: 70,
            sp: 115
        },
        w: 0.3,
        ab: "Levitate"
    },
    Bastiodon: {
        t1: "Rock",
        t2: "Steel",
        bs: {
            hp: 60,
            at: 52,
            df: 168,
            sa: 47,
            sd: 138,
            sp: 30
        },
        w: 149.5
    },
    Bibarel: {
        t1: "Normal",
        t2: "Water",
        bs: {
            hp: 79,
            at: 85,
            df: 60,
            sa: 55,
            sd: 60,
            sp: 71
        },
        w: 31.5
    },
    Bidoof: {
        t1: "Normal",
        bs: {
            hp: 59,
            at: 45,
            df: 40,
            sa: 35,
            sd: 40,
            sp: 31
        },
        w: 20.0
    },
    Bonsly: {
        t1: "Rock",
        bs: {
            hp: 50,
            at: 80,
            df: 95,
            sa: 10,
            sd: 45,
            sp: 10
        },
        w: 15.0
    },
    Bronzong: {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 67,
            at: 89,
            df: 116,
            sa: 79,
            sd: 116,
            sp: 33
        },
        w: 187.0
    },
    Bronzor: {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 57,
            at: 24,
            df: 86,
            sa: 24,
            sd: 86,
            sp: 23
        },
        w: 60.5
    },
    Budew: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 40,
            at: 30,
            df: 35,
            sa: 50,
            sd: 70,
            sp: 55
        },
        w: 1.2
    },
    Buizel: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 65,
            df: 35,
            sa: 60,
            sd: 30,
            sp: 85
        },
        w: 29.5,
        ab: "Swift Swim"
    },
    Buneary: {
        t1: "Normal",
        bs: {
            hp: 55,
            at: 66,
            df: 44,
            sa: 44,
            sd: 56,
            sp: 85
        },
        w: 5.5
    },
    Burmy: {
        t1: "Bug",
        bs: {
            hp: 40,
            at: 29,
            df: 45,
            sa: 29,
            sd: 45,
            sp: 36
        },
        w: 3.4
    },
    Carnivine: {
        t1: "Grass",
        bs: {
            hp: 74,
            at: 100,
            df: 72,
            sa: 90,
            sd: 72,
            sp: 46
        },
        w: 27.0,
        ab: "Levitate"
    },
    Chatot: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 76,
            at: 65,
            df: 45,
            sa: 92,
            sd: 42,
            sp: 91
        },
        w: 1.9
    },
    Cherrim: {
        t1: "Grass",
        bs: {
            hp: 70,
            at: 60,
            df: 70,
            sa: 87,
            sd: 78,
            sp: 85
        },
        w: 9.3,
        ab: "Flower Gift"
    },
    Cherubi: {
        t1: "Grass",
        bs: {
            hp: 45,
            at: 35,
            df: 45,
            sa: 62,
            sd: 53,
            sp: 35
        },
        w: 3.3,
        ab: "Chlorophyll"
    },
    Chimchar: {
        t1: "Fire",
        bs: {
            hp: 44,
            at: 58,
            df: 44,
            sa: 58,
            sd: 44,
            sp: 61
        },
        w: 6.2,
        ab: "Blaze"
    },
    Chingling: {
        t1: "Psychic",
        bs: {
            hp: 45,
            at: 30,
            df: 50,
            sa: 65,
            sd: 50,
            sp: 45
        },
        w: 0.6,
        ab: "Levitate"
    },
    Combee: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 30,
            at: 30,
            df: 42,
            sa: 30,
            sd: 42,
            sp: 70
        },
        w: 5.5
    },
    Cranidos: {
        t1: "Rock",
        bs: {
            hp: 67,
            at: 125,
            df: 40,
            sa: 30,
            sd: 30,
            sp: 58
        },
        w: 31.5
    },
    Cresselia: {
        t1: "Psychic",
        bs: {
            hp: 120,
            at: 70,
            df: 120,
            sa: 75,
            sd: 130,
            sp: 85
        },
        w: 85.6,
        ab: "Levitate"
    },
    Croagunk: {
        t1: "Poison",
        t2: "Fighting",
        bs: {
            hp: 48,
            at: 61,
            df: 40,
            sa: 61,
            sd: 40,
            sp: 50
        },
        w: 23.0
    },
    Darkrai: {
        t1: "Dark",
        bs: {
            hp: 70,
            at: 90,
            df: 90,
            sa: 135,
            sd: 90,
            sp: 125
        },
        w: 50.5,
        ab: "Bad Dreams"
    },
    Dialga: {
        t1: "Steel",
        t2: "Dragon",
        bs: {
            hp: 100,
            at: 120,
            df: 120,
            sa: 150,
            sd: 100,
            sp: 90
        },
        w: 683.0
    },
    Drapion: {
        t1: "Poison",
        t2: "Dark",
        bs: {
            hp: 70,
            at: 90,
            df: 110,
            sa: 60,
            sd: 75,
            sp: 95
        },
        w: 61.5
    },
    Drifblim: {
        t1: "Ghost",
        t2: "Flying",
        bs: {
            hp: 150,
            at: 80,
            df: 44,
            sa: 90,
            sd: 54,
            sp: 80
        },
        w: 15.0
    },
    Drifloon: {
        t1: "Ghost",
        t2: "Flying",
        bs: {
            hp: 90,
            at: 50,
            df: 34,
            sa: 60,
            sd: 44,
            sp: 70
        },
        w: 1.2
    },
    Dusknoir: {
        t1: "Ghost",
        bs: {
            hp: 45,
            at: 100,
            df: 135,
            sa: 65,
            sd: 135,
            sp: 45
        },
        w: 106.6
    },
    Electivire: {
        t1: "Electric",
        bs: {
            hp: 75,
            at: 123,
            df: 67,
            sa: 95,
            sd: 85,
            sp: 95
        },
        w: 138.6,
        ab: "Motor Drive"
    },
    Empoleon: {
        t1: "Water",
        t2: "Steel",
        bs: {
            hp: 84,
            at: 86,
            df: 88,
            sa: 111,
            sd: 101,
            sp: 60
        },
        w: 84.5,
        ab: "Torrent"
    },
    Finneon: {
        t1: "Water",
        bs: {
            hp: 49,
            at: 49,
            df: 56,
            sa: 49,
            sd: 61,
            sp: 66
        },
        w: 7.0
    },
    Floatzel: {
        t1: "Water",
        bs: {
            hp: 85,
            at: 105,
            df: 55,
            sa: 85,
            sd: 50,
            sp: 115
        },
        w: 33.5,
        ab: "Swift Swim"
    },
    Froslass: {
        t1: "Ice",
        t2: "Ghost",
        bs: {
            hp: 70,
            at: 80,
            df: 70,
            sa: 80,
            sd: 70,
            sp: 110
        },
        w: 26.6,
        ab: "Snow Cloak"
    },
    Gabite: {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 68,
            at: 90,
            df: 65,
            sa: 50,
            sd: 55,
            sp: 82
        },
        w: 56.0,
        ab: "Sand Veil"
    },
    Gallade: {
        t1: "Psychic",
        t2: "Fighting",
        bs: {
            hp: 68,
            at: 125,
            df: 65,
            sa: 65,
            sd: 115,
            sp: 80
        },
        w: 52.0
    },
    Garchomp: {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 108,
            at: 130,
            df: 95,
            sa: 80,
            sd: 85,
            sp: 102
        },
        w: 95.0,
        ab: "Sand Veil"
    },
    Gastrodon: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 111,
            at: 83,
            df: 68,
            sa: 92,
            sd: 82,
            sp: 39
        },
        w: 29.9
    },
    Gible: {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 58,
            at: 70,
            df: 45,
            sa: 40,
            sd: 45,
            sp: 42
        },
        w: 20.5,
        ab: "Sand Veil"
    },
    Giratina: {
        t1: "Ghost",
        t2: "Dragon",
        bs: {
            hp: 150,
            at: 100,
            df: 120,
            sa: 100,
            sd: 120,
            sp: 90
        },
        w: 750.0
    },
    "Giratina-O": {
        t1: "Ghost",
        t2: "Dragon",
        bs: {
            hp: 150,
            at: 120,
            df: 100,
            sa: 120,
            sd: 100,
            sp: 90
        },
        w: 650.0,
        ab: "Levitate"
    },
    Glaceon: {
        t1: "Ice",
        bs: {
            hp: 65,
            at: 60,
            df: 110,
            sa: 130,
            sd: 95,
            sp: 65
        },
        w: 25.9,
        ab: "Snow Cloak"
    },
    Glameow: {
        t1: "Normal",
        bs: {
            hp: 49,
            at: 55,
            df: 42,
            sa: 42,
            sd: 37,
            sp: 85
        },
        w: 3.9
    },
    Gliscor: {
        t1: "Ground",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 95,
            df: 125,
            sa: 45,
            sd: 75,
            sp: 95
        },
        w: 42.5
    },
    Grotle: {
        t1: "Grass",
        bs: {
            hp: 75,
            at: 89,
            df: 85,
            sa: 55,
            sd: 65,
            sp: 36
        },
        w: 97.0,
        ab: "Overgrow"
    },
    Happiny: {
        t1: "Normal",
        bs: {
            hp: 100,
            at: 5,
            df: 5,
            sa: 15,
            sd: 65,
            sp: 30
        },
        w: 24.4
    },
    Heatran: {
        t1: "Fire",
        t2: "Steel",
        bs: {
            hp: 91,
            at: 90,
            df: 106,
            sa: 130,
            sd: 106,
            sp: 77
        },
        w: 430.0,
        ab: "Flash Fire"
    },
    Hippopotas: {
        t1: "Ground",
        bs: {
            hp: 68,
            at: 72,
            df: 78,
            sa: 38,
            sd: 42,
            sp: 32
        },
        w: 49.5,
        ab: "Sand Stream"
    },
    Hippowdon: {
        t1: "Ground",
        bs: {
            hp: 108,
            at: 112,
            df: 118,
            sa: 68,
            sd: 72,
            sp: 47
        },
        w: 300.0,
        ab: "Sand Stream"
    },
    Honchkrow: {
        t1: "Dark",
        t2: "Flying",
        bs: {
            hp: 100,
            at: 125,
            df: 52,
            sa: 105,
            sd: 52,
            sp: 71
        },
        w: 27.3
    },
    Infernape: {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 76,
            at: 104,
            df: 71,
            sa: 104,
            sd: 71,
            sp: 108
        },
        w: 55.0,
        ab: "Blaze"
    },
    Kricketot: {
        t1: "Bug",
        bs: {
            hp: 37,
            at: 25,
            df: 41,
            sa: 25,
            sd: 41,
            sp: 25
        },
        w: 2.2
    },
    Kricketune: {
        t1: "Bug",
        bs: {
            hp: 77,
            at: 85,
            df: 51,
            sa: 55,
            sd: 51,
            sp: 65
        },
        w: 25.5,
        ab: "Swarm"
    },
    Leafeon: {
        t1: "Grass",
        bs: {
            hp: 65,
            at: 110,
            df: 130,
            sa: 60,
            sd: 65,
            sp: 95
        },
        w: 25.5
    },
    Lickilicky: {
        t1: "Normal",
        bs: {
            hp: 110,
            at: 85,
            df: 95,
            sa: 80,
            sd: 95,
            sp: 50
        },
        w: 140.0
    },
    Lopunny: {
        t1: "Normal",
        bs: {
            hp: 65,
            at: 76,
            df: 84,
            sa: 54,
            sd: 96,
            sp: 105
        },
        w: 33.3
    },
    Lucario: {
        t1: "Fighting",
        t2: "Steel",
        bs: {
            hp: 70,
            at: 110,
            df: 70,
            sa: 115,
            sd: 70,
            sp: 90
        },
        w: 54.0
    },
    Lumineon: {
        t1: "Water",
        bs: {
            hp: 69,
            at: 69,
            df: 76,
            sa: 69,
            sd: 86,
            sp: 91
        },
        w: 24.0
    },
    Luxio: {
        t1: "Electric",
        bs: {
            hp: 60,
            at: 85,
            df: 49,
            sa: 60,
            sd: 49,
            sp: 60
        },
        w: 30.5
    },
    Luxray: {
        t1: "Electric",
        bs: {
            hp: 80,
            at: 120,
            df: 79,
            sa: 95,
            sd: 79,
            sp: 70
        },
        w: 42.0
    },
    Magmortar: {
        t1: "Fire",
        bs: {
            hp: 75,
            at: 95,
            df: 67,
            sa: 125,
            sd: 95,
            sp: 83
        },
        w: 68.0
    },
    Magnezone: {
        t1: "Electric",
        t2: "Steel",
        bs: {
            hp: 70,
            at: 70,
            df: 115,
            sa: 130,
            sd: 90,
            sp: 60
        },
        w: 180.0
    },
    Mamoswine: {
        t1: "Ice",
        t2: "Ground",
        bs: {
            hp: 110,
            at: 130,
            df: 80,
            sa: 70,
            sd: 60,
            sp: 80
        },
        w: 291.0
    },
    Manaphy: {
        t1: "Water",
        bs: {
            hp: 100,
            at: 100,
            df: 100,
            sa: 100,
            sd: 100,
            sp: 100
        },
        w: 1.4,
        ab: "Hydration"
    },
    Mantyke: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 45,
            at: 20,
            df: 50,
            sa: 60,
            sd: 120,
            sp: 50
        },
        w: 65.0
    },
    Mesprit: {
        t1: "Psychic",
        bs: {
            hp: 80,
            at: 105,
            df: 105,
            sa: 105,
            sd: 105,
            sp: 80
        },
        w: 0.3,
        ab: "Levitate"
    },
    "Mime Jr.": {
        t1: "Psychic",
        bs: {
            hp: 20,
            at: 25,
            df: 45,
            sa: 70,
            sd: 90,
            sp: 60
        },
        w: 13.0
    },
    Mismagius: {
        t1: "Ghost",
        bs: {
            hp: 60,
            at: 60,
            df: 60,
            sa: 105,
            sd: 105,
            sp: 105
        },
        w: 4.4,
        ab: "Levitate"
    },
    Monferno: {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 64,
            at: 78,
            df: 52,
            sa: 78,
            sd: 52,
            sp: 81
        },
        w: 22.0,
        ab: "Blaze"
    },
    Mothim: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 70,
            at: 94,
            df: 50,
            sa: 94,
            sd: 50,
            sp: 66
        },
        w: 23.3,
        ab: "Swarm"
    },
    Munchlax: {
        t1: "Normal",
        bs: {
            hp: 135,
            at: 85,
            df: 40,
            sa: 40,
            sd: 85,
            sp: 5
        },
        w: 105.0
    },
    Pachirisu: {
        t1: "Electric",
        bs: {
            hp: 60,
            at: 45,
            df: 70,
            sa: 45,
            sd: 90,
            sp: 95
        },
        w: 3.9
    },
    Palkia: {
        t1: "Water",
        t2: "Dragon",
        bs: {
            hp: 90,
            at: 120,
            df: 100,
            sa: 150,
            sd: 120,
            sp: 100
        },
        w: 336.0
    },
    Phione: {
        t1: "Water",
        bs: {
            hp: 80,
            at: 80,
            df: 80,
            sa: 80,
            sd: 80,
            sp: 80
        },
        w: 3.1,
        ab: "Hydration"
    },
    Piplup: {
        t1: "Water",
        bs: {
            hp: 53,
            at: 51,
            df: 53,
            sa: 61,
            sd: 56,
            sp: 40
        },
        w: 5.2,
        ab: "Torrent"
    },
    "Porygon-Z": {
        t1: "Normal",
        bs: {
            hp: 85,
            at: 80,
            df: 70,
            sa: 135,
            sd: 75,
            sp: 90
        },
        w: 34.0
    },
    Prinplup: {
        t1: "Water",
        bs: {
            hp: 64,
            at: 66,
            df: 68,
            sa: 81,
            sd: 76,
            sp: 50
        },
        w: 23.0,
        ab: "Torrent"
    },
    Probopass: {
        t1: "Rock",
        t2: "Steel",
        bs: {
            hp: 60,
            at: 55,
            df: 145,
            sa: 75,
            sd: 150,
            sp: 40
        },
        w: 340.0
    },
    Purugly: {
        t1: "Normal",
        bs: {
            hp: 71,
            at: 82,
            df: 64,
            sa: 64,
            sd: 59,
            sp: 112
        },
        w: 43.8
    },
    Rampardos: {
        t1: "Rock",
        bs: {
            hp: 97,
            at: 165,
            df: 60,
            sa: 65,
            sd: 50,
            sp: 58
        },
        w: 102.5
    },
    Regigigas: {
        t1: "Normal",
        bs: {
            hp: 110,
            at: 160,
            df: 110,
            sa: 80,
            sd: 110,
            sp: 100
        },
        w: 420.0,
        ab: "Slow Start"
    },
    Rhyperior: {
        t1: "Ground",
        t2: "Rock",
        bs: {
            hp: 115,
            at: 140,
            df: 130,
            sa: 55,
            sd: 55,
            sp: 40
        },
        w: 282.8
    },
    Riolu: {
        t1: "Fighting",
        bs: {
            hp: 40,
            at: 70,
            df: 40,
            sa: 35,
            sd: 40,
            sp: 60
        },
        w: 20.2
    },
    Roserade: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 70,
            df: 55,
            sa: 125,
            sd: 105,
            sp: 90
        },
        w: 14.5
    },
    Rotom: {
        t1: "Electric",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 50,
            df: 77,
            sa: 95,
            sd: 77,
            sp: 91
        },
        w: 0.3,
        ab: "Levitate"
    },
    "Rotom-C": {
        t1: "Electric",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 65,
            df: 107,
            sa: 105,
            sd: 107,
            sp: 86
        },
        w: 0.3,
        ab: "Levitate"
    },
    "Rotom-F": {
        t1: "Electric",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 65,
            df: 107,
            sa: 105,
            sd: 107,
            sp: 86
        },
        w: 0.3,
        ab: "Levitate"
    },
    "Rotom-H": {
        t1: "Electric",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 65,
            df: 107,
            sa: 105,
            sd: 107,
            sp: 86
        },
        w: 0.3,
        ab: "Levitate"
    },
    "Rotom-S": {
        t1: "Electric",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 65,
            df: 107,
            sa: 105,
            sd: 107,
            sp: 86
        },
        w: 0.3,
        ab: "Levitate"
    },
    "Rotom-W": {
        t1: "Electric",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 65,
            df: 107,
            sa: 105,
            sd: 107,
            sp: 86
        },
        w: 0.3,
        ab: "Levitate"
    },
    Shaymin: {
        t1: "Grass",
        bs: {
            hp: 100,
            at: 100,
            df: 100,
            sa: 100,
            sd: 100,
            sp: 100
        },
        w: 2.1,
        ab: "Natural Cure"
    },
    "Shaymin-S": {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 100,
            at: 103,
            df: 75,
            sa: 120,
            sd: 75,
            sp: 127
        },
        w: 5.2,
        ab: "Serene Grace"
    },
    Shellos: {
        t1: "Water",
        bs: {
            hp: 76,
            at: 48,
            df: 48,
            sa: 57,
            sd: 62,
            sp: 34
        },
        w: 6.3
    },
    Shieldon: {
        t1: "Rock",
        t2: "Steel",
        bs: {
            hp: 30,
            at: 42,
            df: 118,
            sa: 42,
            sd: 88,
            sp: 30
        },
        w: 57.0
    },
    Shinx: {
        t1: "Electric",
        bs: {
            hp: 45,
            at: 65,
            df: 34,
            sa: 40,
            sd: 34,
            sp: 45
        },
        w: 9.5
    },
    Skorupi: {
        t1: "Poison",
        t2: "Bug",
        bs: {
            hp: 40,
            at: 50,
            df: 90,
            sa: 30,
            sd: 55,
            sp: 65
        },
        w: 12.0
    },
    Skuntank: {
        t1: "Poison",
        t2: "Dark",
        bs: {
            hp: 103,
            at: 93,
            df: 67,
            sa: 71,
            sd: 61,
            sp: 84
        },
        w: 38.0
    },
    Snover: {
        t1: "Grass",
        t2: "Ice",
        bs: {
            hp: 60,
            at: 62,
            df: 50,
            sa: 62,
            sd: 60,
            sp: 40
        },
        w: 50.5,
        ab: "Snow Warning"
    },
    Spiritomb: {
        t1: "Ghost",
        t2: "Dark",
        bs: {
            hp: 50,
            at: 92,
            df: 108,
            sa: 92,
            sd: 108,
            sp: 35
        },
        w: 108.0
    },
    Staraptor: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 85,
            at: 120,
            df: 70,
            sa: 50,
            sd: 50,
            sp: 100
        },
        w: 24.9,
        ab: "Intimidate"
    },
    Staravia: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 75,
            df: 50,
            sa: 40,
            sd: 40,
            sp: 80
        },
        w: 15.5,
        ab: "Intimidate"
    },
    Starly: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 40,
            at: 55,
            df: 30,
            sa: 30,
            sd: 30,
            sp: 60
        },
        w: 2.0
    },
    Stunky: {
        t1: "Poison",
        t2: "Dark",
        bs: {
            hp: 63,
            at: 63,
            df: 47,
            sa: 41,
            sd: 41,
            sp: 74
        },
        w: 19.2
    },
    Tangrowth: {
        t1: "Grass",
        bs: {
            hp: 100,
            at: 100,
            df: 125,
            sa: 110,
            sd: 50,
            sp: 50
        },
        w: 128.6
    },
    Togekiss: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 85,
            at: 50,
            df: 95,
            sa: 120,
            sd: 115,
            sp: 80
        },
        w: 38.0
    },
    Torterra: {
        t1: "Grass",
        t2: "Ground",
        bs: {
            hp: 95,
            at: 109,
            df: 105,
            sa: 75,
            sd: 85,
            sp: 56
        },
        w: 310.0,
        ab: "Overgrow"
    },
    Toxicroak: {
        t1: "Poison",
        t2: "Fighting",
        bs: {
            hp: 83,
            at: 106,
            df: 65,
            sa: 86,
            sd: 65,
            sp: 85
        },
        w: 44.4
    },
    Turtwig: {
        t1: "Grass",
        bs: {
            hp: 55,
            at: 68,
            df: 64,
            sa: 45,
            sd: 55,
            sp: 31
        },
        w: 10.2,
        ab: "Overgrow"
    },
    Uxie: {
        t1: "Psychic",
        bs: {
            hp: 75,
            at: 75,
            df: 130,
            sa: 75,
            sd: 130,
            sp: 95
        },
        w: 0.3,
        ab: "Levitate"
    },
    Vespiquen: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 70,
            at: 80,
            df: 102,
            sa: 80,
            sd: 102,
            sp: 40
        },
        w: 38.5
    },
    Weavile: {
        t1: "Dark",
        t2: "Ice",
        bs: {
            hp: 70,
            at: 120,
            df: 65,
            sa: 45,
            sd: 85,
            sp: 125
        },
        w: 34.0
    },
    Wormadam: {
        t1: "Bug",
        t2: "Grass",
        bs: {
            hp: 60,
            at: 59,
            df: 85,
            sa: 79,
            sd: 105,
            sp: 36
        },
        w: 6.5
    },
    "Wormadam-G": {
        t1: "Bug",
        t2: "Ground",
        bs: {
            hp: 60,
            at: 79,
            df: 105,
            sa: 59,
            sd: 85,
            sp: 36
        },
        w: 6.5
    },
    "Wormadam-S": {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 60,
            at: 69,
            df: 95,
            sa: 69,
            sd: 95,
            sp: 36
        },
        w: 6.5
    },
    Yanmega: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 86,
            at: 76,
            df: 86,
            sa: 116,
            sd: 56,
            sp: 95
        },
        w: 51.5
    }
};
const GEN_5 = {
    "Rotom-C": { t2: "Grass" },
    "Rotom-F": { t2: "Ice" },
    "Rotom-H": { t2: "Fire" },
    "Rotom-S": { t2: "Flying" },
    "Rotom-W": { t2: "Water" },
    Accelgor: {
        t1: "Bug",
        bs: {
            hp: 80,
            at: 70,
            df: 40,
            sa: 100,
            sd: 60,
            sp: 145
        },
        w: 25.3
    },
    Alomomola: {
        t1: "Water",
        bs: {
            hp: 165,
            at: 75,
            df: 80,
            sa: 40,
            sd: 45,
            sp: 65
        },
        w: 31.6
    },
    Amoonguss: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 114,
            at: 85,
            df: 70,
            sa: 85,
            sd: 80,
            sp: 30
        },
        w: 10.5
    },
    Archen: {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 112,
            df: 45,
            sa: 74,
            sd: 45,
            sp: 70
        },
        w: 9.5,
        ab: "Defeatist"
    },
    Archeops: {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 140,
            df: 65,
            sa: 112,
            sd: 65,
            sp: 110
        },
        w: 32.0,
        ab: "Defeatist"
    },
    Audino: {
        t1: "Normal",
        bs: {
            hp: 103,
            at: 60,
            df: 86,
            sa: 60,
            sd: 86,
            sp: 50
        },
        w: 31.0
    },
    Axew: {
        t1: "Dragon",
        bs: {
            hp: 46,
            at: 87,
            df: 60,
            sa: 30,
            sd: 40,
            sp: 57
        },
        w: 18.0
    },
    Basculin: {
        t1: "Water",
        bs: {
            hp: 70,
            at: 92,
            df: 65,
            sa: 80,
            sd: 55,
            sp: 98
        },
        w: 18.0
    },
    Beartic: {
        t1: "Ice",
        bs: {
            hp: 95,
            at: 110,
            df: 80,
            sa: 70,
            sd: 80,
            sp: 50
        },
        w: 260.0
    },
    Beheeyem: {
        t1: "Psychic",
        bs: {
            hp: 75,
            at: 75,
            df: 75,
            sa: 125,
            sd: 95,
            sp: 40
        },
        w: 34.5
    },
    Bisharp: {
        t1: "Dark",
        t2: "Steel",
        bs: {
            hp: 65,
            at: 125,
            df: 100,
            sa: 60,
            sd: 70,
            sp: 70
        },
        w: 70.0
    },
    Blitzle: {
        t1: "Electric",
        bs: {
            hp: 45,
            at: 60,
            df: 32,
            sa: 50,
            sd: 32,
            sp: 76
        },
        w: 29.8
    },
    Boldore: {
        t1: "Rock",
        bs: {
            hp: 70,
            at: 105,
            df: 105,
            sa: 50,
            sd: 40,
            sp: 20
        },
        w: 102.0
    },
    Bouffalant: {
        t1: "Normal",
        bs: {
            hp: 95,
            at: 110,
            df: 95,
            sa: 40,
            sd: 95,
            sp: 55
        },
        w: 94.6
    },
    Braviary: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 100,
            at: 123,
            df: 75,
            sa: 57,
            sd: 75,
            sp: 80
        },
        w: 41.0
    },
    Carracosta: {
        t1: "Water",
        t2: "Rock",
        bs: {
            hp: 74,
            at: 108,
            df: 133,
            sa: 83,
            sd: 65,
            sp: 32
        },
        w: 81.0
    },
    Chandelure: {
        t1: "Ghost",
        t2: "Fire",
        bs: {
            hp: 60,
            at: 55,
            df: 90,
            sa: 145,
            sd: 90,
            sp: 80
        },
        w: 34.3
    },
    Cinccino: {
        t1: "Normal",
        bs: {
            hp: 75,
            at: 95,
            df: 60,
            sa: 65,
            sd: 60,
            sp: 115
        },
        w: 7.5
    },
    Cobalion: {
        t1: "Steel",
        t2: "Fighting",
        bs: {
            hp: 91,
            at: 90,
            df: 129,
            sa: 90,
            sd: 72,
            sp: 108
        },
        w: 250.0,
        ab: "Justified"
    },
    Cofagrigus: {
        t1: "Ghost",
        bs: {
            hp: 58,
            at: 50,
            df: 145,
            sa: 95,
            sd: 105,
            sp: 30
        },
        w: 76.5,
        ab: "Mummy"
    },
    Conkeldurr: {
        t1: "Fighting",
        bs: {
            hp: 105,
            at: 140,
            df: 95,
            sa: 55,
            sd: 65,
            sp: 45
        },
        w: 87.0
    },
    Cottonee: {
        t1: "Grass",
        bs: {
            hp: 40,
            at: 27,
            df: 60,
            sa: 37,
            sd: 50,
            sp: 66
        },
        w: 0.6
    },
    Crustle: {
        t1: "Bug",
        t2: "Rock",
        bs: {
            hp: 70,
            at: 95,
            df: 125,
            sa: 65,
            sd: 75,
            sp: 45
        },
        w: 200.0
    },
    Cryogonal: {
        t1: "Ice",
        bs: {
            hp: 70,
            at: 50,
            df: 30,
            sa: 95,
            sd: 135,
            sp: 105
        },
        w: 148.0,
        ab: "Levitate"
    },
    Cubchoo: {
        t1: "Ice",
        bs: {
            hp: 55,
            at: 70,
            df: 40,
            sa: 60,
            sd: 40,
            sp: 40
        },
        w: 8.5
    },
    Darmanitan: {
        t1: "Fire",
        bs: {
            hp: 105,
            at: 140,
            df: 55,
            sa: 30,
            sd: 55,
            sp: 95
        },
        w: 92.9,
        formes: ["Darmanitan", "Darmanitan-Z"]
    },
    "Darmanitan-Z": {
        t1: "Fire",
        t2: "Psychic",
        bs: {
            hp: 105,
            at: 30,
            df: 105,
            sa: 140,
            sd: 105,
            sp: 55
        },
        w: 92.9,
        ab: "Zen Mode",
        isAlternateForme: true
    },
    Darumaka: {
        t1: "Fire",
        bs: {
            hp: 70,
            at: 90,
            df: 45,
            sa: 15,
            sd: 45,
            sp: 50
        },
        w: 37.5
    },
    Deerling: {
        t1: "Normal",
        t2: "Grass",
        bs: {
            hp: 60,
            at: 60,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 75
        },
        w: 19.5
    },
    Deino: {
        t1: "Dark",
        t2: "Dragon",
        bs: {
            hp: 52,
            at: 65,
            df: 50,
            sa: 45,
            sd: 50,
            sp: 38
        },
        w: 17.3,
        ab: "Hustle"
    },
    Dewott: {
        t1: "Water",
        bs: {
            hp: 75,
            at: 75,
            df: 60,
            sa: 83,
            sd: 60,
            sp: 60
        },
        w: 24.5,
        ab: "Torrent"
    },
    Drilbur: {
        t1: "Ground",
        bs: {
            hp: 60,
            at: 85,
            df: 40,
            sa: 30,
            sd: 45,
            sp: 68
        },
        w: 8.5
    },
    Druddigon: {
        t1: "Dragon",
        bs: {
            hp: 77,
            at: 120,
            df: 90,
            sa: 60,
            sd: 90,
            sp: 48
        },
        w: 139.0
    },
    Ducklett: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 62,
            at: 44,
            df: 50,
            sa: 44,
            sd: 50,
            sp: 55
        },
        w: 5.5
    },
    Duosion: {
        t1: "Psychic",
        bs: {
            hp: 65,
            at: 40,
            df: 50,
            sa: 125,
            sd: 60,
            sp: 30
        },
        w: 8.0
    },
    Durant: {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 58,
            at: 109,
            df: 112,
            sa: 48,
            sd: 48,
            sp: 109
        },
        w: 33.0
    },
    Dwebble: {
        t1: "Bug",
        t2: "Rock",
        bs: {
            hp: 50,
            at: 65,
            df: 85,
            sa: 35,
            sd: 35,
            sp: 55
        },
        w: 14.5
    },
    Eelektrik: {
        t1: "Electric",
        bs: {
            hp: 65,
            at: 85,
            df: 70,
            sa: 75,
            sd: 70,
            sp: 40
        },
        w: 22.0,
        ab: "Levitate"
    },
    Eelektross: {
        t1: "Electric",
        bs: {
            hp: 85,
            at: 115,
            df: 80,
            sa: 105,
            sd: 80,
            sp: 50
        },
        w: 80.5,
        ab: "Levitate"
    },
    Elgyem: {
        t1: "Psychic",
        bs: {
            hp: 55,
            at: 55,
            df: 55,
            sa: 85,
            sd: 55,
            sp: 30
        },
        w: 9.0
    },
    Emboar: {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 110,
            at: 123,
            df: 65,
            sa: 100,
            sd: 65,
            sp: 65
        },
        w: 150.0,
        ab: "Blaze"
    },
    Emolga: {
        t1: "Electric",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 75,
            df: 60,
            sa: 75,
            sd: 60,
            sp: 103
        },
        w: 5.0
    },
    Escavalier: {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 70,
            at: 135,
            df: 105,
            sa: 60,
            sd: 105,
            sp: 20
        },
        w: 33.0
    },
    Excadrill: {
        t1: "Ground",
        t2: "Steel",
        bs: {
            hp: 110,
            at: 135,
            df: 60,
            sa: 50,
            sd: 65,
            sp: 88
        },
        w: 40.4
    },
    Ferroseed: {
        t1: "Grass",
        t2: "Steel",
        bs: {
            hp: 44,
            at: 50,
            df: 91,
            sa: 24,
            sd: 86,
            sp: 10
        },
        w: 18.8
    },
    Ferrothorn: {
        t1: "Grass",
        t2: "Steel",
        bs: {
            hp: 74,
            at: 94,
            df: 131,
            sa: 54,
            sd: 116,
            sp: 20
        },
        w: 110.0
    },
    Foongus: {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 69,
            at: 55,
            df: 45,
            sa: 55,
            sd: 55,
            sp: 15
        },
        w: 1.0
    },
    Fraxure: {
        t1: "Dragon",
        bs: {
            hp: 66,
            at: 117,
            df: 70,
            sa: 40,
            sd: 50,
            sp: 67
        },
        w: 36.0
    },
    Frillish: {
        t1: "Water",
        t2: "Ghost",
        bs: {
            hp: 55,
            at: 40,
            df: 50,
            sa: 65,
            sd: 85,
            sp: 40
        },
        w: 33.0
    },
    Galvantula: {
        t1: "Bug",
        t2: "Electric",
        bs: {
            hp: 70,
            at: 77,
            df: 60,
            sa: 97,
            sd: 60,
            sp: 108
        },
        w: 14.3
    },
    Garbodor: {
        t1: "Poison",
        bs: {
            hp: 80,
            at: 95,
            df: 82,
            sa: 60,
            sd: 82,
            sp: 75
        },
        w: 107.3
    },
    Genesect: {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 71,
            at: 120,
            df: 95,
            sa: 120,
            sd: 95,
            sp: 99
        },
        w: 82.5,
        ab: "Download"
    },
    Gigalith: {
        t1: "Rock",
        bs: {
            hp: 85,
            at: 135,
            df: 130,
            sa: 60,
            sd: 70,
            sp: 25
        },
        w: 260.0
    },
    Golett: {
        t1: "Ground",
        t2: "Ghost",
        bs: {
            hp: 59,
            at: 74,
            df: 50,
            sa: 35,
            sd: 50,
            sp: 35
        },
        w: 92.0
    },
    Golurk: {
        t1: "Ground",
        t2: "Ghost",
        bs: {
            hp: 89,
            at: 124,
            df: 80,
            sa: 55,
            sd: 80,
            sp: 55
        },
        w: 330.0
    },
    Gothita: {
        t1: "Psychic",
        bs: {
            hp: 45,
            at: 30,
            df: 50,
            sa: 55,
            sd: 65,
            sp: 45
        },
        w: 5.8
    },
    Gothitelle: {
        t1: "Psychic",
        bs: {
            hp: 70,
            at: 55,
            df: 95,
            sa: 95,
            sd: 110,
            sp: 65
        },
        w: 44.0
    },
    Gothorita: {
        t1: "Psychic",
        bs: {
            hp: 60,
            at: 45,
            df: 70,
            sa: 75,
            sd: 85,
            sp: 55
        },
        w: 18.0
    },
    Gurdurr: {
        t1: "Fighting",
        bs: {
            hp: 85,
            at: 105,
            df: 85,
            sa: 40,
            sd: 50,
            sp: 40
        },
        w: 40.0
    },
    Haxorus: {
        t1: "Dragon",
        bs: {
            hp: 76,
            at: 147,
            df: 90,
            sa: 60,
            sd: 70,
            sp: 97
        },
        w: 105.5
    },
    Heatmor: {
        t1: "Fire",
        bs: {
            hp: 85,
            at: 97,
            df: 66,
            sa: 105,
            sd: 66,
            sp: 65
        },
        w: 58.0
    },
    Herdier: {
        t1: "Normal",
        bs: {
            hp: 65,
            at: 80,
            df: 65,
            sa: 35,
            sd: 65,
            sp: 60
        },
        w: 14.7
    },
    Hydreigon: {
        t1: "Dark",
        t2: "Dragon",
        bs: {
            hp: 92,
            at: 105,
            df: 90,
            sa: 125,
            sd: 90,
            sp: 98
        },
        w: 160.0,
        ab: "Levitate"
    },
    Jellicent: {
        t1: "Water",
        t2: "Ghost",
        bs: {
            hp: 100,
            at: 60,
            df: 70,
            sa: 85,
            sd: 105,
            sp: 60
        },
        w: 135.0
    },
    Joltik: {
        t1: "Bug",
        t2: "Electric",
        bs: {
            hp: 50,
            at: 47,
            df: 50,
            sa: 57,
            sd: 50,
            sp: 65
        },
        w: 0.6
    },
    Karrablast: {
        t1: "Bug",
        bs: {
            hp: 50,
            at: 75,
            df: 45,
            sa: 40,
            sd: 45,
            sp: 60
        },
        w: 5.9
    },
    Keldeo: {
        t1: "Water",
        t2: "Fighting",
        bs: {
            hp: 91,
            at: 72,
            df: 90,
            sa: 129,
            sd: 90,
            sp: 108
        },
        w: 48.5,
        ab: "Justified"
    },
    Klang: {
        t1: "Steel",
        bs: {
            hp: 60,
            at: 80,
            df: 95,
            sa: 70,
            sd: 85,
            sp: 50
        },
        w: 51.0
    },
    Klink: {
        t1: "Steel",
        bs: {
            hp: 40,
            at: 55,
            df: 70,
            sa: 45,
            sd: 60,
            sp: 30
        },
        w: 21.0
    },
    Klinklang: {
        t1: "Steel",
        bs: {
            hp: 60,
            at: 100,
            df: 115,
            sa: 70,
            sd: 85,
            sp: 90
        },
        w: 81.0
    },
    Krokorok: {
        t1: "Ground",
        t2: "Dark",
        bs: {
            hp: 60,
            at: 82,
            df: 45,
            sa: 45,
            sd: 45,
            sp: 74
        },
        w: 33.4
    },
    Krookodile: {
        t1: "Ground",
        t2: "Dark",
        bs: {
            hp: 95,
            at: 117,
            df: 70,
            sa: 65,
            sd: 70,
            sp: 92
        },
        w: 96.3
    },
    Kyurem: {
        t1: "Dragon",
        t2: "Ice",
        bs: {
            hp: 125,
            at: 130,
            df: 90,
            sa: 130,
            sd: 90,
            sp: 95
        },
        w: 325.0,
        ab: "Pressure"
    },
    "Kyurem-B": {
        t1: "Dragon",
        t2: "Ice",
        bs: {
            hp: 125,
            at: 170,
            df: 100,
            sa: 120,
            sd: 90,
            sp: 95
        },
        w: 325.0,
        ab: "Teravolt"
    },
    "Kyurem-W": {
        t1: "Dragon",
        t2: "Ice",
        bs: {
            hp: 125,
            at: 120,
            df: 90,
            sa: 170,
            sd: 100,
            sp: 95
        },
        w: 325.0,
        ab: "Turboblaze"
    },
    Lampent: {
        t1: "Ghost",
        t2: "Fire",
        bs: {
            hp: 60,
            at: 40,
            df: 60,
            sa: 95,
            sd: 60,
            sp: 55
        },
        w: 13.0
    },
    Landorus: {
        t1: "Ground",
        t2: "Flying",
        bs: {
            hp: 89,
            at: 125,
            df: 90,
            sa: 115,
            sd: 80,
            sp: 101
        },
        w: 68.0
    },
    "Landorus-T": {
        t1: "Ground",
        t2: "Flying",
        bs: {
            hp: 89,
            at: 145,
            df: 90,
            sa: 105,
            sd: 80,
            sp: 91
        },
        w: 68.0,
        ab: "Intimidate"
    },
    Larvesta: {
        t1: "Bug",
        t2: "Fire",
        bs: {
            hp: 55,
            at: 85,
            df: 55,
            sa: 50,
            sd: 55,
            sp: 60
        },
        w: 28.8
    },
    Leavanny: {
        t1: "Bug",
        t2: "Grass",
        bs: {
            hp: 75,
            at: 103,
            df: 80,
            sa: 70,
            sd: 70,
            sp: 92
        },
        w: 20.5
    },
    Liepard: {
        t1: "Dark",
        bs: {
            hp: 64,
            at: 88,
            df: 50,
            sa: 88,
            sd: 50,
            sp: 106
        },
        w: 37.5
    },
    Lilligant: {
        t1: "Grass",
        bs: {
            hp: 70,
            at: 60,
            df: 75,
            sa: 110,
            sd: 75,
            sp: 90
        },
        w: 16.3
    },
    Lillipup: {
        t1: "Normal",
        bs: {
            hp: 45,
            at: 60,
            df: 45,
            sa: 25,
            sd: 45,
            sp: 55
        },
        w: 4.1
    },
    Litwick: {
        t1: "Ghost",
        t2: "Fire",
        bs: {
            hp: 50,
            at: 30,
            df: 55,
            sa: 65,
            sd: 55,
            sp: 20
        },
        w: 3.1
    },
    Mandibuzz: {
        t1: "Dark",
        t2: "Flying",
        bs: {
            hp: 110,
            at: 65,
            df: 105,
            sa: 55,
            sd: 95,
            sp: 80
        },
        w: 39.5
    },
    Maractus: {
        t1: "Grass",
        bs: {
            hp: 75,
            at: 86,
            df: 67,
            sa: 106,
            sd: 67,
            sp: 60
        },
        w: 28.0
    },
    Meloetta: {
        t1: "Normal",
        t2: "Psychic",
        bs: {
            hp: 100,
            at: 77,
            df: 77,
            sa: 128,
            sd: 128,
            sp: 90
        },
        w: 6.5,
        ab: "Serene Grace",
        formes: ["Meloetta", "Meloetta-P"]
    },
    "Meloetta-P": {
        t1: "Normal",
        t2: "Fighting",
        bs: {
            hp: 100,
            at: 128,
            df: 90,
            sa: 77,
            sd: 77,
            sp: 128
        },
        w: 6.5,
        ab: "Serene Grace",
        isAlternateForme: true
    },
    Mienfoo: {
        t1: "Fighting",
        bs: {
            hp: 45,
            at: 85,
            df: 50,
            sa: 55,
            sd: 50,
            sp: 65
        },
        w: 20.0
    },
    Mienshao: {
        t1: "Fighting",
        bs: {
            hp: 65,
            at: 125,
            df: 60,
            sa: 95,
            sd: 60,
            sp: 105
        },
        w: 35.5
    },
    Minccino: {
        t1: "Normal",
        bs: {
            hp: 55,
            at: 50,
            df: 40,
            sa: 40,
            sd: 40,
            sp: 75
        },
        w: 5.8
    },
    Munna: {
        t1: "Psychic",
        bs: {
            hp: 76,
            at: 25,
            df: 45,
            sa: 67,
            sd: 55,
            sp: 24
        },
        w: 23.3
    },
    Musharna: {
        t1: "Psychic",
        bs: {
            hp: 116,
            at: 55,
            df: 85,
            sa: 107,
            sd: 95,
            sp: 29
        },
        w: 60.5
    },
    Oshawott: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 55,
            df: 45,
            sa: 63,
            sd: 45,
            sp: 45
        },
        w: 5.9,
        ab: "Torrent"
    },
    Palpitoad: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 75,
            at: 65,
            df: 55,
            sa: 65,
            sd: 55,
            sp: 69
        },
        w: 17.0
    },
    Panpour: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 53,
            df: 48,
            sa: 53,
            sd: 48,
            sp: 64
        },
        w: 13.5
    },
    Pansage: {
        t1: "Grass",
        bs: {
            hp: 50,
            at: 53,
            df: 48,
            sa: 53,
            sd: 48,
            sp: 64
        },
        w: 10.5
    },
    Pansear: {
        t1: "Fire",
        bs: {
            hp: 50,
            at: 53,
            df: 48,
            sa: 53,
            sd: 48,
            sp: 64
        },
        w: 11.0
    },
    Patrat: {
        t1: "Normal",
        bs: {
            hp: 45,
            at: 55,
            df: 39,
            sa: 35,
            sd: 39,
            sp: 42
        },
        w: 11.6
    },
    Pawniard: {
        t1: "Dark",
        t2: "Steel",
        bs: {
            hp: 45,
            at: 85,
            df: 70,
            sa: 40,
            sd: 40,
            sp: 60
        },
        w: 10.2
    },
    Petilil: {
        t1: "Grass",
        bs: {
            hp: 45,
            at: 35,
            df: 50,
            sa: 70,
            sd: 50,
            sp: 30
        },
        w: 6.6
    },
    Pidove: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 50,
            at: 55,
            df: 50,
            sa: 36,
            sd: 30,
            sp: 43
        },
        w: 2.1
    },
    Pignite: {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 90,
            at: 93,
            df: 55,
            sa: 70,
            sd: 55,
            sp: 55
        },
        w: 55.5,
        ab: "Blaze"
    },
    Purrloin: {
        t1: "Dark",
        bs: {
            hp: 41,
            at: 50,
            df: 37,
            sa: 50,
            sd: 37,
            sp: 66
        },
        w: 10.1
    },
    Reshiram: {
        t1: "Dragon",
        t2: "Fire",
        bs: {
            hp: 100,
            at: 120,
            df: 100,
            sa: 150,
            sd: 120,
            sp: 90
        },
        w: 330.0,
        ab: "Turboblaze"
    },
    Reuniclus: {
        t1: "Psychic",
        bs: {
            hp: 110,
            at: 65,
            df: 75,
            sa: 125,
            sd: 85,
            sp: 30
        },
        w: 20.1
    },
    Roggenrola: {
        t1: "Rock",
        bs: {
            hp: 55,
            at: 75,
            df: 85,
            sa: 25,
            sd: 25,
            sp: 15
        },
        w: 18.0
    },
    Rufflet: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 70,
            at: 83,
            df: 50,
            sa: 37,
            sd: 50,
            sp: 60
        },
        w: 10.5
    },
    Samurott: {
        t1: "Water",
        bs: {
            hp: 95,
            at: 100,
            df: 85,
            sa: 108,
            sd: 70,
            sp: 70
        },
        w: 94.6,
        ab: "Torrent"
    },
    Sandile: {
        t1: "Ground",
        t2: "Dark",
        bs: {
            hp: 50,
            at: 72,
            df: 35,
            sa: 35,
            sd: 35,
            sp: 65
        },
        w: 15.2
    },
    Sawk: {
        t1: "Fighting",
        bs: {
            hp: 75,
            at: 125,
            df: 75,
            sa: 30,
            sd: 75,
            sp: 85
        },
        w: 51.0
    },
    Sawsbuck: {
        t1: "Normal",
        t2: "Grass",
        bs: {
            hp: 80,
            at: 100,
            df: 70,
            sa: 60,
            sd: 70,
            sp: 95
        },
        w: 92.5
    },
    Scolipede: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 90,
            df: 89,
            sa: 55,
            sd: 69,
            sp: 112
        },
        w: 200.5
    },
    Scrafty: {
        t1: "Dark",
        t2: "Fighting",
        bs: {
            hp: 65,
            at: 90,
            df: 115,
            sa: 45,
            sd: 115,
            sp: 58
        },
        w: 30.0
    },
    Scraggy: {
        t1: "Dark",
        t2: "Fighting",
        bs: {
            hp: 50,
            at: 75,
            df: 70,
            sa: 35,
            sd: 70,
            sp: 48
        },
        w: 11.8
    },
    Seismitoad: {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 105,
            at: 85,
            df: 75,
            sa: 85,
            sd: 75,
            sp: 74
        },
        w: 62.0
    },
    Serperior: {
        t1: "Grass",
        bs: {
            hp: 75,
            at: 75,
            df: 95,
            sa: 75,
            sd: 95,
            sp: 113
        },
        w: 63.0,
        ab: "Overgrow"
    },
    Servine: {
        t1: "Grass",
        bs: {
            hp: 60,
            at: 60,
            df: 75,
            sa: 60,
            sd: 75,
            sp: 83
        },
        w: 16.0,
        ab: "Overgrow"
    },
    Sewaddle: {
        t1: "Bug",
        t2: "Grass",
        bs: {
            hp: 45,
            at: 53,
            df: 70,
            sa: 40,
            sd: 60,
            sp: 42
        },
        w: 2.5
    },
    Shelmet: {
        t1: "Bug",
        bs: {
            hp: 50,
            at: 40,
            df: 85,
            sa: 40,
            sd: 65,
            sp: 25
        },
        w: 7.7
    },
    Sigilyph: {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 72,
            at: 58,
            df: 80,
            sa: 103,
            sd: 80,
            sp: 97
        },
        w: 14.0
    },
    Simipour: {
        t1: "Water",
        bs: {
            hp: 75,
            at: 98,
            df: 63,
            sa: 98,
            sd: 63,
            sp: 101
        },
        w: 29.0
    },
    Simisage: {
        t1: "Grass",
        bs: {
            hp: 75,
            at: 98,
            df: 63,
            sa: 98,
            sd: 63,
            sp: 101
        },
        w: 30.5
    },
    Simisear: {
        t1: "Fire",
        bs: {
            hp: 75,
            at: 98,
            df: 63,
            sa: 98,
            sd: 63,
            sp: 101
        },
        w: 28.0
    },
    Snivy: {
        t1: "Grass",
        bs: {
            hp: 45,
            at: 45,
            df: 55,
            sa: 45,
            sd: 55,
            sp: 63
        },
        w: 8.1,
        ab: "Overgrow"
    },
    Solosis: {
        t1: "Psychic",
        bs: {
            hp: 45,
            at: 30,
            df: 40,
            sa: 105,
            sd: 50,
            sp: 20
        },
        w: 1.0
    },
    Stoutland: {
        t1: "Normal",
        bs: {
            hp: 85,
            at: 100,
            df: 90,
            sa: 45,
            sd: 90,
            sp: 80
        },
        w: 61.0
    },
    Stunfisk: {
        t1: "Ground",
        t2: "Electric",
        bs: {
            hp: 109,
            at: 66,
            df: 84,
            sa: 81,
            sd: 99,
            sp: 32
        },
        w: 11.0
    },
    Swadloon: {
        t1: "Bug",
        t2: "Grass",
        bs: {
            hp: 55,
            at: 63,
            df: 90,
            sa: 50,
            sd: 80,
            sp: 42
        },
        w: 7.3
    },
    Swanna: {
        t1: "Water",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 87,
            df: 63,
            sa: 87,
            sd: 63,
            sp: 98
        },
        w: 24.2
    },
    Swoobat: {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 67,
            at: 57,
            df: 55,
            sa: 77,
            sd: 55,
            sp: 114
        },
        w: 10.5
    },
    Tepig: {
        t1: "Fire",
        bs: {
            hp: 65,
            at: 63,
            df: 45,
            sa: 45,
            sd: 45,
            sp: 45
        },
        w: 9.9,
        ab: "Blaze"
    },
    Terrakion: {
        t1: "Rock",
        t2: "Fighting",
        bs: {
            hp: 91,
            at: 129,
            df: 90,
            sa: 72,
            sd: 90,
            sp: 108
        },
        w: 260.0,
        ab: "Justified"
    },
    Throh: {
        t1: "Fighting",
        bs: {
            hp: 120,
            at: 100,
            df: 85,
            sa: 30,
            sd: 85,
            sp: 45
        },
        w: 55.5
    },
    Thundurus: {
        t1: "Electric",
        t2: "Flying",
        bs: {
            hp: 79,
            at: 115,
            df: 70,
            sa: 125,
            sd: 80,
            sp: 111
        },
        w: 61.0
    },
    "Thundurus-T": {
        t1: "Electric",
        t2: "Flying",
        bs: {
            hp: 79,
            at: 105,
            df: 70,
            sa: 145,
            sd: 80,
            sp: 101
        },
        w: 61.0,
        ab: "Volt Absorb"
    },
    Timburr: {
        t1: "Fighting",
        bs: {
            hp: 75,
            at: 80,
            df: 55,
            sa: 25,
            sd: 35,
            sp: 35
        },
        w: 12.5
    },
    Tirtouga: {
        t1: "Water",
        t2: "Rock",
        bs: {
            hp: 54,
            at: 78,
            df: 103,
            sa: 53,
            sd: 45,
            sp: 22
        },
        w: 16.5
    },
    Tornadus: {
        t1: "Flying",
        bs: {
            hp: 79,
            at: 115,
            df: 70,
            sa: 125,
            sd: 80,
            sp: 111
        },
        w: 63.0
    },
    "Tornadus-T": {
        t1: "Flying",
        bs: {
            hp: 79,
            at: 100,
            df: 80,
            sa: 110,
            sd: 90,
            sp: 121
        },
        w: 63.0,
        ab: "Regenerator"
    },
    Tranquill: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 62,
            at: 77,
            df: 62,
            sa: 50,
            sd: 42,
            sp: 65
        },
        w: 15.0
    },
    Trubbish: {
        t1: "Poison",
        bs: {
            hp: 50,
            at: 50,
            df: 62,
            sa: 40,
            sd: 62,
            sp: 65
        },
        w: 31.0
    },
    Tympole: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 50,
            df: 40,
            sa: 50,
            sd: 40,
            sp: 64
        },
        w: 4.5
    },
    Tynamo: {
        t1: "Electric",
        bs: {
            hp: 35,
            at: 55,
            df: 40,
            sa: 45,
            sd: 40,
            sp: 60
        },
        w: 0.3,
        ab: "Levitate"
    },
    Unfezant: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 80,
            at: 105,
            df: 80,
            sa: 65,
            sd: 55,
            sp: 93
        },
        w: 29.0
    },
    Vanillish: {
        t1: "Ice",
        bs: {
            hp: 51,
            at: 65,
            df: 65,
            sa: 80,
            sd: 75,
            sp: 59
        },
        w: 41.0
    },
    Vanillite: {
        t1: "Ice",
        bs: {
            hp: 36,
            at: 50,
            df: 50,
            sa: 65,
            sd: 60,
            sp: 44
        },
        w: 5.7
    },
    Vanilluxe: {
        t1: "Ice",
        bs: {
            hp: 71,
            at: 95,
            df: 85,
            sa: 110,
            sd: 95,
            sp: 79
        },
        w: 57.5
    },
    Venipede: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 30,
            at: 45,
            df: 59,
            sa: 30,
            sd: 39,
            sp: 57
        },
        w: 5.3
    },
    Victini: {
        t1: "Psychic",
        t2: "Fire",
        bs: {
            hp: 100,
            at: 100,
            df: 100,
            sa: 100,
            sd: 100,
            sp: 100
        },
        w: 4.0,
        ab: "Victory Star"
    },
    Virizion: {
        t1: "Grass",
        t2: "Fighting",
        bs: {
            hp: 91,
            at: 90,
            df: 72,
            sa: 90,
            sd: 129,
            sp: 108
        },
        w: 200.0,
        ab: "Justified"
    },
    Volcarona: {
        t1: "Bug",
        t2: "Fire",
        bs: {
            hp: 85,
            at: 60,
            df: 65,
            sa: 135,
            sd: 105,
            sp: 100
        },
        w: 46.0
    },
    Vullaby: {
        t1: "Dark",
        t2: "Flying",
        bs: {
            hp: 70,
            at: 55,
            df: 75,
            sa: 45,
            sd: 65,
            sp: 60
        },
        w: 9.0
    },
    Watchog: {
        t1: "Normal",
        bs: {
            hp: 60,
            at: 85,
            df: 69,
            sa: 60,
            sd: 69,
            sp: 77
        },
        w: 27.0
    },
    Whimsicott: {
        t1: "Grass",
        bs: {
            hp: 60,
            at: 67,
            df: 85,
            sa: 77,
            sd: 75,
            sp: 116
        },
        w: 6.6
    },
    Whirlipede: {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 40,
            at: 55,
            df: 99,
            sa: 40,
            sd: 79,
            sp: 47
        },
        w: 58.5
    },
    Woobat: {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 45,
            df: 43,
            sa: 55,
            sd: 43,
            sp: 72
        },
        w: 2.1
    },
    Yamask: {
        t1: "Ghost",
        bs: {
            hp: 38,
            at: 30,
            df: 85,
            sa: 55,
            sd: 65,
            sp: 30
        },
        w: 1.5,
        ab: "Mummy"
    },
    Zebstrika: {
        t1: "Electric",
        bs: {
            hp: 75,
            at: 100,
            df: 63,
            sa: 80,
            sd: 63,
            sp: 116
        },
        w: 79.5
    },
    Zekrom: {
        t1: "Dragon",
        t2: "Electric",
        bs: {
            hp: 100,
            at: 150,
            df: 120,
            sa: 120,
            sd: 100,
            sp: 90
        },
        w: 345.0,
        ab: "Teravolt"
    },
    Zoroark: {
        t1: "Dark",
        bs: {
            hp: 60,
            at: 105,
            df: 60,
            sa: 120,
            sd: 60,
            sp: 105
        },
        w: 81.1,
        ab: "Illusion"
    },
    Zorua: {
        t1: "Dark",
        bs: {
            hp: 40,
            at: 65,
            df: 40,
            sa: 80,
            sd: 40,
            sp: 65
        },
        w: 12.5,
        ab: "Illusion"
    },
    Zweilous: {
        t1: "Dark",
        t2: "Dragon",
        bs: {
            hp: 72,
            at: 85,
            df: 70,
            sa: 65,
            sd: 70,
            sp: 58
        },
        w: 50.0,
        ab: "Hustle"
    }
};
const GEN_6 = {
    Venusaur: { formes: ["Venusaur", "Mega Venusaur"] },
    Charizard: { formes: ["Mega Charizard Y", "Mega Charizard X", "Charizard"] },
    Blastoise: { formes: ["Blastoise", "Mega Blastoise"] },
    Butterfree: { bs: { sa: 90 } },
    Beedrill: { bs: { at: 90 }, formes: ["Mega Beedrill", "Beedrill"] },
    Pidgeot: { bs: { sp: 101 }, formes: ["Mega Pidgeot", "Pidgeot"] },
    Pikachu: { bs: { df: 40, sd: 50 } },
    Raichu: { bs: { sp: 110 } },
    Nidoqueen: { bs: { at: 92 } },
    Nidoking: { bs: { at: 102 } },
    Clefairy: { t1: "Fairy" },
    Clefable: { t1: "Fairy", bs: { sa: 95 } },
    Jigglypuff: { t2: "Fairy" },
    Wigglytuff: { t2: "Fairy", bs: { sa: 85 } },
    Vileplume: { bs: { sa: 110 } },
    Poliwrath: { bs: { at: 95 } },
    Alakazam: { bs: { sd: 95 }, formes: ["Mega Alakazam", "Alakazam"] },
    Victreebel: { bs: { sd: 70 } },
    Golem: { bs: { at: 120 } },
    Slowbro: { formes: ["Mega Slowbro", "Slowbro"] },
    Gengar: { formes: ["Mega Gengar", "Gengar"] },
    Kangaskhan: { formes: ["Mega Kangaskhan", "Kangaskhan"] },
    "Mr. Mime": { t2: "Fairy" },
    Pinsir: { formes: ["Mega Pinsir", "Pinsir"] },
    Gyarados: { formes: ["Mega Gyarados", "Gyarados"] },
    Aerodactyl: { formes: ["Aerodactyl", "Mega Aerodactyl"] },
    Mewtwo: { formes: ["Mewtwo", "Mega Mewtwo X", "Mega Mewtwo Y"] },
    Cleffa: { t1: "Fairy" },
    Igglybuff: { t2: "Fairy" },
    Togepi: { t1: "Fairy" },
    Togetic: { t1: "Fairy" },
    Ampharos: { bs: { df: 85 }, formes: ["Mega Ampharos", "Ampharos"] },
    Bellossom: { bs: { df: 95 } },
    Marill: { t2: "Fairy" },
    Azumarill: { t2: "Fairy", bs: { sa: 60 } },
    Jumpluff: { bs: { sd: 95 } },
    Steelix: { formes: ["Steelix", "Mega Steelix"] },
    Snubbull: { t1: "Fairy" },
    Granbull: { t1: "Fairy" },
    Scizor: { formes: ["Scizor", "Mega Scizor"] },
    Heracross: { formes: ["Heracross", "Mega Heracross"] },
    Houndoom: { formes: ["Houndoom", "Mega Houndoom"] },
    Tyranitar: { formes: ["Tyranitar", "Mega Tyranitar"] },
    Sceptile: { formes: ["Sceptile", "Mega Sceptile"] },
    Blaziken: { formes: ["Blaziken", "Mega Blaziken"] },
    Swampert: { formes: ["Swampert", "Mega Swampert"] },
    Beautifly: { bs: { sa: 100 } },
    Ralts: { t2: "Fairy" },
    Kirlia: { t2: "Fairy" },
    Gardevoir: { t2: "Fairy", formes: ["Mega Gardevoir", "Gardevoir"] },
    Exploud: { bs: { sd: 73 } },
    Azurill: { t2: "Fairy" },
    Sableye: { formes: ["Sableye", "Mega Sableye"] },
    Mawile: { t2: "Fairy", formes: ["Mega Mawile", "Mawile"] },
    Aggron: { formes: ["Aggron", "Mega Aggron"] },
    Medicham: { formes: ["Mega Medicham", "Medicham"] },
    Manectric: { formes: ["Mega Manectric", "Manectric"] },
    Sharpedo: { formes: ["Sharpedo", "Mega Sharpedo"] },
    Camerupt: { formes: ["Mega Camerupt", "Camerupt"] },
    Altaria: { formes: ["Altaria", "Mega Altaria"] },
    Banette: { formes: ["Banette", "Mega Banette"] },
    Absol: { formes: ["Absol", "Mega Absol"] },
    Glalie: { formes: ["Glalie", "Mega Glalie"] },
    Salamence: { formes: ["Mega Salamence", "Salamence"] },
    Metagross: { formes: ["Mega Metagross", "Metagross"] },
    Latias: { formes: ["Latias", "Mega Latias"] },
    Latios: { formes: ["Latios", "Mega Latios"] },
    Kyogre: { formes: ["Primal Kyogre", "Kyogre"] },
    Groudon: { formes: ["Primal Groudon", "Groudon"] },
    Rayquaza: { formes: ["Rayquaza", "Mega Rayquaza"] },
    Staraptor: { bs: { sd: 60 } },
    Roserade: { bs: { df: 65 } },
    Lopunny: { formes: ["Mega Lopunny", "Lopunny"] },
    "Mime Jr.": { t2: "Fairy" },
    Garchomp: { formes: ["Garchomp", "Mega Garchomp"] },
    Lucario: { formes: ["Lucario", "Mega Lucario"] },
    Abomasnow: { formes: ["Abomasnow", "Mega Abomasnow"] },
    Togekiss: { t1: "Fairy" },
    Gallade: { formes: ["Gallade", "Mega Gallade"] },
    Stoutland: { bs: { at: 110 } },
    Unfezant: { bs: { at: 115 } },
    Gigalith: { bs: { sd: 80 } },
    Audino: { formes: ["Audino", "Mega Audino"] },
    Seismitoad: { bs: { at: 95 } },
    Leavanny: { bs: { sd: 80 } },
    Scolipede: { bs: { at: 100 } },
    Cottonee: { t2: "Fairy" },
    Whimsicott: { t2: "Fairy" },
    Krookodile: { bs: { df: 80 } },
    Aegislash: {
        t1: "Steel",
        t2: "Ghost",
        bs: {
            hp: 60,
            at: 50,
            df: 150,
            sa: 50,
            sd: 150,
            sp: 60
        },
        w: 53.0,
        ab: "Stance Change",
        formes: ["Aegislash-Shield", "Aegislash-Blade"]
    },
    "Aegislash-Blade": {
        t1: "Steel",
        t2: "Ghost",
        bs: {
            hp: 60,
            at: 150,
            df: 50,
            sa: 150,
            sd: 50,
            sp: 60
        },
        w: 53.0,
        ab: "Stance Change"
    },
    "Aegislash-Shield": {
        t1: "Steel",
        t2: "Ghost",
        bs: {
            hp: 60,
            at: 50,
            df: 150,
            sa: 50,
            sd: 150,
            sp: 60
        },
        w: 53.0,
        ab: "Stance Change",
        isAlternateForme: true
    },
    Amaura: {
        t1: "Rock",
        t2: "Ice",
        bs: {
            hp: 77,
            at: 59,
            df: 50,
            sa: 67,
            sd: 63,
            sp: 46
        },
        w: 25.2,
        ab: "Refrigerate"
    },
    "Arceus-Fairy": {
        t1: "Fairy",
        bs: {
            hp: 120,
            at: 120,
            df: 120,
            sa: 120,
            sd: 120,
            sp: 120
        },
        w: 320.0,
        ab: "Multitype"
    },
    Aromatisse: {
        t1: "Fairy",
        bs: {
            hp: 101,
            at: 72,
            df: 72,
            sa: 99,
            sd: 89,
            sp: 29
        },
        w: 15.5
    },
    Aurorus: {
        t1: "Rock",
        t2: "Ice",
        bs: {
            hp: 123,
            at: 77,
            df: 72,
            sa: 99,
            sd: 92,
            sp: 58
        },
        w: 225.0,
        ab: "Refrigerate"
    },
    Avalugg: {
        t1: "Ice",
        bs: {
            hp: 95,
            at: 117,
            df: 184,
            sa: 44,
            sd: 46,
            sp: 28
        },
        w: 505.0
    },
    Barbaracle: {
        t1: "Rock",
        t2: "Water",
        bs: {
            hp: 72,
            at: 105,
            df: 115,
            sa: 54,
            sd: 86,
            sp: 68
        },
        w: 96.0
    },
    Bergmite: {
        t1: "Ice",
        bs: {
            hp: 55,
            at: 69,
            df: 85,
            sa: 32,
            sd: 35,
            sp: 28
        },
        w: 99.5
    },
    Binacle: {
        t1: "Rock",
        t2: "Water",
        bs: {
            hp: 42,
            at: 52,
            df: 67,
            sa: 39,
            sd: 56,
            sp: 50
        },
        w: 31.0
    },
    Braixen: {
        t1: "Fire",
        bs: {
            hp: 59,
            at: 59,
            df: 58,
            sa: 90,
            sd: 70,
            sp: 73
        },
        w: 14.5
    },
    Bunnelby: {
        t1: "Normal",
        bs: {
            hp: 38,
            at: 36,
            df: 38,
            sa: 32,
            sd: 36,
            sp: 57
        },
        w: 5.0
    },
    Carbink: {
        t1: "Rock",
        t2: "Fairy",
        bs: {
            hp: 50,
            at: 50,
            df: 150,
            sa: 50,
            sd: 150,
            sp: 50
        },
        w: 5.7
    },
    Chesnaught: {
        t1: "Grass",
        t2: "Fighting",
        bs: {
            hp: 88,
            at: 107,
            df: 122,
            sa: 74,
            sd: 75,
            sp: 64
        },
        w: 90.0
    },
    Chespin: {
        t1: "Grass",
        bs: {
            hp: 56,
            at: 61,
            df: 65,
            sa: 48,
            sd: 45,
            sp: 38
        },
        w: 9.0
    },
    Clauncher: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 53,
            df: 62,
            sa: 58,
            sd: 63,
            sp: 44
        },
        w: 8.3,
        ab: "Mega Launcher"
    },
    Clawitzer: {
        t1: "Water",
        bs: {
            hp: 71,
            at: 73,
            df: 88,
            sa: 120,
            sd: 89,
            sp: 59
        },
        w: 35.3,
        ab: "Mega Launcher"
    },
    Diancie: {
        t1: "Rock",
        t2: "Fairy",
        bs: {
            hp: 50,
            at: 100,
            df: 150,
            sa: 100,
            sd: 150,
            sp: 50
        },
        w: 8.8,
        ab: "Clear Body",
        formes: ["Diancie", "Mega Diancie"]
    },
    Dedenne: {
        t1: "Electric",
        t2: "Fairy",
        bs: {
            hp: 67,
            at: 58,
            df: 57,
            sa: 81,
            sd: 67,
            sp: 101
        },
        w: 2.2
    },
    Delphox: {
        t1: "Fire",
        t2: "Psychic",
        bs: {
            hp: 75,
            at: 69,
            df: 72,
            sa: 114,
            sd: 100,
            sp: 104
        },
        w: 39.0
    },
    Diggersby: {
        t1: "Normal",
        t2: "Ground",
        bs: {
            hp: 85,
            at: 56,
            df: 77,
            sa: 50,
            sd: 77,
            sp: 78
        },
        w: 42.4
    },
    Doublade: {
        t1: "Steel",
        t2: "Ghost",
        bs: {
            hp: 59,
            at: 110,
            df: 150,
            sa: 45,
            sd: 49,
            sp: 35
        },
        w: 4.5,
        ab: "No Guard"
    },
    Dragalge: {
        t1: "Poison",
        t2: "Dragon",
        bs: {
            hp: 65,
            at: 75,
            df: 90,
            sa: 97,
            sd: 123,
            sp: 44
        },
        w: 81.5
    },
    Espurr: {
        t1: "Psychic",
        bs: {
            hp: 62,
            at: 48,
            df: 54,
            sa: 63,
            sd: 60,
            sp: 68
        },
        w: 3.5
    },
    Fennekin: {
        t1: "Fire",
        bs: {
            hp: 40,
            at: 45,
            df: 40,
            sa: 62,
            sd: 60,
            sp: 60
        },
        w: 9.4
    },
    Flabebe: {
        t1: "Fairy",
        bs: {
            hp: 44,
            at: 38,
            df: 39,
            sa: 61,
            sd: 79,
            sp: 42
        },
        w: 0.1
    },
    Fletchinder: {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 62,
            at: 73,
            df: 55,
            sa: 56,
            sd: 52,
            sp: 84
        },
        w: 16.0
    },
    Fletchling: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 45,
            at: 50,
            df: 43,
            sa: 40,
            sd: 38,
            sp: 62
        },
        w: 1.7
    },
    Floette: {
        t1: "Fairy",
        bs: {
            hp: 54,
            at: 45,
            df: 47,
            sa: 75,
            sd: 98,
            sp: 52
        },
        w: 0.9
    },
    "Floette-E": {
        t1: "Fairy",
        bs: {
            hp: 74,
            at: 65,
            df: 67,
            sa: 125,
            sd: 128,
            sp: 92
        },
        w: 0.9,
        ab: "Flower Veil"
    },
    Florges: {
        t1: "Fairy",
        bs: {
            hp: 78,
            at: 65,
            df: 68,
            sa: 112,
            sd: 154,
            sp: 75
        },
        w: 10.0
    },
    Froakie: {
        t1: "Water",
        bs: {
            hp: 41,
            at: 56,
            df: 40,
            sa: 62,
            sd: 44,
            sp: 71
        },
        w: 7.0
    },
    Frogadier: {
        t1: "Water",
        bs: {
            hp: 54,
            at: 63,
            df: 52,
            sa: 83,
            sd: 56,
            sp: 97
        },
        w: 10.9
    },
    Furfrou: {
        t1: "Normal",
        bs: {
            hp: 75,
            at: 80,
            df: 60,
            sa: 65,
            sd: 90,
            sp: 102
        },
        w: 28.0,
        ab: "Fur Coat"
    },
    Gogoat: {
        t1: "Grass",
        bs: {
            hp: 123,
            at: 100,
            df: 62,
            sa: 97,
            sd: 81,
            sp: 68
        },
        w: 91.0
    },
    Goodra: {
        t1: "Dragon",
        bs: {
            hp: 90,
            at: 100,
            df: 70,
            sa: 110,
            sd: 150,
            sp: 80
        },
        w: 150.5
    },
    Goomy: {
        t1: "Dragon",
        bs: {
            hp: 45,
            at: 50,
            df: 35,
            sa: 55,
            sd: 75,
            sp: 40
        },
        w: 2.8
    },
    "Gourgeist-Average": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 65,
            at: 90,
            df: 122,
            sa: 58,
            sd: 75,
            sp: 84
        },
        w: 12.5
    },
    "Gourgeist-Large": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 75,
            at: 95,
            df: 122,
            sa: 58,
            sd: 75,
            sp: 69
        },
        w: 14.0
    },
    "Gourgeist-Small": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 55,
            at: 85,
            df: 122,
            sa: 58,
            sd: 75,
            sp: 99
        },
        w: 9.5
    },
    "Gourgeist-Super": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 85,
            at: 100,
            df: 122,
            sa: 58,
            sd: 75,
            sp: 54
        },
        w: 39.0
    },
    Greninja: {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 72,
            at: 95,
            df: 67,
            sa: 103,
            sd: 71,
            sp: 122
        },
        ab: "Protean",
        w: 40.0
    },
    Hawlucha: {
        t1: "Fighting",
        t2: "Flying",
        bs: {
            hp: 78,
            at: 92,
            df: 75,
            sa: 74,
            sd: 63,
            sp: 118
        },
        w: 21.5
    },
    Heliolisk: {
        t1: "Electric",
        t2: "Normal",
        bs: {
            hp: 62,
            at: 55,
            df: 52,
            sa: 109,
            sd: 94,
            sp: 109
        },
        w: 21.0
    },
    Helioptile: {
        t1: "Electric",
        t2: "Normal",
        bs: {
            hp: 44,
            at: 38,
            df: 33,
            sa: 61,
            sd: 43,
            sp: 70
        },
        w: 6.0
    },
    Honedge: {
        t1: "Steel",
        t2: "Ghost",
        bs: {
            hp: 45,
            at: 80,
            df: 100,
            sa: 35,
            sd: 37,
            sp: 28
        },
        w: 2.0,
        ab: "No Guard"
    },
    Hoopa: {
        t1: "Psychic",
        t2: "Ghost",
        bs: {
            hp: 80,
            at: 110,
            df: 60,
            sa: 150,
            sd: 130,
            sp: 70
        },
        w: 9.0,
        ab: "Magician"
    },
    "Hoopa-Unbound": {
        t1: "Psychic",
        t2: "Dark",
        bs: {
            hp: 80,
            at: 160,
            df: 60,
            sa: 170,
            sd: 130,
            sp: 80
        },
        w: 490.0,
        ab: "Magician"
    },
    Inkay: {
        t1: "Dark",
        t2: "Psychic",
        bs: {
            hp: 53,
            at: 54,
            df: 53,
            sa: 37,
            sd: 46,
            sp: 45
        },
        w: 3.5
    },
    Klefki: {
        t1: "Steel",
        t2: "Fairy",
        bs: {
            hp: 57,
            at: 80,
            df: 91,
            sa: 80,
            sd: 87,
            sp: 75
        },
        w: 3.0
    },
    Litleo: {
        t1: "Fire",
        t2: "Normal",
        bs: {
            hp: 62,
            at: 50,
            df: 58,
            sa: 73,
            sd: 54,
            sp: 72
        },
        w: 13.5
    },
    Malamar: {
        t1: "Dark",
        t2: "Psychic",
        bs: {
            hp: 86,
            at: 92,
            df: 88,
            sa: 68,
            sd: 75,
            sp: 73
        },
        w: 47.0
    },
    "Mega Abomasnow": {
        t1: "Grass",
        t2: "Ice",
        bs: {
            hp: 90,
            at: 132,
            df: 105,
            sa: 132,
            sd: 105,
            sp: 30
        },
        w: 185.0,
        ab: "Snow Warning",
        isAlternateForme: true
    },
    "Mega Absol": {
        t1: "Dark",
        bs: {
            hp: 65,
            at: 150,
            df: 60,
            sa: 115,
            sd: 60,
            sp: 115
        },
        w: 49.0,
        ab: "Magic Bounce",
        isAlternateForme: true
    },
    "Mega Aerodactyl": {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 80,
            at: 135,
            df: 85,
            sa: 70,
            sd: 95,
            sp: 150
        },
        w: 79.0,
        ab: "Tough Claws",
        isAlternateForme: true
    },
    "Mega Aggron": {
        t1: "Steel",
        bs: {
            hp: 70,
            at: 140,
            df: 230,
            sa: 60,
            sd: 80,
            sp: 50
        },
        w: 395.0,
        ab: "Filter",
        isAlternateForme: true
    },
    "Mega Alakazam": {
        t1: "Psychic",
        bs: {
            hp: 55,
            at: 50,
            df: 65,
            sa: 175,
            sd: 95,
            sp: 150
        },
        w: 48.0,
        ab: "Trace",
        isAlternateForme: true
    },
    "Mega Altaria": {
        t1: "Dragon",
        t2: "Fairy",
        bs: {
            hp: 75,
            at: 110,
            df: 110,
            sa: 110,
            sd: 105,
            sp: 80
        },
        w: 20.6,
        ab: "Pixilate",
        isAlternateForme: true
    },
    "Mega Ampharos": {
        t1: "Electric",
        t2: "Dragon",
        bs: {
            hp: 90,
            at: 95,
            df: 105,
            sa: 165,
            sd: 110,
            sp: 45
        },
        w: 61.5,
        ab: "Mold Breaker",
        isAlternateForme: true
    },
    "Mega Audino": {
        t1: "Normal",
        t2: "Fairy",
        bs: {
            hp: 103,
            at: 60,
            df: 126,
            sa: 80,
            sd: 126,
            sp: 50
        },
        w: 32.0,
        ab: "Healer",
        isAlternateForme: true
    },
    "Mega Banette": {
        t1: "Ghost",
        bs: {
            hp: 64,
            at: 165,
            df: 75,
            sa: 93,
            sd: 83,
            sp: 75
        },
        w: 13.0,
        ab: "Prankster",
        isAlternateForme: true
    },
    "Mega Beedrill": {
        t1: "Bug",
        t2: "Poison",
        bs: {
            hp: 65,
            at: 150,
            df: 40,
            sa: 15,
            sd: 80,
            sp: 145
        },
        w: 40.5,
        ab: "Adaptability",
        isAlternateForme: true
    },
    "Mega Blastoise": {
        t1: "Water",
        bs: {
            hp: 79,
            at: 103,
            df: 120,
            sa: 135,
            sd: 115,
            sp: 78
        },
        w: 101.1,
        ab: "Mega Launcher",
        isAlternateForme: true
    },
    "Mega Blaziken": {
        t1: "Fire",
        t2: "Fighting",
        bs: {
            hp: 80,
            at: 160,
            df: 80,
            sa: 130,
            sd: 80,
            sp: 100
        },
        w: 52.0,
        ab: "Speed Boost",
        isAlternateForme: true
    },
    "Mega Camerupt": {
        t1: "Fire",
        t2: "Ground",
        bs: {
            hp: 70,
            at: 120,
            df: 100,
            sa: 145,
            sd: 105,
            sp: 20
        },
        w: 320.5,
        ab: "Sheer Force",
        isAlternateForme: true
    },
    "Mega Charizard X": {
        t1: "Fire",
        t2: "Dragon",
        bs: {
            hp: 78,
            at: 130,
            df: 111,
            sa: 130,
            sd: 85,
            sp: 100
        },
        w: 110.5,
        ab: "Tough Claws",
        isAlternateForme: true
    },
    "Mega Charizard Y": {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 78,
            at: 104,
            df: 78,
            sa: 159,
            sd: 115,
            sp: 100
        },
        w: 100.5,
        ab: "Drought",
        isAlternateForme: true
    },
    "Mega Diancie": {
        t1: "Rock",
        t2: "Fairy",
        bs: {
            hp: 50,
            at: 160,
            df: 110,
            sa: 160,
            sd: 110,
            sp: 110
        },
        w: 27.8,
        ab: "Magic Bounce",
        isAlternateForme: true
    },
    "Mega Gallade": {
        t1: "Psychic",
        t2: "Fighting",
        bs: {
            hp: 68,
            at: 165,
            df: 95,
            sa: 65,
            sd: 115,
            sp: 110
        },
        w: 56.4,
        ab: "Inner Focus",
        isAlternateForme: true
    },
    "Mega Garchomp": {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 108,
            at: 170,
            df: 115,
            sa: 120,
            sd: 95,
            sp: 92
        },
        w: 95.0,
        ab: "Sand Force",
        isAlternateForme: true
    },
    "Mega Gardevoir": {
        t1: "Psychic",
        t2: "Fairy",
        bs: {
            hp: 68,
            at: 85,
            df: 65,
            sa: 165,
            sd: 135,
            sp: 100
        },
        w: 48.4,
        ab: "Pixilate",
        isAlternateForme: true
    },
    "Mega Gengar": {
        t1: "Ghost",
        t2: "Poison",
        bs: {
            hp: 60,
            at: 65,
            df: 80,
            sa: 170,
            sd: 95,
            sp: 130
        },
        w: 40.5,
        ab: "Shadow Tag",
        isAlternateForme: true
    },
    "Mega Glalie": {
        t1: "Ice",
        bs: {
            hp: 80,
            at: 120,
            df: 80,
            sa: 120,
            sd: 80,
            sp: 100
        },
        w: 350.2,
        ab: "Refrigerate",
        isAlternateForme: true
    },
    "Mega Gyarados": {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 95,
            at: 155,
            df: 109,
            sa: 70,
            sd: 130,
            sp: 81
        },
        w: 305.0,
        ab: "Mold Breaker",
        isAlternateForme: true
    },
    "Mega Heracross": {
        t1: "Bug",
        t2: "Fighting",
        bs: {
            hp: 80,
            at: 185,
            df: 115,
            sa: 40,
            sd: 105,
            sp: 75
        },
        w: 62.5,
        ab: "Skill Link",
        isAlternateForme: true
    },
    "Mega Houndoom": {
        t1: "Dark",
        t2: "Fire",
        bs: {
            hp: 75,
            at: 90,
            df: 90,
            sa: 140,
            sd: 90,
            sp: 115
        },
        w: 49.5,
        ab: "Solar Power",
        isAlternateForme: true
    },
    "Mega Kangaskhan": {
        t1: "Normal",
        bs: {
            hp: 105,
            at: 125,
            df: 100,
            sa: 60,
            sd: 100,
            sp: 100
        },
        w: 100.0,
        ab: "Parental Bond",
        isAlternateForme: true
    },
    "Mega Latias": {
        t1: "Dragon",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 100,
            df: 120,
            sa: 140,
            sd: 150,
            sp: 110
        },
        w: 52.0,
        ab: "Levitate",
        isAlternateForme: true
    },
    "Mega Latios": {
        t1: "Dragon",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 130,
            df: 100,
            sa: 160,
            sd: 120,
            sp: 110
        },
        w: 70.0,
        ab: "Levitate",
        isAlternateForme: true
    },
    "Mega Lopunny": {
        t1: "Normal",
        t2: "Fighting",
        bs: {
            hp: 65,
            at: 136,
            df: 94,
            sa: 54,
            sd: 96,
            sp: 135
        },
        w: 28.3,
        ab: "Scrappy",
        isAlternateForme: true
    },
    "Mega Lucario": {
        t1: "Fighting",
        t2: "Steel",
        bs: {
            hp: 70,
            at: 145,
            df: 88,
            sa: 140,
            sd: 70,
            sp: 112
        },
        w: 57.5,
        ab: "Adaptability",
        isAlternateForme: true
    },
    "Mega Manectric": {
        t1: "Electric",
        bs: {
            hp: 70,
            at: 75,
            df: 80,
            sa: 135,
            sd: 80,
            sp: 135
        },
        w: 44.0,
        ab: "Intimidate",
        isAlternateForme: true
    },
    "Mega Mawile": {
        t1: "Steel",
        t2: "Fairy",
        bs: {
            hp: 50,
            at: 105,
            df: 125,
            sa: 55,
            sd: 95,
            sp: 50
        },
        w: 23.5,
        ab: "Huge Power",
        isAlternateForme: true
    },
    "Mega Medicham": {
        t1: "Fighting",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 100,
            df: 85,
            sa: 80,
            sd: 85,
            sp: 100
        },
        w: 31.5,
        ab: "Pure Power",
        isAlternateForme: true
    },
    "Mega Metagross": {
        t1: "Steel",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 145,
            df: 150,
            sa: 105,
            sd: 110,
            sp: 110
        },
        w: 942.9,
        ab: "Tough Claws",
        isAlternateForme: true
    },
    "Mega Mewtwo X": {
        t1: "Psychic",
        t2: "Fighting",
        bs: {
            hp: 106,
            at: 190,
            df: 100,
            sa: 154,
            sd: 100,
            sp: 130
        },
        w: 127.0,
        ab: "Steadfast",
        isAlternateForme: true
    },
    "Mega Mewtwo Y": {
        t1: "Psychic",
        bs: {
            hp: 106,
            at: 150,
            df: 70,
            sa: 194,
            sd: 120,
            sp: 140
        },
        w: 33.0,
        ab: "Insomnia",
        isAlternateForme: true
    },
    "Mega Pidgeot": {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 83,
            at: 80,
            df: 80,
            sa: 135,
            sd: 80,
            sp: 121
        },
        w: 50.5,
        ab: "No Guard",
        isAlternateForme: true
    },
    "Mega Pinsir": {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 65,
            at: 155,
            df: 120,
            sa: 65,
            sd: 90,
            sp: 105
        },
        w: 59.0,
        ab: "Aerilate",
        isAlternateForme: true
    },
    "Mega Rayquaza": {
        t1: "Dragon",
        t2: "Flying",
        bs: {
            hp: 105,
            at: 180,
            df: 100,
            sa: 180,
            sd: 100,
            sp: 115
        },
        w: 392.0,
        ab: "Delta Stream",
        isAlternateForme: true
    },
    "Mega Sableye": {
        t1: "Dark",
        t2: "Ghost",
        bs: {
            hp: 50,
            at: 85,
            df: 125,
            sa: 85,
            sd: 115,
            sp: 20
        },
        w: 161.0,
        ab: "Magic Bounce",
        isAlternateForme: true
    },
    "Mega Salamence": {
        t1: "Dragon",
        t2: "Flying",
        bs: {
            hp: 95,
            at: 145,
            df: 130,
            sa: 120,
            sd: 90,
            sp: 120
        },
        w: 112.6,
        ab: "Aerilate",
        isAlternateForme: true
    },
    "Mega Sceptile": {
        t1: "Grass",
        t2: "Dragon",
        bs: {
            hp: 70,
            at: 110,
            df: 75,
            sa: 145,
            sd: 85,
            sp: 145
        },
        w: 55.2,
        ab: "Lightning Rod",
        isAlternateForme: true
    },
    "Mega Scizor": {
        t1: "Bug",
        t2: "Steel",
        bs: {
            hp: 70,
            at: 150,
            df: 140,
            sa: 65,
            sd: 100,
            sp: 75
        },
        w: 125.0,
        ab: "Technician",
        isAlternateForme: true
    },
    "Mega Sharpedo": {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 70,
            at: 140,
            df: 70,
            sa: 110,
            sd: 65,
            sp: 105
        },
        w: 130.3,
        ab: "Strong Jaw",
        isAlternateForme: true
    },
    "Mega Slowbro": {
        t1: "Water",
        t2: "Psychic",
        bs: {
            hp: 95,
            at: 75,
            df: 180,
            sa: 130,
            sd: 80,
            sp: 30
        },
        w: 120.0,
        ab: "Shell Armor",
        isAlternateForme: true
    },
    "Mega Steelix": {
        t1: "Steel",
        t2: "Ground",
        bs: {
            hp: 75,
            at: 125,
            df: 230,
            sa: 55,
            sd: 95,
            sp: 30
        },
        w: 740.0,
        ab: "Sand Force",
        isAlternateForme: true
    },
    "Mega Swampert": {
        t1: "Water",
        t2: "Ground",
        bs: {
            hp: 100,
            at: 150,
            df: 110,
            sa: 95,
            sd: 110,
            sp: 70
        },
        w: 102.0,
        ab: "Swift Swim",
        isAlternateForme: true
    },
    "Mega Tyranitar": {
        t1: "Rock",
        t2: "Dark",
        bs: {
            hp: 100,
            at: 164,
            df: 150,
            sa: 95,
            sd: 120,
            sp: 71
        },
        w: 255.0,
        ab: "Sand Stream",
        isAlternateForme: true
    },
    "Mega Venusaur": {
        t1: "Grass",
        t2: "Poison",
        bs: {
            hp: 80,
            at: 100,
            df: 123,
            sa: 122,
            sd: 120,
            sp: 80
        },
        w: 155.5,
        ab: "Thick Fat",
        isAlternateForme: true
    },
    Meowstic: {
        t1: "Psychic",
        bs: {
            hp: 74,
            at: 48,
            df: 76,
            sa: 83,
            sd: 81,
            sp: 104
        },
        w: 8.5
    },
    Noibat: {
        t1: "Flying",
        t2: "Dragon",
        bs: {
            hp: 40,
            at: 30,
            df: 35,
            sa: 45,
            sd: 40,
            sp: 55
        },
        w: 8.0
    },
    Noivern: {
        t1: "Flying",
        t2: "Dragon",
        bs: {
            hp: 85,
            at: 70,
            df: 80,
            sa: 97,
            sd: 80,
            sp: 123
        },
        w: 85.0
    },
    Pancham: {
        t1: "Fighting",
        bs: {
            hp: 67,
            at: 82,
            df: 62,
            sa: 46,
            sd: 48,
            sp: 43
        },
        w: 8.0
    },
    Pangoro: {
        t1: "Fighting",
        t2: "Dark",
        bs: {
            hp: 95,
            at: 124,
            df: 78,
            sa: 69,
            sd: 71,
            sp: 58
        },
        w: 136.0
    },
    Phantump: {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 43,
            at: 70,
            df: 48,
            sa: 50,
            sd: 60,
            sp: 38
        },
        w: 7.0
    },
    "Primal Groudon": {
        t1: "Ground",
        t2: "Fire",
        bs: {
            hp: 100,
            at: 180,
            df: 160,
            sa: 150,
            sd: 90,
            sp: 90
        },
        w: 999.7,
        ab: "Desolate Land"
    },
    "Primal Kyogre": {
        t1: "Water",
        bs: {
            hp: 100,
            at: 150,
            df: 90,
            sa: 180,
            sd: 160,
            sp: 90
        },
        w: 430.0,
        ab: "Primordial Sea"
    },
    "Pumpkaboo-Average": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 49,
            at: 66,
            df: 70,
            sa: 44,
            sd: 55,
            sp: 51
        },
        w: 5.0
    },
    "Pumpkaboo-Large": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 54,
            at: 66,
            df: 70,
            sa: 44,
            sd: 55,
            sp: 46
        },
        w: 7.5
    },
    "Pumpkaboo-Small": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 44,
            at: 66,
            df: 70,
            sa: 44,
            sd: 55,
            sp: 56
        },
        w: 3.5
    },
    "Pumpkaboo-Super": {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 59,
            at: 66,
            df: 70,
            sa: 44,
            sd: 55,
            sp: 41
        },
        w: 15.0
    },
    Pyroar: {
        t1: "Fire",
        t2: "Normal",
        bs: {
            hp: 86,
            at: 68,
            df: 72,
            sa: 109,
            sd: 66,
            sp: 106
        },
        w: 81.5
    },
    Quilladin: {
        t1: "Grass",
        bs: {
            hp: 61,
            at: 78,
            df: 95,
            sa: 56,
            sd: 58,
            sp: 57
        },
        w: 29.0
    },
    Scatterbug: {
        t1: "Bug",
        bs: {
            hp: 38,
            at: 35,
            df: 40,
            sa: 27,
            sd: 25,
            sp: 35
        },
        w: 2.5
    },
    Skiddo: {
        t1: "Grass",
        bs: {
            hp: 66,
            at: 65,
            df: 48,
            sa: 62,
            sd: 57,
            sp: 52
        },
        w: 31.0
    },
    Skrelp: {
        t1: "Poison",
        t2: "Water",
        bs: {
            hp: 50,
            at: 60,
            df: 60,
            sa: 60,
            sd: 60,
            sp: 30
        },
        w: 7.3
    },
    Sliggoo: {
        t1: "Dragon",
        bs: {
            hp: 68,
            at: 75,
            df: 53,
            sa: 83,
            sd: 113,
            sp: 60
        },
        w: 17.5
    },
    Slurpuff: {
        t1: "Fairy",
        bs: {
            hp: 82,
            at: 80,
            df: 86,
            sa: 85,
            sd: 75,
            sp: 72
        },
        w: 5.0
    },
    Spewpa: {
        t1: "Bug",
        bs: {
            hp: 45,
            at: 22,
            df: 60,
            sa: 27,
            sd: 30,
            sp: 29
        },
        w: 8.4
    },
    Spritzee: {
        t1: "Fairy",
        bs: {
            hp: 78,
            at: 52,
            df: 60,
            sa: 63,
            sd: 65,
            sp: 23
        },
        w: 0.5
    },
    Swirlix: {
        t1: "Fairy",
        bs: {
            hp: 62,
            at: 48,
            df: 66,
            sa: 59,
            sd: 57,
            sp: 49
        },
        w: 3.5
    },
    Sylveon: {
        t1: "Fairy",
        bs: {
            hp: 95,
            at: 65,
            df: 65,
            sa: 110,
            sd: 130,
            sp: 60
        },
        w: 23.5
    },
    Talonflame: {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 78,
            at: 81,
            df: 71,
            sa: 74,
            sd: 69,
            sp: 126
        },
        w: 24.5
    },
    Trevenant: {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 85,
            at: 110,
            df: 76,
            sa: 65,
            sd: 82,
            sp: 56
        },
        w: 71.0
    },
    Tyrantrum: {
        t1: "Rock",
        t2: "Dragon",
        bs: {
            hp: 82,
            at: 121,
            df: 119,
            sa: 69,
            sd: 59,
            sp: 71
        },
        w: 270.0,
        ab: "Strong Jaw"
    },
    Tyrunt: {
        t1: "Rock",
        t2: "Dragon",
        bs: {
            hp: 58,
            at: 89,
            df: 77,
            sa: 45,
            sd: 45,
            sp: 48
        },
        w: 26.0,
        ab: "Strong Jaw"
    },
    Vivillon: {
        t1: "Bug",
        t2: "Flying",
        bs: {
            hp: 80,
            at: 52,
            df: 50,
            sa: 90,
            sd: 50,
            sp: 89
        },
        w: 17.0
    },
    Volcanion: {
        t1: "Fire",
        t2: "Water",
        bs: {
            hp: 80,
            at: 110,
            df: 120,
            sa: 130,
            sd: 90,
            sp: 70
        },
        w: 195.0,
        ab: "Water Absorb"
    },
    Xerneas: {
        t1: "Fairy",
        bs: {
            hp: 126,
            at: 131,
            df: 95,
            sa: 131,
            sd: 98,
            sp: 99
        },
        w: 215.0,
        ab: "Fairy Aura"
    },
    Yveltal: {
        t1: "Dark",
        t2: "Flying",
        bs: {
            hp: 126,
            at: 131,
            df: 95,
            sa: 131,
            sd: 98,
            sp: 99
        },
        w: 203.0,
        ab: "Dark Aura"
    },
    Zygarde: {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 108,
            at: 100,
            df: 121,
            sa: 81,
            sd: 95,
            sp: 95
        },
        w: 305.0,
        ab: "Power Construct",
        formes: ["Zygarde 50%", "Zygarde 10%", "Zygarde Complete"]
    },
    "Zygarde 50%": {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 108,
            at: 100,
            df: 121,
            sa: 81,
            sd: 95,
            sp: 95
        },
        w: 305.0,
        ab: "Power Construct",
        isAlternateForme: true
    },
    "Zygarde 10%": {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 54,
            at: 100,
            df: 71,
            sa: 61,
            sd: 85,
            sp: 115
        },
        w: 33.5,
        ab: "Power Construct",
        isAlternateForme: true
    },
    "Zygarde Complete": {
        t1: "Dragon",
        t2: "Ground",
        bs: {
            hp: 216,
            at: 100,
            df: 121,
            sa: 91,
            sd: 95,
            sp: 85
        },
        w: 610.0,
        ab: "Power Construct",
        isAlternateForme: true
    }
};
const GEN_7 = {
    //new forms
    Greninja: { formes: ["Greninja", "Ash-Greninja"] },
    //abilities
    Pelipper: { bs: { sa: 95 }, ab: "Drizzle" },
    Torkoal: { ab: "Drought" },
    //hp buffs
    Magcargo: { bs: { hp: 60, sa: 90 } },
    Corsola: { bs: { hp: 65, df: 95, sd: 95 } },
    Mantine: { bs: { hp: 85 } },
    Lunatone: { bs: { hp: 90 } },
    Solrock: { bs: { hp: 90 } },
    Chimecho: { bs: { hp: 75, df: 80, sd: 90 } },
    Woobat: { bs: { hp: 65 } },
    Cryogonal: { bs: { hp: 80, df: 50 } },
    //attack buffs
    Dugtrio: { bs: { at: 100 } },
    Arbok: { bs: { at: 95 } },
    "Farfetch'd": { bs: { at: 90 } },
    Crustle: { bs: { at: 105 } },
    Beartic: { bs: { at: 130 } },
    //defense buffs
    Qwilfish: { bs: { df: 85 } },
    Volbeat: { bs: { df: 75, sd: 85 } },
    Illumise: { bs: { df: 75, sd: 85 } },
    //special attack buffs
    Noctowl: { bs: { sa: 86 } },
    Swellow: { bs: { sa: 75 } },
    //special defense buffs
    Exeggutor: { bs: { sd: 75 } },
    Ariados: { bs: { sd: 70 } },
    //speed buffs
    Dodrio: { bs: { sp: 110 } },
    Electrode: { bs: { sp: 150 } },
    Delcatty: { bs: { sp: 90 } },
    Masquerain: { bs: { sp: 80, sa: 100 } },
    //alakazam buff because he's a special snowflake
    "Mega Alakazam": { bs: { sd: 105 } },
    //and here's the dex!
    "Ash-Greninja": {
        t1: "Water",
        t2: "Dark",
        bs: {
            hp: 72,
            at: 145,
            df: 67,
            sa: 153,
            sd: 71,
            sp: 132
        },
        w: 40.0,
        ab: "Battle Bond",
        isAlternateForme: true
    },
    "Rattata-Alola": {
        t1: "Dark",
        t2: "Normal",
        bs: {
            hp: 30,
            at: 56,
            df: 35,
            sa: 25,
            sd: 35,
            sp: 72
        },
        w: 3.8
    },
    "Raticate-Alola": {
        t1: "Dark",
        t2: "Normal",
        bs: {
            hp: 75,
            at: 71,
            df: 70,
            sa: 40,
            sd: 80,
            sp: 77
        },
        w: 25.5
    },
    "Meowth-Alola": {
        t1: "Dark",
        bs: {
            hp: 40,
            at: 35,
            df: 35,
            sa: 50,
            sd: 40,
            sp: 90
        },
        w: 4.2
    },
    "Persian-Alola": {
        t1: "Dark",
        bs: {
            hp: 65,
            at: 60,
            df: 60,
            sa: 75,
            sd: 65,
            sp: 115
        },
        w: 33.0
    },
    "Raichu-Alola": {
        t1: "Electric",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 85,
            df: 50,
            sa: 95,
            sd: 85,
            sp: 110
        },
        w: 21.0
    },
    "Marowak-Alola": {
        t1: "Fire",
        t2: "Ghost",
        bs: {
            hp: 60,
            at: 80,
            df: 110,
            sa: 50,
            sd: 80,
            sp: 45
        },
        w: 34.0
    },
    "Geodude-Alola": {
        t1: "Rock",
        t2: "Electric",
        bs: {
            hp: 40,
            at: 80,
            df: 100,
            sa: 30,
            sd: 30,
            sp: 20
        },
        w: 20.3
    },
    "Graveler-Alola": {
        t1: "Rock",
        t2: "Electric",
        bs: {
            hp: 55,
            at: 95,
            df: 115,
            sa: 45,
            sd: 45,
            sp: 35
        },
        w: 110.0
    },
    "Golem-Alola": {
        t1: "Rock",
        t2: "Electric",
        bs: {
            hp: 80,
            at: 120,
            df: 130,
            sa: 55,
            sd: 65,
            sp: 45
        },
        ab: "Galvanize",
        w: 316.0
    },
    "Exeggutor-Alola": {
        t1: "Grass",
        t2: "Dragon",
        bs: {
            hp: 95,
            at: 105,
            df: 85,
            sa: 125,
            sd: 75,
            sp: 45
        },
        w: 415.6
    },
    "Diglett-Alola": {
        t1: "Ground",
        t2: "Steel",
        bs: {
            hp: 10,
            at: 55,
            df: 30,
            sa: 35,
            sd: 45,
            sp: 90
        },
        w: 1.0
    },
    "Dugtrio-Alola": {
        t1: "Ground",
        t2: "Steel",
        bs: {
            hp: 35,
            at: 100,
            df: 60,
            sa: 50,
            sd: 70,
            sp: 110
        },
        w: 66.6
    },
    "Vulpix-Alola": {
        t1: "Ice",
        bs: {
            hp: 38,
            at: 41,
            df: 40,
            sa: 50,
            sd: 65,
            sp: 65
        },
        w: 9.9
    },
    "Ninetales-Alola": {
        t1: "Ice",
        t2: "Fairy",
        bs: {
            hp: 73,
            at: 67,
            df: 75,
            sa: 81,
            sd: 100,
            sp: 109
        },
        w: 19.9,
        ab: "Snow Warning"
    },
    "Sandshrew-Alola": {
        t1: "Ice",
        t2: "Steel",
        bs: {
            hp: 50,
            at: 75,
            df: 90,
            sa: 10,
            sd: 35,
            sp: 40
        },
        w: 40.0
    },
    "Sandslash-Alola": {
        t1: "Ice",
        t2: "Steel",
        bs: {
            hp: 75,
            at: 100,
            df: 120,
            sa: 25,
            sd: 65,
            sp: 65
        },
        w: 55.0
    },
    "Grimer-Alola": {
        t1: "Poison",
        t2: "Dark",
        bs: {
            hp: 80,
            at: 80,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 25
        },
        w: 42.0
    },
    "Muk-Alola": {
        t1: "Poison",
        t2: "Dark",
        bs: {
            hp: 105,
            at: 105,
            df: 75,
            sa: 65,
            sd: 100,
            sp: 50
        },
        w: 52.0
    },
    Rowlet: {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 68,
            at: 55,
            df: 55,
            sa: 50,
            sd: 50,
            sp: 42
        },
        w: 1.5
    },
    Dartrix: {
        t1: "Grass",
        t2: "Flying",
        bs: {
            hp: 78,
            at: 75,
            df: 75,
            sa: 70,
            sd: 70,
            sp: 52
        },
        w: 16.0
    },
    Decidueye: {
        t1: "Grass",
        t2: "Ghost",
        bs: {
            hp: 78,
            at: 107,
            df: 75,
            sa: 100,
            sd: 100,
            sp: 70
        },
        w: 36.6
    },
    Litten: {
        t1: "Fire",
        bs: {
            hp: 45,
            at: 65,
            df: 40,
            sa: 60,
            sd: 40,
            sp: 70
        },
        w: 4.3
    },
    Torracat: {
        t1: "Fire",
        bs: {
            hp: 65,
            at: 85,
            df: 50,
            sa: 80,
            sd: 50,
            sp: 90
        },
        w: 25.0
    },
    Incineroar: {
        t1: "Fire",
        t2: "Dark",
        bs: {
            hp: 95,
            at: 115,
            df: 90,
            sa: 80,
            sd: 90,
            sp: 60
        },
        w: 83.0
    },
    Popplio: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 54,
            df: 54,
            sa: 66,
            sd: 56,
            sp: 40
        },
        w: 7.5
    },
    Brionne: {
        t1: "Water",
        bs: {
            hp: 60,
            at: 69,
            df: 69,
            sa: 91,
            sd: 81,
            sp: 50
        },
        w: 17.5
    },
    Primarina: {
        t1: "Water",
        t2: "Fairy",
        bs: {
            hp: 80,
            at: 74,
            df: 74,
            sa: 126,
            sd: 116,
            sp: 60
        },
        w: 44.0
    },
    Pikipek: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 35,
            at: 75,
            df: 30,
            sa: 30,
            sd: 30,
            sp: 65
        },
        w: 1.2
    },
    Trumbeak: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 55,
            at: 85,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 75
        },
        w: 14.8
    },
    Toucannon: {
        t1: "Normal",
        t2: "Flying",
        bs: {
            hp: 80,
            at: 120,
            df: 75,
            sa: 75,
            sd: 75,
            sp: 60
        },
        w: 26.0
    },
    Yungoos: {
        t1: "Normal",
        bs: {
            hp: 48,
            at: 70,
            df: 30,
            sa: 30,
            sd: 30,
            sp: 45
        },
        w: 6.0
    },
    Gumshoos: {
        t1: "Normal",
        bs: {
            hp: 88,
            at: 110,
            df: 60,
            sa: 55,
            sd: 60,
            sp: 45
        },
        w: 14.2
    },
    Grubbin: {
        t1: "Bug",
        bs: {
            hp: 47,
            at: 62,
            df: 45,
            sa: 55,
            sd: 45,
            sp: 46
        },
        w: 4.4
    },
    Charjabug: {
        t1: "Bug",
        t2: "Electric",
        bs: {
            hp: 57,
            at: 82,
            df: 95,
            sa: 55,
            sd: 75,
            sp: 36
        },
        w: 10.5
    },
    Vikavolt: {
        t1: "Bug",
        t2: "Electric",
        bs: {
            hp: 77,
            at: 70,
            df: 90,
            sa: 145,
            sd: 75,
            sp: 43
        },
        ab: "Levitate",
        w: 45.0
    },
    Crabrawler: {
        t1: "Fighting",
        bs: {
            hp: 47,
            at: 82,
            df: 57,
            sa: 42,
            sd: 47,
            sp: 63
        },
        w: 7.0
    },
    Crabominable: {
        t1: "Fighting",
        t2: "Ice",
        bs: {
            hp: 97,
            at: 132,
            df: 77,
            sa: 62,
            sd: 67,
            sp: 43
        },
        w: 180.0
    },
    "Oricorio-Fire": {
        t1: "Fire",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 70,
            df: 70,
            sa: 98,
            sd: 70,
            sp: 93
        },
        w: 3.4
    },
    "Oricorio-Electric": {
        t1: "Electric",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 70,
            df: 70,
            sa: 98,
            sd: 70,
            sp: 93
        },
        w: 3.4
    },
    "Oricorio-Psychic": {
        t1: "Psychic",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 70,
            df: 70,
            sa: 98,
            sd: 70,
            sp: 93
        },
        w: 3.4
    },
    "Oricorio-Ghost": {
        t1: "Ghost",
        t2: "Flying",
        bs: {
            hp: 75,
            at: 70,
            df: 70,
            sa: 98,
            sd: 70,
            sp: 93
        },
        w: 3.4
    },
    Cutiefly: {
        t1: "Bug",
        t2: "Fairy",
        bs: {
            hp: 40,
            at: 45,
            df: 40,
            sa: 55,
            sd: 40,
            sp: 84
        },
        w: 0.2
    },
    Ribombee: {
        t1: "Bug",
        t2: "Fairy",
        bs: {
            hp: 60,
            at: 55,
            df: 60,
            sa: 95,
            sd: 70,
            sp: 124
        },
        w: 0.5
    },
    Rockruff: {
        t1: "Rock",
        bs: {
            hp: 45,
            at: 65,
            df: 40,
            sa: 30,
            sd: 40,
            sp: 60
        },
        w: 9.2
    },
    "Lycanroc-Midday": {
        t1: "Rock",
        bs: {
            hp: 75,
            at: 115,
            df: 65,
            sa: 55,
            sd: 65,
            sp: 112
        },
        w: 25.0
    },
    "Lycanroc-Midnight": {
        t1: "Rock",
        bs: {
            hp: 85,
            at: 115,
            df: 75,
            sa: 55,
            sd: 75,
            sp: 82
        },
        w: 25.0
    },
    Wishiwashi: {
        t1: "Water",
        bs: {
            hp: 45,
            at: 140,
            df: 130,
            sa: 140,
            sd: 135,
            sp: 30
        },
        w: 78.6,
        formes: ["Wishiwashi-School", "Wishiwashi-Solo"]
    },
    "Wishiwashi-School": {
        t1: "Water",
        bs: {
            hp: 45,
            at: 140,
            df: 130,
            sa: 140,
            sd: 135,
            sp: 30
        },
        w: 78.6,
        isAlternateForme: true
    },
    "Wishiwashi-Solo": {
        t1: "Water",
        bs: {
            hp: 45,
            at: 20,
            df: 20,
            sa: 25,
            sd: 25,
            sp: 40
        },
        w: 0.3,
        isAlternateForme: true
    },
    Mareanie: {
        t1: "Poison",
        t2: "Water",
        bs: {
            hp: 50,
            at: 53,
            df: 62,
            sa: 43,
            sd: 52,
            sp: 45
        },
        w: 8.0
    },
    Toxapex: {
        t1: "Poison",
        t2: "Water",
        bs: {
            hp: 50,
            at: 63,
            df: 152,
            sa: 53,
            sd: 142,
            sp: 35
        },
        w: 14.5
    },
    Mudbray: {
        t1: "Ground",
        bs: {
            hp: 70,
            at: 100,
            df: 70,
            sa: 45,
            sd: 55,
            sp: 45
        },
        w: 110.0
    },
    Mudsdale: {
        t1: "Ground",
        bs: {
            hp: 100,
            at: 125,
            df: 100,
            sa: 55,
            sd: 85,
            sp: 35
        },
        w: 920.0
    },
    Dewpider: {
        t1: "Water",
        t2: "Bug",
        bs: {
            hp: 38,
            at: 40,
            df: 52,
            sa: 40,
            sd: 72,
            sp: 27
        },
        w: 4.0
    },
    Araquanid: {
        t1: "Water",
        t2: "Bug",
        bs: {
            hp: 68,
            at: 70,
            df: 92,
            sa: 50,
            sd: 132,
            sp: 42
        },
        ab: "Water Bubble",
        w: 82.0
    },
    Fomantis: {
        t1: "Grass",
        bs: {
            hp: 40,
            at: 55,
            df: 35,
            sa: 50,
            sd: 35,
            sp: 35
        },
        w: 1.5
    },
    Lurantis: {
        t1: "Grass",
        bs: {
            hp: 70,
            at: 105,
            df: 90,
            sa: 80,
            sd: 90,
            sp: 45
        },
        w: 19.5
    },
    Morelull: {
        t1: "Grass",
        t2: "Fairy",
        bs: {
            hp: 40,
            at: 35,
            df: 55,
            sa: 65,
            sd: 75,
            sp: 15
        },
        w: 1.5
    },
    Shiinotic: {
        t1: "Grass",
        t2: "Fairy",
        bs: {
            hp: 60,
            at: 45,
            df: 80,
            sa: 90,
            sd: 100,
            sp: 30
        },
        w: 11.5
    },
    Salandit: {
        t1: "Poison",
        t2: "Fire",
        bs: {
            hp: 48,
            at: 44,
            df: 40,
            sa: 71,
            sd: 40,
            sp: 77
        },
        w: 4.8
    },
    Salazzle: {
        t1: "Poison",
        t2: "Fire",
        bs: {
            hp: 68,
            at: 64,
            df: 60,
            sa: 111,
            sd: 60,
            sp: 117
        },
        w: 22.2
    },
    Stufful: {
        t1: "Normal",
        t2: "Fighting",
        bs: {
            hp: 70,
            at: 75,
            df: 50,
            sa: 45,
            sd: 50,
            sp: 50
        },
        ab: "Fluffy",
        w: 6.8
    },
    Bewear: {
        t1: "Normal",
        t2: "Fighting",
        bs: {
            hp: 120,
            at: 125,
            df: 80,
            sa: 55,
            sd: 60,
            sp: 60
        },
        ab: "Fluffy",
        w: 135.0
    },
    Bounsweet: {
        t1: "Grass",
        bs: {
            hp: 42,
            at: 30,
            df: 38,
            sa: 30,
            sd: 38,
            sp: 32
        },
        w: 3.2
    },
    Steenee: {
        t1: "Grass",
        bs: {
            hp: 52,
            at: 40,
            df: 48,
            sa: 40,
            sd: 48,
            sp: 62
        },
        w: 8.2
    },
    Tsareena: {
        t1: "Grass",
        bs: {
            hp: 72,
            at: 120,
            df: 98,
            sa: 50,
            sd: 98,
            sp: 72
        },
        w: 21.4
    },
    Comfey: {
        t1: "Fairy",
        bs: {
            hp: 51,
            at: 52,
            df: 90,
            sa: 82,
            sd: 110,
            sp: 100
        },
        w: 0.3
    },
    Oranguru: {
        t1: "Normal",
        t2: "Psychic",
        bs: {
            hp: 90,
            at: 60,
            df: 80,
            sa: 90,
            sd: 110,
            sp: 60
        },
        w: 76.0
    },
    Passimian: {
        t1: "Fighting",
        bs: {
            hp: 100,
            at: 120,
            df: 90,
            sa: 40,
            sd: 60,
            sp: 80
        },
        w: 82.8
    },
    Wimpod: {
        t1: "Bug",
        t2: "Water",
        bs: {
            hp: 25,
            at: 35,
            df: 40,
            sa: 20,
            sd: 30,
            sp: 80
        },
        w: 12.0
    },
    Golisopod: {
        t1: "Bug",
        t2: "Water",
        bs: {
            hp: 75,
            at: 125,
            df: 140,
            sa: 60,
            sd: 90,
            sp: 40
        },
        w: 108.0
    },
    Sandygast: {
        t1: "Ghost",
        t2: "Ground",
        bs: {
            hp: 55,
            at: 55,
            df: 80,
            sa: 70,
            sd: 45,
            sp: 15
        },
        w: 70.0
    },
    Palossand: {
        t1: "Ghost",
        t2: "Ground",
        bs: {
            hp: 85,
            at: 75,
            df: 110,
            sa: 100,
            sd: 75,
            sp: 35
        },
        w: 250.0
    },
    Pyukumuku: {
        t1: "Water",
        bs: {
            hp: 55,
            at: 60,
            df: 130,
            sa: 30,
            sd: 130,
            sp: 5
        },
        w: 1.2
    },
    "Type: Null": {
        t1: "Normal",
        bs: {
            hp: 95,
            at: 95,
            df: 95,
            sa: 95,
            sd: 95,
            sp: 59
        },
        w: 120.5
    },
    Silvally: {
        t1: "Normal",
        bs: {
            hp: 95,
            at: 95,
            df: 95,
            sa: 95,
            sd: 95,
            sp: 95
        },
        w: 100.5
    },
    Minior: {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 60,
            df: 100,
            sa: 60,
            sd: 100,
            sp: 60
        },
        w: 40.0,
        formes: ["Minior-Up", "Minior-Down"]
    },
    "Minior-Up": {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 60,
            df: 100,
            sa: 60,
            sd: 100,
            sp: 60
        },
        w: 40.0,
        isAlternateForme: true
    },
    "Minior-Down": {
        t1: "Rock",
        t2: "Flying",
        bs: {
            hp: 60,
            at: 100,
            df: 60,
            sa: 100,
            sd: 60,
            sp: 120
        },
        w: 0.3,
        isAlternateForme: true
    },
    Komala: {
        t1: "Normal",
        bs: {
            hp: 65,
            at: 115,
            df: 65,
            sa: 75,
            sd: 95,
            sp: 65
        },
        w: 19.9
    },
    Turtonator: {
        t1: "Fire",
        t2: "Dragon",
        bs: {
            hp: 60,
            at: 78,
            df: 135,
            sa: 91,
            sd: 85,
            sp: 36
        },
        w: 212.0
    },
    Togedemaru: {
        t1: "Electric",
        t2: "Steel",
        bs: {
            hp: 65,
            at: 98,
            df: 63,
            sa: 40,
            sd: 73,
            sp: 96
        },
        w: 3.3
    },
    Mimikyu: {
        t1: "Ghost",
        t2: "Fairy",
        bs: {
            hp: 55,
            at: 90,
            df: 80,
            sa: 50,
            sd: 105,
            sp: 96
        },
        w: 0.7
    },
    Bruxish: {
        t1: "Water",
        t2: "Psychic",
        bs: {
            hp: 68,
            at: 105,
            df: 70,
            sa: 70,
            sd: 70,
            sp: 92
        },
        w: 19.0
    },
    Drampa: {
        t1: "Normal",
        t2: "Dragon",
        bs: {
            hp: 78,
            at: 60,
            df: 85,
            sa: 135,
            sd: 91,
            sp: 36
        },
        w: 185.0
    },
    Dhelmise: {
        t1: "Ghost",
        t2: "Grass",
        bs: {
            hp: 70,
            at: 131,
            df: 100,
            sa: 86,
            sd: 90,
            sp: 40
        },
        w: 210.0
    },
    "Jangmo-o": {
        t1: "Dragon",
        bs: {
            hp: 45,
            at: 55,
            df: 65,
            sa: 45,
            sd: 45,
            sp: 45
        },
        w: 29.7
    },
    "Hakamo-o": {
        t1: "Dragon",
        t2: "Fighting",
        bs: {
            hp: 55,
            at: 75,
            df: 90,
            sa: 65,
            sd: 70,
            sp: 65
        },
        w: 47.0
    },
    "Kommo-o": {
        t1: "Dragon",
        t2: "Fighting",
        bs: {
            hp: 75,
            at: 110,
            df: 125,
            sa: 100,
            sd: 105,
            sp: 85
        },
        w: 78.2
    },
    "Tapu Koko": {
        t1: "Electric",
        t2: "Fairy",
        bs: {
            hp: 70,
            at: 115,
            df: 85,
            sa: 95,
            sd: 75,
            sp: 130
        },
        ab: "Electric Surge",
        w: 20.5
    },
    "Tapu Lele": {
        t1: "Psychic",
        t2: "Fairy",
        bs: {
            hp: 70,
            at: 85,
            df: 75,
            sa: 130,
            sd: 115,
            sp: 95
        },
        ab: "Psychic Surge",
        w: 18.6
    },
    "Tapu Bulu": {
        t1: "Grass",
        t2: "Fairy",
        bs: {
            hp: 70,
            at: 130,
            df: 115,
            sa: 85,
            sd: 95,
            sp: 75
        },
        ab: "Grassy Surge",
        w: 45.5
    },
    "Tapu Fini": {
        t1: "Water",
        t2: "Fairy",
        bs: {
            hp: 70,
            at: 75,
            df: 115,
            sa: 95,
            sd: 130,
            sp: 85
        },
        ab: "Misty Surge",
        w: 21.2
    },
    Cosmog: {
        t1: "Psychic",
        bs: {
            hp: 43,
            at: 29,
            df: 31,
            sa: 29,
            sd: 31,
            sp: 37
        },
        w: 0.1
    },
    Cosmoem: {
        t1: "Psychic",
        bs: {
            hp: 43,
            at: 29,
            df: 131,
            sa: 29,
            sd: 131,
            sp: 37
        },
        w: 999.9
    },
    Solgaleo: {
        t1: "Psychic",
        t2: "Steel",
        bs: {
            hp: 137,
            at: 137,
            df: 107,
            sa: 113,
            sd: 89,
            sp: 97
        },
        ab: "Full Metal Body",
        w: 230.0
    },
    Lunala: {
        t1: "Psychic",
        t2: "Ghost",
        bs: {
            hp: 137,
            at: 113,
            df: 89,
            sa: 137,
            sd: 107,
            sp: 97
        },
        ab: "Shadow Shield",
        w: 120.0
    },
    Nihilego: {
        t1: "Rock",
        t2: "Poison",
        bs: {
            hp: 109,
            at: 53,
            df: 47,
            sa: 127,
            sd: 131,
            sp: 103
        },
        w: 55.5
    },
    Buzzwole: {
        t1: "Bug",
        t2: "Fighting",
        bs: {
            hp: 107,
            at: 139,
            df: 139,
            sa: 53,
            sd: 53,
            sp: 79
        },
        w: 333.6
    },
    Pheromosa: {
        t1: "Bug",
        t2: "Fighting",
        bs: {
            hp: 71,
            at: 137,
            df: 37,
            sa: 137,
            sd: 37,
            sp: 151
        },
        w: 25.0
    },
    Xurkitree: {
        t1: "Electric",
        bs: {
            hp: 83,
            at: 89,
            df: 71,
            sa: 173,
            sd: 71,
            sp: 83
        },
        w: 100.0
    },
    Celesteela: {
        t1: "Steel",
        t2: "Flying",
        bs: {
            hp: 97,
            at: 101,
            df: 103,
            sa: 107,
            sd: 101,
            sp: 61
        },
        w: 999.9
    },
    Kartana: {
        t1: "Grass",
        t2: "Steel",
        bs: {
            hp: 59,
            at: 181,
            df: 131,
            sa: 59,
            sd: 31,
            sp: 109
        },
        w: 0.1
    },
    Guzzlord: {
        t1: "Dark",
        t2: "Dragon",
        bs: {
            hp: 223,
            at: 101,
            df: 53,
            sa: 97,
            sd: 53,
            sp: 43
        },
        w: 888.0
    },
    Necrozma: {
        t1: "Psychic",
        bs: {
            hp: 97,
            at: 107,
            df: 101,
            sa: 127,
            sd: 89,
            sp: 79
        },
        w: 230.0
    },
    Magearna: {
        t1: "Steel",
        t2: "Fairy",
        bs: {
            hp: 80,
            at: 95,
            df: 115,
            sa: 130,
            sd: 115,
            sp: 65
        },
        w: 80.5
    },
    Marshadow: {
        t1: "Fighting",
        t2: "Ghost",
        bs: {
            hp: 90,
            at: 125,
            df: 80,
            sa: 90,
            sd: 90,
            sp: 125
        },
        w: 22.2
    },
    Poiple: {
        t1: "Poison",
        bs: {
            hp: 67,
            at: 73,
            df: 67,
            sa: 73,
            sd: 67,
            sp: 73
        },
        w: 1.8
    },
    Naganadel: {
        t1: "Poison",
        t2: "Dragon",
        bs: {
            hp: 73,
            at: 73,
            df: 73,
            sa: 127,
            sd: 73,
            sp: 121
        },
        w: 150.0
    },
    Blacephalon: {
        t1: "Fire",
        t2: "Ghost",
        bs: {
            hp: 53,
            at: 127,
            df: 53,
            sa: 151,
            sd: 79,
            sp: 107
        },
        w: 13.0
    },
    Stakataka: {
        t1: "Rock",
        t2: "Steel",
        bs: {
            hp: 61,
            at: 131,
            df: 211,
            sa: 53,
            sd: 101,
            sp: 13
        },
        w: 820.0
    },
    "Lycanroc-Dusk": {
        t1: "Rock",
        bs: {
            hp: 75,
            at: 117,
            df: 65,
            sa: 55,
            sd: 65,
            sp: 110
        },
        w: 25.0,
        ab: "Tough Claws"
    },
    "Necrozma-Dawn-Wings": {
        t1: "Psychic",
        t2: "Ghost",
        bs: {
            hp: 97,
            at: 113,
            df: 109,
            sa: 157,
            sd: 127,
            sp: 77
        },
        w: 350.0,
        ab: "Prism Armor",
        formes: ["Necrozma-Dawn-Wings", "Ultra Necrozma"]
    },
    "Necrozma-Dusk-Mane": {
        t1: "Psychic",
        t2: "Steel",
        bs: {
            hp: 97,
            at: 157,
            df: 127,
            sa: 113,
            sd: 109,
            sp: 77
        },
        w: 460.0,
        ab: "Prism Armor",
        formes: ["Necrozma-Dusk-Mane", "Ultra Necrozma"]
    },
    "Ultra Necrozma": {
        t1: "Psychic",
        t2: "Dragon",
        bs: {
            hp: 97,
            at: 167,
            df: 97,
            sa: 167,
            sd: 97,
            sp: 129
        },
        w: 230.0,
        ab: "Neuroforce",
        isAlternateForme: true
    },
    Zeraora: {
        t1: "Electric",
        bs: {
            hp: 88,
            at: 112,
            df: 75,
            sa: 102,
            sd: 80,
            sp: 143
        },
        w: 44.5,
        ab: "Volt Absorb"
    }
};
const GEN_8 = {
    Meltan: {
        t1: "Steel",
        bs: {
            hp: 46,
            at: 65,
            df: 65,
            sa: 55,
            sd: 35,
            sp: 34
        },
        w: 8
    },
    Melmetal: {
        t1: "Steel",
        bs: {
            hp: 135,
            at: 143,
            df: 143,
            sa: 80,
            sd: 65,
            sp: 34
        },
        w: 800
    },
    "Meowth-Galar": {
        t1: "Steel",
        bs: {
            hp: 50,
            at: 65,
            df: 55,
            sa: 40,
            sd: 40,
            sp: 40
        },
        w: 7.5,
        ab: "Tough Claws"
    },
    "Ponyta-Galar": {
        t1: "Psychic",
        bs: {
            hp: 50,
            at: 85,
            df: 55,
            sa: 65,
            sd: 65,
            sp: 90
        },
        w: 24
    },
    "Rapidash-Galar": {
        t1: "Psychic",
        t2: "Fairy",
        bs: {
            hp: 65,
            at: 100,
            df: 70,
            sa: 80,
            sd: 80,
            sp: 105
        },
        w: 80
    },
    "Farfetch'd-Galar": {
        t1: "Fighting",
        bs: {
            hp: 52,
            at: 95,
            df: 55,
            sa: 58,
            sd: 62,
            sp: 55
        },
        w: 15,
        ab: "Scrappy"
    },
    "Weezing-Galar": {
        t1: "Poison",
        t2: "Fairy",
        bs: {
            hp: 65,
            at: 90,
            df: 120,
            sa: 85,
            sd: 70,
            sp: 60
        },
        w: 16
    },
    "Mr. Mime-Galar": {
        t1: "Ice",
        t2: "Psychic",
        bs: {
            hp: 50,
            at: 65,
            df: 65,
            sa: 90,
            sd: 90,
            sp: 100
        },
        w: 56.8
    },
    "Corsola-Galar": {
        t1: "Ghost",
        bs: {
            hp: 60,
            at: 55,
            df: 100,
            sa: 65,
            sd: 100,
            sp: 30
        },
        w: 0.5
    },
    "Zigzagoon-Galar": {
        t1: "Dark",
        t2: "Normal",
        bs: {
            hp: 38,
            at: 30,
            df: 41,
            sa: 30,
            sd: 41,
            sp: 60
        },
        w: 17.5
    },
    "Linoone-Galar": {
        t1: "Dark",
        t2: "Normal",
        bs: {
            hp: 78,
            at: 70,
            df: 61,
            sa: 50,
            sd: 61,
            sp: 100
        },
        w: 32.5
    },
    "Darumaka-Galar": {
        t1: "Ice",
        bs: {
            hp: 70,
            at: 90,
            df: 45,
            sa: 15,
            sd: 45,
            sp: 50
        },
        w: 40
    },
    "Darmanitan-Galar": {
        t1: "Ice",
        bs: {
            hp: 105,
            at: 140,
            df: 55,
            sa: 30,
            sd: 55,
            sp: 95
        },
        w: 120,
        ab: "Gorilla Tactics",
        formes: ["Darmanitan-Galar", "Darmanitan-Z-Galar"]
    },
    "Darmanitan-Z-Galar": {
        t1: "Ice",
        t2: "Fire",
        bs: {
            hp: 105,
            at: 160,
            df: 55,
            sa: 30,
            sd: 55,
            sp: 135
        },
        w: 120,
        ab: "Zen Mode",
        isAlternateForme: true
    },
    "Yamask-Galar": {
        t1: "Ground",
        t2: "Ghost",
        bs: {
            hp: 38,
            at: 55,
            df: 85,
            sa: 30,
            sd: 65,
            sp: 30
        },
        w: 1.5
    },
    "Stunfisk-Galar": {
        t1: "Ground",
        t2: "Steel",
        bs: {
            hp: 109,
            at: 81,
            df: 99,
            sa: 66,
            sd: 84,
            sp: 32
        },
        w: 20.5
    },
    Grookey: {
        t1: "Grass",
        bs: {
            hp: 50,
            at: 65,
            df: 50,
            sa: 40,
            sd: 40,
            sp: 65
        },
        w: 5,
        ab: "Overgrow"
    },
    Thwackey: {
        t1: "Grass",
        bs: {
            hp: 70,
            at: 85,
            df: 70,
            sa: 55,
            sd: 60,
            sp: 80
        },
        w: 14,
        ab: "Overgrow"
    },
    Rillaboom: {
        t1: "Grass",
        bs: {
            hp: 100,
            at: 125,
            df: 90,
            sa: 60,
            sd: 70,
            sp: 85
        },
        w: 90,
        ab: "Overgrow"
    },
    Scorbunny: {
        t1: "Fire",
        bs: {
            hp: 50,
            at: 71,
            df: 40,
            sa: 40,
            sd: 40,
            sp: 69
        },
        w: 4.5,
        ab: "Blaze"
    },
    Raboot: {
        t1: "Fire",
        bs: {
            hp: 65,
            at: 86,
            df: 60,
            sa: 55,
            sd: 60,
            sp: 94
        },
        w: 9,
        ab: "Blaze"
    },
    Cinderace: {
        t1: "Fire",
        bs: {
            hp: 80,
            at: 116,
            df: 75,
            sa: 65,
            sd: 75,
            sp: 119
        },
        w: 33,
        ab: "Blaze"
    },
    Sobble: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 40,
            df: 40,
            sa: 70,
            sd: 40,
            sp: 70
        },
        w: 4,
        ab: "Torrent"
    },
    Drizzile: {
        t1: "Water",
        bs: {
            hp: 65,
            at: 60,
            df: 55,
            sa: 95,
            sd: 55,
            sp: 90
        },
        w: 11.5,
        ab: "Torrent"
    },
    Inteleon: {
        t1: "Water",
        bs: {
            hp: 70,
            at: 85,
            df: 65,
            sa: 125,
            sd: 65,
            sp: 120
        },
        w: 45.2,
        ab: "Torrent"
    },
    Blipbug: {
        t1: "Bug",
        bs: {
            hp: 25,
            at: 20,
            df: 20,
            sa: 25,
            sd: 45,
            sp: 45
        },
        w: 8
    },
    Dottler: {
        t1: "Bug",
        t2: "Psychic",
        bs: {
            hp: 50,
            at: 35,
            df: 80,
            sa: 50,
            sd: 90,
            sp: 30
        },
        w: 19.5
    },
    Orbeetle: {
        t1: "Bug",
        t2: "Psychic",
        bs: {
            hp: 60,
            at: 45,
            df: 110,
            sa: 80,
            sd: 120,
            sp: 90
        },
        w: 40.8
    },
    Rookidee: {
        t1: "Flying",
        bs: {
            hp: 38,
            at: 47,
            df: 35,
            sa: 33,
            sd: 35,
            sp: 57
        },
        w: 1.8
    },
    Corvisquire: {
        t1: "Flying",
        bs: {
            hp: 68,
            at: 67,
            df: 55,
            sa: 43,
            sd: 55,
            sp: 77
        },
        w: 16
    },
    Corviknight: {
        t1: "Flying",
        t2: "Steel",
        bs: {
            hp: 98,
            at: 87,
            df: 105,
            sa: 53,
            sd: 85,
            sp: 67
        },
        w: 75
    },
    Skwovet: {
        t1: "Normal",
        bs: {
            hp: 70,
            at: 55,
            df: 55,
            sa: 35,
            sd: 35,
            sp: 25
        },
        w: 2.5
    },
    Greedent: {
        t1: "Normal",
        bs: {
            hp: 120,
            at: 95,
            df: 95,
            sa: 55,
            sd: 75,
            sp: 20
        },
        w: 6
    },
    Nickit: {
        t1: "Dark",
        bs: {
            hp: 40,
            at: 28,
            df: 28,
            sa: 47,
            sd: 52,
            sp: 50
        },
        w: 8.9
    },
    Thievul: {
        t1: "Dark",
        bs: {
            hp: 70,
            at: 58,
            df: 58,
            sa: 87,
            sd: 92,
            sp: 90
        },
        w: 19.9
    },
    Gossifleur: {
        t1: "Grass",
        bs: {
            hp: 40,
            at: 40,
            df: 60,
            sa: 40,
            sd: 60,
            sp: 10
        },
        w: 2.2
    },
    Eldegoss: {
        t1: "Grass",
        bs: {
            hp: 60,
            at: 50,
            df: 90,
            sa: 80,
            sd: 120,
            sp: 60
        },
        w: 2.5
    },
    Wooloo: {
        t1: "Normal",
        bs: {
            hp: 42,
            at: 40,
            df: 55,
            sa: 40,
            sd: 45,
            sp: 48
        },
        w: 6,
        ab: "Fluffy"
    },
    Dubwool: {
        t1: "Normal",
        bs: {
            hp: 72,
            at: 80,
            df: 100,
            sa: 60,
            sd: 90,
            sp: 88
        },
        w: 43,
        ab: "Fluffy"
    },
    Chewtle: {
        t1: "Water",
        bs: {
            hp: 50,
            at: 64,
            df: 50,
            sa: 38,
            sd: 38,
            sp: 44
        },
        w: 8.5,
        ab: "Strong Jaw"
    },
    Drednaw: {
        t1: "Water",
        t2: "Rock",
        bs: {
            hp: 90,
            at: 115,
            df: 90,
            sa: 48,
            sd: 68,
            sp: 74
        },
        w: 8.5,
        ab: "Strong Jaw"
    },
    Yamper: {
        t1: "Electric",
        bs: {
            hp: 59,
            at: 45,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 26
        },
        w: 13.5
    },
    Boltund: {
        t1: "Electric",
        bs: {
            hp: 69,
            at: 90,
            df: 60,
            sa: 90,
            sd: 60,
            sp: 121
        },
        w: 34
    },
    Rolycoly: {
        t1: "Rock",
        bs: {
            hp: 30,
            at: 40,
            df: 50,
            sa: 40,
            sd: 50,
            sp: 30
        },
        w: 12
    },
    Carkol: {
        t1: "Rock",
        t2: "Fire",
        bs: {
            hp: 80,
            at: 60,
            df: 90,
            sa: 60,
            sd: 70,
            sp: 50
        },
        w: 78
    },
    Coalossal: {
        t1: "Rock",
        t2: "Fire",
        bs: {
            hp: 110,
            at: 80,
            df: 120,
            sa: 80,
            sd: 90,
            sp: 30
        },
        w: 310.5
    },
    Applin: {
        t1: "Grass",
        t2: "Dragon",
        bs: {
            hp: 40,
            at: 40,
            df: 80,
            sa: 40,
            sd: 40,
            sp: 20
        },
        w: 0.5
    },
    Flapple: {
        t1: "Grass",
        t2: "Dragon",
        bs: {
            hp: 70,
            at: 110,
            df: 80,
            sa: 95,
            sd: 60,
            sp: 70
        },
        w: 1
    },
    Appletun: {
        t1: "Grass",
        t2: "Dragon",
        bs: {
            hp: 110,
            at: 85,
            df: 80,
            sa: 100,
            sd: 80,
            sp: 30
        },
        w: 13
    },
    Silicobra: {
        t1: "Ground",
        bs: {
            hp: 52,
            at: 57,
            df: 75,
            sa: 35,
            sd: 50,
            sp: 46
        },
        w: 7.6,
        ab: "Sand Spit"
    },
    Sandaconda: {
        t1: "Ground",
        bs: {
            hp: 72,
            at: 107,
            df: 125,
            sa: 65,
            sd: 70,
            sp: 71
        },
        w: 65.5,
        ab: "Sand Spit"
    },
    Cramorant: {
        t1: "Flying",
        t2: "Water",
        bs: {
            hp: 70,
            at: 85,
            df: 55,
            sa: 85,
            sd: 95,
            sp: 85
        },
        w: 18
    },
    Arrokuda: {
        t1: "Water",
        bs: {
            hp: 41,
            at: 63,
            df: 40,
            sa: 40,
            sd: 30,
            sp: 66
        },
        w: 1
    },
    Barraskewda: {
        t1: "Water",
        bs: {
            hp: 61,
            at: 123,
            df: 60,
            sa: 60,
            sd: 50,
            sp: 136
        },
        w: 30
    },
    Toxel: {
        t1: "Electric",
        t2: "Poison",
        bs: {
            hp: 40,
            at: 38,
            df: 35,
            sa: 54,
            sd: 35,
            sp: 40
        },
        w: 11
    },
    Toxtricity: {
        t1: "Electric",
        t2: "Poison",
        bs: {
            hp: 75,
            at: 98,
            df: 70,
            sa: 114,
            sd: 70,
            sp: 75
        },
        w: 40
    },
    Sizzlipede: {
        t1: "Fire",
        t2: "Bug",
        bs: {
            hp: 50,
            at: 65,
            df: 45,
            sa: 50,
            sd: 50,
            sp: 45
        },
        w: 1,
        ab: "White Smoke"
    },
    Centiskorch: {
        t1: "Fire",
        t2: "Bug",
        bs: {
            hp: 100,
            at: 115,
            df: 65,
            sa: 90,
            sd: 90,
            sp: 65
        },
        w: 120,
        ab: "White Smoke"
    },
    Clobbopus: {
        t1: "Fighting",
        bs: {
            hp: 50,
            at: 68,
            df: 60,
            sa: 50,
            sd: 50,
            sp: 32
        },
        w: 4,
        ab: "Technician"
    },
    Grapploct: {
        t1: "Fighting",
        bs: {
            hp: 80,
            at: 118,
            df: 90,
            sa: 70,
            sd: 80,
            sp: 42
        },
        w: 39,
        ab: "Technician"
    },
    Sinistea: {
        t1: "Ghost",
        bs: {
            hp: 40,
            at: 45,
            df: 45,
            sa: 74,
            sd: 54,
            sp: 50
        },
        w: 0.2
    },
    Polteageist: {
        t1: "Ghost",
        bs: {
            hp: 60,
            at: 65,
            df: 65,
            sa: 134,
            sd: 114,
            sp: 70
        },
        w: 0.4
    },
    Hatenna: {
        t1: "Psychic",
        bs: {
            hp: 42,
            at: 30,
            df: 45,
            sa: 56,
            sd: 53,
            sp: 39
        },
        w: 3.4
    },
    Hattrem: {
        t1: "Psychic",
        bs: {
            hp: 57,
            at: 40,
            df: 65,
            sa: 86,
            sd: 73,
            sp: 49
        },
        w: 4.8
    },
    Hatterene: {
        t1: "Psychic",
        t2: "Fairy",
        bs: {
            hp: 57,
            at: 90,
            df: 95,
            sa: 136,
            sd: 103,
            sp: 29
        },
        w: 5.1
    },
    Impidimp: {
        t1: "Dark",
        t2: "Fairy",
        bs: {
            hp: 45,
            at: 45,
            df: 30,
            sa: 55,
            sd: 40,
            sp: 50
        },
        w: 5.5
    },
    Morgrem: {
        t1: "Dark",
        t2: "Fairy",
        bs: {
            hp: 65,
            at: 60,
            df: 45,
            sa: 75,
            sd: 55,
            sp: 70
        },
        w: 12.5
    },
    Grimmsnarl: {
        t1: "Dark",
        t2: "Fairy",
        bs: {
            hp: 95,
            at: 120,
            df: 65,
            sa: 95,
            sd: 75,
            sp: 60
        },
        w: 61
    },
    Obstagoon: {
        t1: "Dark",
        t2: "Normal",
        bs: {
            hp: 93,
            at: 90,
            df: 101,
            sa: 60,
            sd: 81,
            sp: 95
        },
        w: 46,
        ab: "Defiant"
    },
    Perrserker: {
        t1: "Steel",
        bs: {
            hp: 70,
            at: 110,
            df: 100,
            sa: 50,
            sd: 60,
            sp: 50
        },
        w: 28,
        ab: "Tough Claws"
    },
    Cursola: {
        t1: "Ghost",
        bs: {
            hp: 60,
            at: 95,
            df: 50,
            sa: 145,
            sd: 130,
            sp: 30
        },
        w: 0.4
    },
    "Sirfetch'd": {
        t1: "Fighting",
        bs: {
            hp: 62,
            at: 135,
            df: 95,
            sa: 68,
            sd: 82,
            sp: 65
        },
        w: 117,
        ab: "Scrappy"
    },
    "Mr. Rime": {
        t1: "Ice",
        t2: "Psychic",
        bs: {
            hp: 80,
            at: 85,
            df: 75,
            sa: 110,
            sd: 100,
            sp: 70
        },
        w: 58.2
    },
    Runerigus: {
        t1: "Ground",
        t2: "Ghost",
        bs: {
            hp: 58,
            at: 95,
            df: 145,
            sa: 50,
            sd: 105,
            sp: 30
        },
        w: 66.6
    },
    Milcery: {
        t1: "Fairy",
        bs: {
            hp: 45,
            at: 40,
            df: 40,
            sa: 50,
            sd: 61,
            sp: 34
        },
        w: 0.3
    },
    Alcremie: {
        t1: "Fairy",
        bs: {
            hp: 65,
            at: 60,
            df: 75,
            sa: 110,
            sd: 121,
            sp: 64
        },
        w: 0.5
    },
    Falinks: {
        t1: "Fighting",
        bs: {
            hp: 65,
            at: 100,
            df: 100,
            sa: 70,
            sd: 60,
            sp: 75
        },
        w: 62,
        ab: "Defiant"
    },
    Pincurchin: {
        t1: "Electric",
        bs: {
            hp: 48,
            at: 101,
            df: 95,
            sa: 91,
            sd: 85,
            sp: 15
        },
        w: 1,
        ab: "Lightning Rod"
    },
    Snom: {
        t1: "Ice",
        t2: "Bug",
        bs: {
            hp: 30,
            at: 25,
            df: 35,
            sa: 45,
            sd: 30,
            sp: 20
        },
        w: 3.8,
        ab: "Ice Scales"
    },
    Frosmoth: {
        t1: "Ice",
        t2: "Bug",
        bs: {
            hp: 70,
            at: 65,
            df: 60,
            sa: 125,
            sd: 90,
            sp: 65
        },
        w: 42,
        ab: "Ice Scales"
    },
    Stonjourner: {
        t1: "Rock",
        bs: {
            hp: 100,
            at: 125,
            df: 135,
            sa: 20,
            sd: 20,
            sp: 70
        },
        w: 520
    },
    Eiscue: {
        t1: "Ice",
        bs: {
            hp: 75,
            at: 80,
            df: 110,
            sa: 65,
            sd: 90,
            sp: 50
        },
        w: 89
    },
    "Eiscue-Noice": {
        t1: "Ice",
        bs: {
            hp: 75,
            at: 80,
            df: 70,
            sa: 65,
            sd: 50,
            sp: 130
        },
        w: 89
    },
    "Indeedee-M": {
        t1: "Psychic",
        t2: "Normal",
        bs: {
            hp: 60,
            at: 65,
            df: 55,
            sa: 105,
            sd: 95,
            sp: 95
        },
        w: 28,
        ab: "Psychic Surge"
    },
    "Indeedee-F": {
        t1: "Psychic",
        t2: "Normal",
        bs: {
            hp: 70,
            at: 55,
            df: 65,
            sa: 95,
            sd: 105,
            sp: 85
        },
        w: 28,
        ab: "Psychic Surge"
    },
    Morpeko: {
        t1: "Electric",
        t2: "Dark",
        bs: {
            hp: 58,
            at: 95,
            df: 58,
            sa: 70,
            sd: 58,
            sp: 97
        },
        w: 3,
        formes: ["Morpeko", "Morpeko-Hangry"]
    },
    "Morpeko-Hangry": {
        t1: "Electric",
        t2: "Dark",
        bs: {
            hp: 58,
            at: 95,
            df: 58,
            sa: 70,
            sd: 58,
            sp: 97
        },
        w: 3,
        isAlternateForme: true
    },
    Cufant: {
        t1: "Steel",
        bs: {
            hp: 72,
            at: 80,
            df: 49,
            sa: 40,
            sd: 49,
            sp: 40
        },
        w: 100,
        ab: "Sheer Force"
    },
    Copperajah: {
        t1: "Steel",
        bs: {
            hp: 122,
            at: 130,
            df: 69,
            sa: 80,
            sd: 69,
            sp: 30
        },
        w: 650,
        ab: "Sheer Force"
    },
    Dracozolt: {
        t1: "Electric",
        t2: "Dragon",
        bs: {
            hp: 90,
            at: 100,
            df: 90,
            sa: 80,
            sd: 70,
            sp: 75
        },
        w: 190,
        ab: "Volt Absorb"
    },
    Arctozolt: {
        t1: "Electric",
        t2: "Ice",
        bs: {
            hp: 90,
            at: 100,
            df: 90,
            sa: 90,
            sd: 80,
            sp: 55
        },
        w: 150,
        ab: "Volt Absorb"
    },
    Dracovish: {
        t1: "Water",
        t2: "Dragon",
        bs: {
            hp: 90,
            at: 90,
            df: 100,
            sa: 70,
            sd: 80,
            sp: 75
        },
        w: 215,
        ab: "Strong Jaw"
    },
    Arctovish: {
        t1: "Water",
        t2: "Ice",
        bs: {
            hp: 90,
            at: 90,
            df: 100,
            sa: 80,
            sd: 90,
            sp: 55
        },
        w: 215,
        ab: "Water Absorb"
    },
    Duraludon: {
        t1: "Steel",
        t2: "Dragon",
        bs: {
            hp: 70,
            at: 95,
            df: 115,
            sa: 120,
            sd: 50,
            sp: 85
        },
        w: 40
    },
    Dreepy: {
        t1: "Dragon",
        t2: "Ghost",
        bs: {
            hp: 28,
            at: 60,
            df: 30,
            sa: 40,
            sd: 30,
            sp: 82
        },
        w: 2,
        ab: "Clear Body"
    },
    Drakloak: {
        t1: "Dragon",
        t2: "Ghost",
        bs: {
            hp: 68,
            at: 80,
            df: 50,
            sa: 60,
            sd: 50,
            sp: 102
        },
        w: 11,
        ab: "Clear Body"
    },
    Dragapult: {
        t1: "Dragon",
        t2: "Ghost",
        bs: {
            hp: 88,
            at: 120,
            df: 75,
            sa: 100,
            sd: 75,
            sp: 142
        },
        w: 50,
        ab: "Clear Body"
    },
    Zacian: {
        t1: "Fairy",
        bs: {
            hp: 92,
            at: 130,
            df: 115,
            sa: 80,
            sd: 115,
            sp: 138
        },
        w: 110,
        ab: "Intrepid Sword",
        formes: ["Zacian", "Zacian-Crowned"]
    },
    "Zacian-Crowned": {
        t1: "Fairy",
        t2: "Steel",
        bs: {
            hp: 92,
            at: 170,
            df: 115,
            sa: 80,
            sd: 115,
            sp: 148
        },
        w: 355,
        ab: "Intrepid Sword",
        isAlternateForme: true
    },
    Zamazenta: {
        t1: "Fighting",
        bs: {
            hp: 92,
            at: 130,
            df: 115,
            sa: 80,
            sd: 115,
            sp: 138
        },
        w: 210,
        ab: "Dauntless Shield",
        formes: ["Zamazenta", "Zamazenta-Crowned"]
    },
    "Zamazenta-Crowned": {
        t1: "Fighting",
        t2: "Steel",
        bs: {
            hp: 92,
            at: 130,
            df: 145,
            sa: 80,
            sd: 145,
            sp: 128
        },
        w: 355,
        ab: "Dauntless Shield",
        isAlternateForme: true
    },
    Eternatus: {
        t1: "Poison",
        t2: "Dragon",
        bs: {
            hp: 140,
            at: 85,
            df: 95,
            sa: 145,
            sd: 95,
            sp: 130
        },
        w: 950
    },
    Aegislash: { bs: { df: 140, sd: 140 } },
    "Aegislash-Blade": { bs: { at: 140, sa: 140 } },
    "Aegislash-Shield": { bs: { df: 140, sd: 140 } },
    Venusaur: { formes: ["Venusaur"] },
    Charizard: { formes: ["Charizard"] },
    Blastoise: { formes: ["Blastoise"] },
    Beedrill: { formes: ["Beedrill"] },
    Pidgeot: { formes: ["Pidgeot"] },
    Alakazam: { formes: ["Alakazam"] },
    Slowbro: { formes: ["Slowbro"] },
    Gengar: { formes: ["Gengar"] },
    Kangaskhan: { formes: ["Kangaskhan"] },
    Pinsir: { formes: ["Pinsir"] },
    Gyarados: { formes: ["Gyarados"] },
    Aerodactyl: { formes: ["Aerodactyl"] },
    Mewtwo: { formes: ["Mewtwo"] },
    Ampharos: { formes: ["Ampharos"] },
    Steelix: { formes: ["Steelix"] },
    Scizor: { formes: ["Scizor"] },
    Heracross: { formes: ["Heracross"] },
    Houndoom: { formes: ["Houndoom"] },
    Tyranitar: { formes: ["Tyranitar"] },
    Sceptile: { formes: ["Sceptile"] },
    Blaziken: { formes: ["Blaziken"] },
    Swampert: { formes: ["Swampert"] },
    Gardevoir: { t2: "Fairy", formes: ["Gardevoir"] },
    Sableye: { formes: ["Sableye"] },
    Mawile: { formes: ["Mawile"] },
    Aggron: { formes: ["Aggron"] },
    Medicham: { formes: ["Medicham"] },
    Manectric: { formes: ["Manectric"] },
    Sharpedo: { formes: ["Sharpedo"] },
    Camerupt: { formes: ["Camerupt"] },
    Altaria: { formes: ["Altaria"] },
    Banette: { formes: ["Banette"] },
    Absol: { formes: ["Absol"] },
    Glalie: { formes: ["Glalie"] },
    Salamence: { formes: ["Salamence"] },
    Metagross: { formes: ["Metagross"] },
    Latias: { formes: ["Latias"] },
    Latios: { formes: ["Latios"] },
    Rayquaza: { formes: ["Rayquaza"] },
    Lopunny: { formes: ["Lopunny"] },
    Garchomp: { formes: ["Garchomp"] },
    Lucario: { formes: ["Lucario"] },
    Abomasnow: { formes: ["Abomasnow"] },
    Gallade: { formes: ["Gallade"] },
    Audino: { formes: ["Audino"] }
};
const dexData = deepmerge_1.default.all([
    GEN_1,
    GEN_2,
    GEN_3,
    GEN_4,
    GEN_5,
    GEN_6,
    GEN_7,
    GEN_8
]);
const dexDataKeys = Object.keys(dexData);
const scrapedKeys = Object.keys(output_json_1.default.pokemon);
function validateStr(str) {
    const isValid = typeof str === "string";
    const hasCharacters = str.length > 0;
    if (!isValid || !hasCharacters) {
        throw new Error("Invalid string parameter provided");
    }
}
function strCheck(query, str) {
    return str.includes(query);
}
function nameCheck(query, str) {
    return query.toLowerCase().includes(str.toLowerCase());
}
function gen8NameFilter(output, key) {
    const matchedKey = scrapedKeys.find(k => strCheck(k, key));
    const result = matchedKey ? [...output, key] : output;
    return result;
}
function getGen8Monsters(output, key) {
    const value = dexData[key];
    output[key] = value;
    return output;
}
function getScrapedMon(query) {
    const queryResult = scrapedKeys.find(k => nameCheck(query, k));
    const output = queryResult
        ? output_json_1.default.pokemon[queryResult]
        : undefined;
    return output;
}
function convertAbility(query) {
    const queryResult = vgcAbilities_1.default.find(ab => nameCheck(query, ab));
    return queryResult ? queryResult : "";
}
function getAbilities(query) {
    const queryResult = getScrapedMon(query);
    const rawAbilities = queryResult ? queryResult.abilities : [];
    const convertedAbilities = rawAbilities.map(convertAbility);
    return convertedAbilities;
}
function rotomCheck(query) {
    var _a;
    validateStr(query);
    const cases = {
        "Rotom-W": "Wash Rotom",
        "Rotom-H": "Heat Rotom",
        "Rotom-C": "Mow Rotom",
        "Rotom-S": "Fan Rotom",
        "Rotom-F": "Frost Rotom",
        Rotom: "Rotom"
    };
    return _a = cases[query], (_a !== null && _a !== void 0 ? _a : "Rotom");
}
function necrozmaCheck(query) {
    var _a;
    validateStr(query);
    const cases = {
        "Necrozma-Dusk-Mane": "Dusk Mane",
        "Necrozma-Dawn-Wings": "Dawn Wings",
        Necrozma: "Normal"
    };
    return _a = cases[query], (_a !== null && _a !== void 0 ? _a : "Normal");
}
function getForm(query) {
    let form = "Normal";
    nameCheck(query, "Alola") ? (form = "Alolan Form") : undefined;
    nameCheck(query, "Galar") ? (form = "Galarian Form") : undefined;
    nameCheck(query, "Rotom") ? (form = rotomCheck(query)) : undefined;
    nameCheck(query, "Necrozma") ? (form = necrozmaCheck(query)) : undefined;
    return form;
}
function filterForms({ name, forms, output, form }) {
    const hasForm = forms.includes(form);
    if (!hasForm) {
        return output;
    }
    return [...output, name];
}
function getMoves(query) {
    const queryResult = getScrapedMon(query);
    const rawMoves = queryResult ? queryResult.moves : {};
    const form = getForm(query);
    const moves = Object.entries(rawMoves).reduce((output, [name, forms]) => filterForms({ output, name, forms, form }), []);
    return moves;
}
function getAbilitiesAndMoves(output, name, stats) {
    const abilities = getAbilities(name);
    const moves = getMoves(name);
    const newStats = Object.assign(Object.assign({}, stats), { ab: abilities, moves });
    output[name] = newStats;
    return output;
}
exports.filteredKeys = dexDataKeys.reduce(gen8NameFilter, []);
const gen8Mons = exports.filteredKeys.reduce(getGen8Monsters, {});
exports.pokedex = Object.entries(gen8Mons).reduce((output, [name, stats]) => getAbilitiesAndMoves(output, name, stats), {});
//# sourceMappingURL=vgcPokedex.js.map